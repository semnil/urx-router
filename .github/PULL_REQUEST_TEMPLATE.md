...
- [ ] Assumptions this PR rests on are listed here, each with what would settle it (a measurement,
      ...
      Open each item with the external thing its observation needs — `[hardware]` a reading
      off the unit, `[operator]` their own hands, eyes or cursor, `[other-checkout]` the
      machine this one is not. An observation needing none of the three is one to take
      before opening this, so it has no tag to write; `pnpm check:pr-assumptions` refuses
      an untagged item
