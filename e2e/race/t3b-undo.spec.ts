...
test.describe("T3b undo", () => {
  ...
  //
  // Three facts are pinned around this, and NONE of them is a guard for the reason —
  // the ledger entry says so and stays `unguarded`. The refusal that would fire if such
  // an entry existed (`src/ui/history.test.ts`, "refuses a rate change while a live
  // session holds the rate") sets `rateLocked` itself, so it asserts the branch, not the
  // unreachability. The picker's locked state (`src/main.device.test.ts`, "locks the
  // model picker and the other device actions for its duration") and reset dropping both
  // stacks (`history.test.ts`, "reset > drops both stacks") are the two ends. What
  // nothing holds is the step between them — that the readback path actually calls reset
  // — and an attempt to pin it from the app entry was withdrawn as vacuous on
  // 2026-08-13. The measurement is in `reference/work/e2e-flakes.md`, under the withdrawn
  // pin's own section.
