...
describe("sync-merged, the ignored-content split", () => {
  ...
  // EVERY tracked .gitignore, not the root one: a nested file's patterns are ignore rules of
  // this repository like any other, and read only at the root they were classified by nothing.
  // `src-tauri/.gitignore` carries three, two of which had been put in the rebuildable list by
  // hand when someone hit them — and the third, Tauri's generated permissions, kept a worktree
  // holding nothing but build output. Asked from git rather than listed here, since a list
  // answers which files were thought of rather than which the repository carries.
  const IGNORE_FILES = execFileSync("git", ["ls-files", "*.gitignore"], { cwd: REPO, encoding: "utf8" })
    .split("\n")
    .filter((f) => f !== "");

  // A pattern anchored with a leading slash names a path under its OWN file's directory, so
  // `/target/` in src-tauri is `src-tauri/target` and not the root's, and that anchoring slash is
  // what joins the two. One that is not anchored matches at any depth and stays a name or an
  // extension.
  const entries = IGNORE_FILES.flatMap((file) => {
    ...
    return readFileSync(join(REPO, file), "utf8")
      ...
      .map((raw) => {
        ...
        return {
          file,
          raw,
          key: dir && anchored ? `${dir}${raw}` : raw,
          path: dir && anchored ? `${dir}/${bare}` : bare,
        };
      });
  });
  const patterns = entries.map((e) => e.key);

  // Classification reads the entry's own path. A key is what LOCAL_WORK spells, and for a nested
  // pattern the two are different strings.
  const rebuildable = ({ path }) => {
  ...
  it("reads every .gitignore the repository tracks, each with patterns in it", () => {
    // Two emptiness assertions follow, and an unreadable file satisfies both.
    expect(patterns.length).toBeGreaterThan(10);
    expect(patterns).toContain("/plans");
    // …and the nested one is really in the set, or reading "every" file is satisfied by one.
    expect(IGNORE_FILES).toContain("src-tauri/.gitignore");
    expect(patterns).toContain("src-tauri/permissions/autogenerated/");
  });

  it("classifies every pattern in every .gitignore as one or the other", () => {
    expect(entries.filter((e) => !rebuildable(e) && !LOCAL_WORK.includes(e.key)).map((e) => e.key)).toEqual([]);
  });

  it("classifies none of them as both", () => {
    // Each pattern is asked both halves: its key against the list, its path against the module.
    expect(entries.filter((e) => rebuildable(e) && LOCAL_WORK.includes(e.key)).map((e) => e.key)).toEqual([]);
  });
});
