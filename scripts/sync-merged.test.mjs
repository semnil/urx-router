...
describe("sync-merged, the ignored-content split", () => {
  ...
  // `/target/` in src-tauri is `src-tauri/target` and not the root's. One that is not anchored
  // matches at any depth and stays a name or an extension.
  ...
  const rebuildable = (entry) => {
    const path = typeof entry === "string" ? entry.replace(/^\//, "").replace(/\/$/, "") : entry.path;
