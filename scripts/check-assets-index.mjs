...
const PROSE_TOKENS = new Map([
  ...
  ["core.excludesFile", "a git configuration key, not a path"],
  ["target/", "the directory the fixture's own symlinks point at, which lives in a throwaway repository"],
...
export function classifyToken(token, { isRowExport = () => false } = {}) {
  ...
  if (/^(window\.)?__[A-Za-z0-9_]+$/.test(token)) return "handle";
  if (/^--[a-z]/.test(token)) return "flag";
  if (/^[A-Z][A-Z0-9_]*=\S+$/.test(token)) return "envAssign";
  if (token.includes("/") || SOURCE_EXT.test(token)) return "path";
  // A name exported by a file the row heads. Tied to the row because the names are
  // ordinary words — accepting `wire` or `test` section-wide would pardon it on a row
  // about a file that exports no such thing.
  if (isRowExport(token)) return "rowExport";
  // The same oracle as `NAME=value`, for a variable the section names on its own — which
  // is how it names one it does not set. It is the LAST shape tried and it requires an
  // underscore, both measured: this section shouts in prose, and a rule of "any all-caps
...
if (isEntry(import.meta.url)) {
  ...
  function ignoredSet(paths) {
    ...
    if (res.error || (res.status !== 0 && res.status !== 1)) {
      finding(
        DOC,
        `git check-ignore is unavailable, so private paths cannot be told from missing ones (${res.status})`,
      );
      return new Set();
    }
  ...
  for (const span of spans) {
    ...
    const kind = classifyToken(token, {
      isRowExport: (name) => (firstCellSpans.get(line) ?? []).some((anchor) => exportOwners.get(name)?.has(anchor)),
    });
    if (kind === "prose") {
      usedProse.add(token);
      continue;
    ...
    switch (kind) {
      ...
      case "path":
        ...
        break;
      case "rowExport":
        // Checkable in the same sense as a path: it stops resolving the moment the export is
        // renamed or dropped.
        checked++;
        break;
      case "env":
