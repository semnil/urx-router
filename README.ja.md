...
[![フロントエンドのテストカバレッジ](https://codecov.io/gh/semnil/urx-router/branch/main/graph/badge.svg)](https://codecov.io/gh/semnil/urx-router)
...
（デモ版は保存ダイアログと画像出力に非対応。代わりに **計画 JSON をウィンドウにドロップして開く**、
計画の共有 URL 生成、デスクトップ版で開ける計画 JSON のダウンロードに対応）。
...
**URX44** は同一と仮定、**URX22** はそこからの推測。例外として **URX22** のコンソールのライブメーターのルーティングは
実機で確認済みだが、制御（書き込み）マップと工場初期プランは未検証。書き込みは実機の現在の設定を上書きする
（[免責事項](#免責事項)を参照）。外部 MIDI コントローラーをコンソールのコントロールに割り当てることもできる
（**デバイス → MIDI コントロール**）。`--experimental` を付けると、これに加えて破壊・復元方式のセルフテスト診断・
`.urxf` 設定ファイルの読み込み・読み取り専用の **デバイスと比較** が有効になる。
...
- スタジオラック調 UI / ダーク・ライトの 2 テーマ切替 (既定は OS のカラースキームに追従)
...
pnpm build:demo     # ブラウザデモ版をビルド (VITE_DEMO=1: 保存ダイアログ・画像出力なし)
pnpm preview        # デモ版を http://localhost:4173 で配信 — 公開デモと同じビルド
```

計画 UI は純フロントエンドのため、Rust 未導入でも `pnpm dev` でブラウザ動作確認できる。
デスクトップビルド (`pnpm tauri dev` / `pnpm tauri build`) には [Rust](https://rustup.rs/) が必要。
`pnpm dev` は通常ビルド。機能を絞った **デモ版**（<https://urx-router.semnil.com> と同じもの）を
ローカルで見るには `pnpm build:demo` の後に `pnpm preview` を実行する。

全コマンド一覧（単体/E2E テスト・整形・キャッシュ削除）と開発上の注意は [CLAUDE.md](CLAUDE.md) にある。

UI は英語を基準に日本語ローカライズを持つが、**本体の画面にあるコントロールを写した行は翻訳しない** —
URX は 3 つの表示言語のどれを選んでも当該画面が英語のままなので、`Threshold` / `Attack` / `Freq` /
`Pan` などは日本語 UI でも同じ表記になる。これはレビューではなくコンパイラが強制する: `src/i18n/en.ts`
の全文字列が `dev()`（実機ラベル）・`fixed()`（レイアウト上の理由で全言語同一）・`tr()`（アプリ自身の
文言）のいずれかで包まれており、`dev()` の文字列に訳語を書くとビルドが失敗する。詳細は
[CONTRIBUTING.ja.md](CONTRIBUTING.ja.md#規約) を参照。

既定では非表示の診断（実機セルフテスト・`.urxf` 設定ファイルの読み込み・読み取り専用の **デバイスと比較**）を
有効にするには `--experimental` を付けて起動する:
