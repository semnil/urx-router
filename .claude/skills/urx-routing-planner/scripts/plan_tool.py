...
def fx_effect_warnings(node_id, fx, out):
    ...
    if not survives_sanitizer(fx):
        ...
        return True
    # Array slots 1 and 2 are not fields of this section: no control of the unit's own reaches
    # either, so the app neither reads them nor writes them. A document naming one loses the key
    # at the load, whatever its value, so the removal is reported here rather than the value
    # being checked — a warning about the value would say the value is wrong, and it is the KEY
    # that is.
    for key, slot in (("on", 1), ("level", 2)):
        if key in fx:
            out.append(
                (
                    f"{node_id}.fxEffect.{key}",
                    f"array slot {slot} is not a parameter this app carries — no control of the "
                    "unit's own reaches it, so the app neither reads it nor writes it and the load "
                    "removes the key. The unit keeps whatever it holds at that address",
                )
            )
...
def node_param_warnings(plan, nodes, pairs, fx_channels, param_space=None):
    ...
    for node_id, params in (node_params or {}).items():
        ...
        # naming a single parameter resets the effect exactly as one naming a type does. Leaving
