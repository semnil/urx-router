...
**Another blind spot — a channel Ducker key ignores that channel's fader/mute.**
A `key` wire from a channel taps the channel's Rec Point, the same
pre-fader/pre-Ducker tap as a direct out, so fading or muting the trigger channel
...
pair holds one effect between the two channels rather than one each. Give the two
members the SAME `insertFx`, `insertFxOn` and `insertFxParams`, or leave both out:
the unit has one of each for the pair, so a document that disagrees with itself is
refused by the app and by `plan_tool.py`. Changing
...
not deliberately setting. **Omitting a parameter does not leave the unit's value
alone**: the app completes a loaded document from the model's factory values, and
the write sends them like any other. What omission buys is a document a person can
read, and what it costs is the unit's own setting for that key. Where the user
...
`plan-schema.md` carries the detail, and both are worth surfacing to the user:
   - **Raw-encoded** — `ssmcs` and `insertFxParams` hold the device's own internal
     units, not an authorable scale. **Do not author one**: the values are not on a
     scale a plan can reason about. Omitting one does not hold the unit's value
     either — it is completed from the model's factory values like every other key
     — so where the user wants what is on the unit, have them dial it in there and
     fetch the plan back.
   - **`fxEffect` and `insertFx` are selectors, and a selector write is not
     reversible.** The device refills that effect's whole parameter array with the
     new type's defaults, and selecting the old type back does **not** bring them
     back. Omitting the section does not avoid this: the factory type is filled in
     and sent, so an FX channel the document never mentions is reset to factory.
     The app states this before it writes — the confirm names every strip whose
     values the document did not describe — but the plan you hand over decides what
     that dialog has to say. Where the user's FX settings matter, the plan has to
     carry them: fetch from the unit, then edit.
