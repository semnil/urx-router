...
- `src/core/` — `plan-history.ts` the reversible plan patch behind undo / redo (a **diff, not a snapshot**,
  ...
  snapshot and the unit's announcements in `live.ts`, and against the round's own read in `client.ts`). A
