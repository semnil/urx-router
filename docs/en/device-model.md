...
detents jump -16 / -14). The sliders walk this grid **by index**, giving each detent equal travel so the
dense steps near 0 dB are not cramped (`core/levels.ts`). **Written over the control link, though, the unit
RETAINS an off-grid value rather than rounding it** — measured on a URX44V, 6/6 across two level parameters,
each written value read back unchanged and announced by its own notify. So the grid is what the unit's own
UI offers, not what its storage accepts, and a readback is deliberately not snapped on the way in
(architecture.md): the plan says what the unit actually holds.
...
- **A channel key is that channel's CH OUT — the same Rec Point tap as a direct out, ahead of its
  ...
  (STEREO / MIX) is the bus's own OUT, i.e. **after its output insert FX** — the block diagram labels
  `STEREO OUT` / `MIX 1 OUT` / `MIX 2 OUT` on the far side of the `INS FX` block, and those are the
  names the `DUCKER 1-4 SOURCE` block takes as its inputs. The planner notes this on a channel-sourced
  key wire in the inspector.
- **A stereo key is summed to mono before the detector — a sum, not an average and not a louder-side
  pick.** The block diagram puts a `MONO` block between the stereo sources (CH 5/6-11/12 OUT, STEREO
  OUT, MIX 1-2 OUT) and the Ducker Source select; measurement settles what it does with the two sides.
  So the level the ducker compares against its threshold sits **6 dB above either side alone** for
  correlated material, and 3 dB above for uncorrelated — a centred source triggers a ducker that the
  same source hard-panned would not. Only the MONO IN channels bypass the block.
...
- The mono CH and stereo CH structure is fixed (only the count varies per model). A MONO IN pair
  ...
  to either channel is auto-mirrored to the other (the node params except the three below, plus each
  ...
  flags live on the primary alone. **A.Gain, Clip Safe and the polarity invert are each member's own, in
  either mode**: with the pair linked, writing one member's left the partner's where it was, in BAL as well
  as in PAN. The app's mirror keeps those three with the partner for that reason; the pair's remaining
  head-amp values, +48V and Hi-Z, are mirrored with everything else. The metering answers to Signal
