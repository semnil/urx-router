...
- Every EQ (input channels and output STEREO / MIX buses) has a **1-knob** mode where one knob drives
  ...
  writing a type forces the 1-knob level to that type's neutral point and sets each band's ON (Loudness
  all four on, Vocal LOW off and the other three on). When 1-knob is on
