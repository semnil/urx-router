...
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。3 分割。`src/**` / `e2e/**` / `playwright.config.ts` を触る PR と、バージョン更新 PR |
...
走るのは **`src/**` / `e2e/**` / `playwright.config.ts` を触る PR と、アプリのバージョンを変える PR**。
片方がもう片方を含まないので 2 つの節に分かれている。バージョン更新 PR は他に何も載せない = どのパスも触らず、
マージがそのままリリースのタグ生成 (`tag-release.yml`) になる。この節がハーネスをユーザーが実際に
インストールする唯一のコミットの直前に立たせている。タグの後ろではなく前である点が本質で、リリース時に
回すと既に存在するタグについて報告することになる。`paths: package.json` でも「バージョン更新」は
意味しない (Dependabot が毎週そのファイルを触る) ため、`race.yml` の `detect` ジョブが PR の前後で
`version` フィールドを比較する。
...
`playwright.config.ts` を入れてあるのは、`race` と `race-webkit` のプロジェクト定義がそこにしか無く、
一方 `pnpm check:skips` が収集するのは `race` プロジェクトだけだから — `race-webkit` が別エンジンを
選ぶ / 何も収集しなくなる編集は、通常の PR ではどのチェックも報告しない。
...
上の 80 マージの標本では 52 件が該当する (`src/**` が 42 件、`e2e/**` が 30 件、`playwright.config.ts`
が 2 件。重複あり)。
