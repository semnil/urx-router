...
ヘッダを持ち込む — ヘッダ集合がシャードごとちょうど 1 本、かつ本数の合計がこのチェックアウトの
収集数、でなければ落とす。これはいつ書かれたログにも効き、既存のログも読める。**届かないのは**、
ヘッダがシャードごとちょうど 1 本になるよう組んだ継ぎ合わせで、2 走行から配列が exit 0 で出ることを
実測した。そこを決着させるのが `race.yml` がテストの手前で echo する run id と attempt で、今後
書かれるログに効く (Playwright は走行を識別する出力を持たず、ファイルから読んだログにはタイム
スタンプも無い)。
