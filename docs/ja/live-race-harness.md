...
テアダウン自体は変えていない。`releaseLive` は切断を await せず繋ぐ (`vd_disconnect` は待てる値を
返さない) 設計で、遅れて届いても安全なのは epoch ガードのおかげ。`end()` から切断が Rust に届くまでの
窓は今も開いていて、世代が閉じるのは「その窓へアプリ自身が送り続けるかどうか」の方。
