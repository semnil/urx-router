...
- **CI の 3 シャードはテストの「本数」で等分されており、ティアごとのコストは等しくない**。Playwright は
  順序付きリストを本数の等しい連続した塊に切る (各 55 ケース、ファイル境界も跨ぐ) ため、分割はファイル名に
  従い、ファイル名はティア順に従う。ある走行での実測: 同じ 55 ケースずつで、shard 1 のテスト時間は
  **749 秒**、shard 2 が 253 秒、shard 3 が 315 秒。スケジューリングの偶然ではない — T1 (overtake) と
  T2 (shape change) は約 800 コマンドの全デバイス読み戻しを誘発し、その最中にバリアを保持するティアで、
  1 ケース 33〜48 秒。順序上隣接しているのでまとめて同じ塊に落ちる。ワークフローの所要は最遅シャードで
  決まるので、これは**リリース 1 回あたり約 2.5 分**の差になる (7m18s 対、時間で均した場合の約 4m35s)。
