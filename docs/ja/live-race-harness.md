...
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。3 分割。`src/**` か `e2e/race/**` を触る PR と、バージョン更新 PR |
...
走るのは **`src/**` か `e2e/race/**` を触る PR と、アプリのバージョンを変える PR**。片方がもう片方を
含まないので 2 つの節に分かれている。バージョン更新 PR は他に何も載せない = どちらのパスも触らず、
...
`src/main.ts` だけだった。`detect` は API のファイル一覧ではなく `git diff --name-only` をベースに
対して読む。バージョン比較のために全履歴を既に取得しており、diff には切り詰めの天井が無い —
`ci.yml` の `preflight` が守っている天井は、このジョブには存在しない。
...
上の 80 マージの標本では `e2e/race/**` を数に入れて 45 件が該当する。
