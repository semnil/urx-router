...
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。3 分割 |
| `race-webkit` | 2.1 分 | 48 秒 | `race.yml`。専用ジョブ |
...
通常の PR ティアから外した理由で、6.5 分の従来スイートは「マージ後に回す」より「PR ごとに回す」方が
安いが、25 分はそうならない。ティア分割を切って以降どちらの数値も増えたが比率はほとんど動いておらず、
境界が実際に依っているのはその比率。
...
するタグについて報告することになる。`paths: package.json` でも「バージョン更新」は意味しない
...
チェックランを 1 つも作らないため、他の PR には存在すらしないチェックをバージョン PR だけが待つ、
という状態にはできない。その 1 本以外ではハーネスの 2 ジョブが自分の条件でスキップし、数秒で成功を
報告する。この配置と「必須チェックはすべての PR で報告できなければならない」理由は CLAUDE.md の
...
表面化し、絞り込みはライブ同期面の `git log` が担う。すべての PR が `race-required` を待ちはするが、
バージョン PR 以外ではそれはスキップされた **2** ジョブの上のゲートで、数秒で緑になる (`detect` は走り、
`race` と `race-webkit` が自分の条件でスキップする — **ジョブ自身の `if:` が偽の matrix ジョブは展開
されない**ので、`race` は 3 回ではなく 1 回だけ報告する)。マージ条件が買っているのは「赤いハーネスの
上でリリースをタグ付けできない」ことであって、全ブランチにハーネスの費用を払わせることではない。WebKit はブラウザのダウンロードが別なので
...
- **何も壊さないアプリ側の変更でケースが無効化されることがあり、それを隠すのがティア分けである。**
  ...
  永遠に解決しない `selectOption` を待ち続けていた。誰も報告しない: ハーネスはバージョン更新 PR で
  しか走らないので、以降どの PR でも `race` ジョブは**スキップされることで**成功を報告し、破綻が
  表に出たのは on-demand ディスパッチのときだけだった。「遅れる」を「見えない」に変えた性質は 2 つ —
...
- **init script は別世界であり、それを教えてくれる仕組みはツールチェインのどこにも無い。**
  疑似デバイスの launch flag への応答を、インラインの `switch` 節から `FAKE_LAUNCH_FLAGS_OFF` の
  参照へ移した — しかしそれを読むコードは `page.context().addInitScript` に渡す関数の内側にあり、
  この関数は文字列化されて**ページ側**で評価される。駆動側のモジュール束縛はそこに存在しない。
  それでもコンパイルは通り、`pnpm typecheck:e2e` も通り、`--list` は全ケースを収集する。破綻は
  実行時に、疑似デバイスが処理する**最初の**コマンドで `Can't find variable:
  FAKE_LAUNCH_FLAGS_OFF` として現れる。そして表に出る形はエラーメッセージではなく不在である:
  ...
  ケースが同じ 30 秒タイムアウト)。失敗文はロケータしか名指ししないので、前の項と同じくそれだけでは
  基盤側のハングと見分けが付かない。見分けたのは失敗した実行自身のトレースに残る page console 行
  `Can't find variable: FAKE_LAUNCH_FLAGS_OFF` (CI run 31635304181 の `trace.zip`) で、
  同定はそこから行った。init script が必要とするものは引数タプルで渡す。規則はそれだけで、いまは
  現場のコメントに書いてある。これを書いた PR とともに `main` に入り、バージョン更新で再び
  ハーネスが走るまで、さらに 3 マージ分を生き延びた
