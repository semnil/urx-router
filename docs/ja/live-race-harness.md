...
- **CI の 3 シャードはテストの「本数」で等分されており、ティアごとのコストは等しくない**。Playwright は
  ...
  従い、ファイル名はティア順に従う。**2026-08-11 に `d85af81` で実測**: shard 1 のテスト時間が
  **752 秒**、shard 2 が 272 秒、shard 3 が 451 秒 (それぞれ 56 / 51 / 54 ケース)。スケジューリングの
  偶然ではない — T1 (overtake) と T2 (shape change) は約 800 コマンドの全デバイス読み戻しを誘発し、
