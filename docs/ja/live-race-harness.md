...
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。3 分割。`src/**` / `e2e/**` / `playwright.config.ts` / `package.json` / `pnpm-lock.yaml` を触る PR と、バージョン更新 PR |
...
走るのは **`src/**` / `e2e/**` / `playwright.config.ts` / `package.json` / `pnpm-lock.yaml` を触る PR
と、アプリのバージョンを変える PR**。節は 2 つで、`detect` は最初に当たった方で返さず両方を評価して
両方を出力する。`race.yml` の `detect` が持つ `case` 一覧に `package.json` が入っており、バージョン
更新はそのファイルの編集そのものなので、パス側の節が必ず先に真になる — 早期 return にすればバージョン
比較へ制御が渡る経路は残らず、それでも「ハーネスをリリースの直前に立たせているもの」として読まれ続ける。
この節を残してあるのはまさにその表明だからで、かつパス集合を後から狭められても生き残る側だから。タグの後ろではなく前である点が本質で、リリース時に回すと既に存在する
タグについて報告することになる。トリガーの `paths: package.json` では「バージョン更新」を意味しない
(Dependabot が毎週そのファイルを触る) ため、比較はフィルタではなくジョブにある。

パス側の節が、破壊を起こしたマージの位置で捕まえる側。ライブ同期のファイル群ではなく `src/**` 全体
なのは、狭い規則では記録に残る 1 件の事故を取り逃がすため — ライブセッションのロック下で解決しなく
なった `page.selectOption` が、このゲートを緑にしたまま 17 マージを通過した。その PR が触っていたのは
`src/main.ts` だけだった。`e2e/race/**` ではなく `e2e/**` なのは 1 段外側で同じ理由で、
`t0b-sweeps.spec.ts` と `t2b-shape-change.spec.ts` が `e2e/race/` の外にある `e2e/graph-helpers.ts` を
import している (CLAUDE.md のアセット表がハーネス側の集約先として名指ししているファイルそのもの)。
`playwright.config.ts` を入れてあるのは `race` と `race-webkit` のプロジェクト定義がそこにしか無いため。
`pnpm-lock.yaml` は Playwright のバージョンを決め、それを `.github/actions/playwright-version` が
両ハーネスジョブの `container:` イメージタグに変えるので、依存更新はスイートの下のブラウザビルドを
差し替える。`package.json` はそれを駆動するスクリプトの定義元。
...
実測で決めた: `-c core.quotePath=false` は、git が非 ASCII バイトを含むパスを C クォートで出力し
(`"src/\346\227\245..."`)、先頭の `"` が `src/` の一致を外すため。`--no-renames` は、rename 検出が
移動したファイルを移動先だけで出力し、モジュールを `src/` の外へ出す変更が「`src/` の変更なし」と
読めてしまうため。どちらもハーネスをスキップさせた上で**偽の理由を出力する**。そのログはスキップと合格を
区別する唯一の手掛かりなので、そこが効く。

**コメントのみの差分はトリガーから除外していない。** 除外案は落とす前に測ってある: `main` の直近 80
...
上の 80 マージの標本では 61 件が該当する (`src/**` 42 件、`e2e/**` 30 件、`package.json` 11 件、
`pnpm-lock.yaml` 3 件、`playwright.config.ts` 2 件。重複あり)。
