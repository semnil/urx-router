...
- **前提条件はジェスチャではないが、ページ全体に掛けた fail-fast の上限はそこまで届く**。`t0b-sweeps`
  ...
  ティアができて以来 WebKit 走行のたびに `page.click("#btn-device")` でちょうど 4000 ms のタイムアウトを
  出していた (1.6.0・1.6.1・その次の走行。1 回目は 9.0〜9.4 秒、リトライは常に緑)。flaky に見えるが
  flaky ではない — **1 回目が決定的に落ち、リトライは必ず通る**。機構は actionability 判定そのもので、
  これは同じ矩形が 2 フレーム連続することを求める `requestAnimationFrame` ループであり、ヘッドレス
  エンジンは document 開始直後のフレームを疎にしか出さない (macOS WebKit で実測: 最初の 187 ms で
  2 フレーム、温まった後は約 16 ms 間隔)。アプリ側の問題ではない — ボタンの矩形は 92 フレーム連続で
  1 バイトも動かず、同じ `goLive` が上限なしの他の 4 つの `@webkit` ケースでは通っている。`goLive` は
