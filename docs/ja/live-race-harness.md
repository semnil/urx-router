...
天井は、このジョブには存在しない。その diff に付けた 2 つのフラグは効いており、いずれも推測ではなく
実測で決めた: `-z` は、git が literal に出力できないパス (非 ASCII バイトに加えて `"`・バックスラッシュ・
制御文字) を C クォートで出力し、`"docs/\346\227\245..."` の先頭の `"` がそのパスをドキュメントに
見えなくするため。`--no-renames` は、rename 検出が移動したファイルを移動先だけで出力し、モジュールを
`docs/` へ移す変更が「ドキュメントのみ」と読めてしまうため。`core.quotePath=false` は前者の非 ASCII の
分しか塞がない。`-z` は一時ファイルを要する — NUL はシェル変数に入らない。
