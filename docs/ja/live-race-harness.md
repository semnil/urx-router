...
**この起動条件がマージ待ちに足す時間。** 2026-08-14、run `31794858430` (`workflow_dispatch` / main、
conclusion は success) の実測。ここだけマシン時間ではなく実時間:

| ジョブ | 実時間 |
| --- | --- |
| `detect` | 9 秒 |
| `race-webkit` | 2 分 00 秒 |
| `race` shard 2 / shard 3 / shard 1 | 3 分 08 秒 / 5 分 08 秒 / 7 分 02 秒 |
| `race-required` | 3 秒 |
| ワークフロー全体 | **7 分 23 秒** |

Playwright はケース数で分割し重いティアが固まるので、ワークフローの実時間はマシン時間の 3 分の 1 では
なく最遅シャードの値になる。並べると `ci.yml` の直近 40 PR 実行 (同日) は中央値 3 分 02 秒、40 件中
38 件が 2 分 32 秒〜3 分 56 秒に収まり、残り 2 件は重いジョブが走らない docs のみの実行。必須ワークフロー
は並列に走るのでマージはその最遅を待つ — **この起動条件の前が約 3 分、ハーネスが走る PR では約 7 分半**。
同じ 80 マージのうち該当は **75 件** — 走らない 5 件がドキュメントのみのマージ。これに置き換えられた
2 つの監視リストはそれぞれ 52 件と 61 件で、61 と 75 の差 14 件のうち 4 件が上表の 3 度目の漏れに
当たる。
...
- **CI の 3 シャードはテストの「本数」で等分されており、ティアごとのコストは等しくない**。Playwright は
  ...
  **752 秒**、shard 2 が 272 秒、shard 3 が 451 秒。うち実行されたのは 56 / 51 / 54 件で、残りは
  ...
  ワークフローの所要は最遅シャードで決まるので、これは**リリース 1 回あたり約 2 分**の差になる
  (6.4 分 対、時間で均した場合の約 4.2 分)。これが置き換えた読みも同じ形 (749 / 253 / 315 秒) だったので、
