...
| 段 | 内容 |
| --- | --- |
| 1 | トレース Bus + フロントのタップ + contract テスト + `ci.yml` の drop guard |
| 2 | オフライン解析器 + 不変条件 |
...
| 11 | メーター購読の排他 | 生きた登録は常に 1 つ。自分が持たないストリームを解除しない |
...
13 には、競合ではなく**受け渡し**であるキーが 1 つある。FX チャンネルの `fxEffect` である。ライブ
セッション中に EFFECT TYPE を変えると、出ていくエフェクトを先に実機から読み、その後でタイプを書く
(`live.ts`・flush が自分の先頭書込の手前で取る park)。したがって実機がプランの知らない値を持っていた場合、1 つのジェスチャの
中でそのキーに `follow-scoped` の書込と `ui` の書込が並ぶ ── これはまさにこの不変条件が報告する対の形
である。順序こそがこのジェスチャの目的で、デバイス側が先に走るのは構造上決まっている。渡された値を
そのまま返す疑似デバイスではそこに何も書かれないので、`t2d-shape-change` が見る書き手は `ui` の 1 つ
だけになる。デバイス側の値を植えてからタイプを変えるケースなら両方が見える。**あらゆる収束が同じ読みを
スコープの残りに対して取る** (`live.ts` の `parkSilent`) ので、この受け渡しはジェスチャが名指ししたノードに
限らずフラッシュが覆うどのノードにも現れうる。そこでもデバイス側が先で、先頭自身のノードは外れる ──
収束が復元するために存在するノードだからである。

6 は当初「毎フラッシュで一致するか」を検証する条件だった。このハーネスのほとんどの期間それは成立
しておらず ── 追従の登録は `begin()` と照合完了後にしか更新されなかったので、アプリ側の構造編集は
登録を古いまま残した ── **設計通りの挙動**として扱われていた。SSMCS モード変更が自分のプリセット
書込への返事を聞けないと分かって、その扱いが終わった。集合を動かしたフラッシュは自分の末尾で登録し
直すようになり、当初の言い方が再び成立する ── 依頼位置は**フラッシュの末尾**で、内側では行わない
(`live.ts` が `followSetStale` を消費するのは flush の末尾、順序の固定は `live.test.ts` の
"does not re-register before the refetch it shares a flush with")。書込はいまも、同じフラッシュの
中でそれを覆う購読より先に出る。疑似デバイスがブリッジの登録集合
フィルタを持って以降、この条件は 3 節構成である。番号は表・解析器・参照している全ケースを揃えた
まま保つため据え置く。
...
- **節 B ── 育った窓**。`emitted ∖ registered` = アプリが現に書いているのに、デバイス側変更を
  ブリッジが落とすアドレス。`capture()` が集合を動かしたフラッシュは自分の末尾で登録し直すので、
  clean は「集合を動かしたジェスチャの中で登録が追いついた」を意味する。`subscribe()` が `begin()` と
  照合完了後にしか再登録しなかった頃は、構造を変える編集が何かが照合するまで窓を開けたままにし、
  この節がそれを報告していた。clean 側の表明の隣に**発火対照**を置くケースが 2 つある
  (`t2` の 1-knob OFF フラッシュ、`t2d` の FX スロット族): 同じスナップショットを 1 フラッシュ前の
  登録と組んで渡す ── アプリが以前残していた古い組そのもの ── ので、「clean」が「もう発火できなく
  なった節」であることはない。発行集合は書込からではなく
...
別の class を出すのは 2 つで、条件 6 の内訳は節 A が `case`、節 B が `product`、節 C が `harness`、
条件 16 はトレースに無い quiesce の印を `harness` として報告する。
...
| `overtake-foreign-notify-inside-our-write` | console | 自分の書込が送信中のアドレスに、その告知より前に届くデバイス側の値。窓の中の 2 つ目の編集の有無と、ノードの読み直しの期限が書込の ACK と告知の間に来る形で分ける |
| `overtake-foreign-notify-with-an-edit-on-the-strip` | console | 同じ上書きされる値に、ノードの別のコントロールへのまだ送っていない編集が重なる形。追従の settle を越えて書込を止める・すぐ解放する・ノード上の実機側の変更と並べる・flush の窓の中で編集する、で分ける |
| `overtake-edit-in-the-flush-window-vs-follow-read` | console | 通常の実機側の変更がノードを読み直す期限に、まだ flush の窓の中にある編集 |
...
| `shape-string-path-writes` | 混合 | 差分機構を完全に迂回する書込。無害な例と重大な例を並置。デバイス側リネームは配達されるようになり、Sweet Spot Data のプリセット書込も配達され、refetch が直す |
...
| `drop-unknown-address-notify-storm` | 混合 | 3 本立て: ノードに置けない (`SAMPLE_RATE`) / ノードに置ける / 未登録 (拒否) |
...
| `teardown-flow-refusals` | 混合 | 守られた読みと守られていない読みを同じフロー (File > New・File > Open) で対比。機種切替は守られた側だけ — ピッカーを固めるライブセッションが無いのはそちらだけ |
...
| p | 同じ窓の**名前経路**版。`staleAfterWrite` は文字列アドレスにも効き、`vd_set_str` の後 n 回の読みが置き換えた前の名前を返す。URX44V で実測 81 ms = 文字列経路は例外ではなく、疑似デバイスはその生涯ずっと例外として扱っていた。**告知も項 o と同じ規則で出る。しかも両方の半分ともこの経路で実測してある** (数値からの流用ではない) — 値を変える名前書込は ACK から `announceMs` 後に告知され、窓を閉じる (2 走行 32 本が 32/32 とも自分の ACK の後・ACK から 1〜102 ms。大半は 66〜102 ms だが 32 本中 2 本が 10 ms 未満 = 数値側の分布に無い下側の裾で、原因は未同定)。同値の名前書込は何も告知しない (ACK 0 ms・2000 ms 沈黙・前後を値の変わる書込で挟んで、死んだストリームが沈黙した実機に化けないようにした上での結果)。名前のアドレスは登録集合に入っているので (追従が実機側のリネームを運ぶようになったとき加わった) アプリはこれを受け取る。Sweet Spot Data (param 91) も加わった — SSMCS のモード変更の flush がそれを購読する (`t2b-shape-change`)。`t1d-name-window` がこの窓を必要とした最初のケース |
加えて **バリア**: `blockAt({ cmd, nth })` で特定のコマンドを止め、`release()` で解放する。

項 n・o・p は疑似デバイスがその生涯ずっと**持っていなかった**ものであり、その欠落は中立ではなかった —
...
`announceMs` の**既定値**は live.ts の 120 ms フラッシュ窓より**下**に置く。通常のケースが揃って同じ 1 点を測る
のを避けるためで、これを超えると告知はドラッグの次の書込に追い越され、動いてしまったスナップショットに対して
届く。この追い越しは疑似デバイスの作り物ではなく**実機の挙動**であり (実測 ACK+58〜151 ms は 120 を跨ぐ)、
その後に起きていたことは告知ではなく**アプリの側の欠陥**だった: エコー判定がスナップショットの 1 値との一致比較
だったため、追い越された告知が実機由来の変更として読まれ、既に置き換えられた値をプランへ書き戻し、続くアイドル
全体照合がアンドゥ項目を全件消していた。`live.ts` は ACK 済みで未告知の書込を保持するようになり、`announceMs` を
意図的に 120 超へ上げるケースは `t3-undo.spec.ts` の `late echo of an overtaken write`。

さらに告知は**前方へ合流する**。**告知が出る前に**同じアドレスへの後続の書込に追い越された書込は、告知されない。
実機は最終的に保持した値を報告し、途中で通過した値については何も言わないので、1 つのアドレスへの連続した書込には
最後の値を載せた notify が 1 本だけ返る (URX44V で実測・`probe-overlap-announce{,2}.mjs`: 重複が保った 21 行が
21/21、2 つのエンコードクラス。3 連も 1 本。後続の書込が最初の告知より先に出られなかった行は逐次の 2 書込であって
重複ではなく、証拠として読まずに除外する)。疑似デバイスは保留中のタイマーを取り消すことでこれを表す。同じことの
言い換えで、**まだ発火していないタイマーは実機がまだ出していない告知**であり、既に出たものは取り消せない。
1 書込につき 1 本を出す形が、`settle.ts` に「義務のある書込ごとに告知 1 本」を要求する判定を出荷させた:
ドラッグは告知が返るより高い頻度でフラッシュするので、最後以外のすべての操作が「どこにも届かなかった書込」として
報告され、そのたびに全体掃引が張られる。ハーネスにはそれを言う手段が無かった。**実測で分離できていないのは、
生き残った 1 本がどちらの書込を起点にするか**である。重複が保った遅延 (2 本目を 1 本目の 0〜32 ms 後に発行) が、
告知遅延そのもののばらつき **1〜172 ms** (2 走行の全 44 サンプル・中央値 65) よりはるかに小さく、2 つの起点の
分布が重なる。ここに書く遅延はすべて**その告知が属する書込の発行**を起点に測る。この起点をバースト起点と
混ぜると、どちらでもない範囲になる。

これは上の追い越しについて明示すべきことでもある。合流が追い越しの境界を決めるからである。追い越しは
「遅れた告知」ではなく**窓**である — 追い越す側の書込の **ISSUE** (アプリのスナップショットが動く点) と
**ACK** (疑似デバイスが取り消す点) の間で、その幅はちょうど `latency.set` である。既定の 0 では窓が無く、
追い越しは起こり得ない。したがって `late echo of an overtaken write` は自前の set レイテンシを設定し、
**最初の書込の告知が実際に出たこと、かつ 2 番目の書込の発行より後に届いたことを assert する**。この assert が
無ければ、何も追い越されなかった走行でも通過する: 他の assert は「何も壊れていない」を言うだけで、それは何も
起きなかった走行の見た目と同じだからである。
...
**`pushNameNotify`** はその文字列経路版 ── 実機の LCD で行われたリネームであり、本文を `value_str` に、
数値の `value` に 0 を載せる。判定の返し方は同じで、**告知の前に名前を格納する**: 格納せずに告知するのは
「自分が持っていない値を報告する実機」であり、直後の読みが自分の notify と矛盾する。名前の**書込**が生む
告知 (項 p) と発行部を共有しているので、台本のリネームと実機自身のリネームが別々の実機を記述することはない。
なお `divergeAt` の文字列版は**無い** — 在ったものは最後の呼び出し元が `pushNameNotify` へ移った時点で
削除した。誰も走らせない操作面は、何もそれを真に保たない「実機についての主張」になるからである。
...
- 各ステップは**意図した位相と実際に達成した位相を両方**記録する。梯子は達成値が入って初めて
  解釈できる — そして段の配置は意図値ではなく達成値に対して決める。sleep からマークまでの
  駆動側自身のコスト (クリック、ロケータ解決) は全段に等しく加算され、しかも**一方向**である:
  ジェスチャを必ず後ろへずらすので、窓の縁の後ろに置いた段は頑健で、縁の手前に置いた段が
  遅いランナーに奪われる
- **可動域の N% を狙うジェスチャは、動きうる値を基準に狙っている。** `t0b` の
  `main fader jump-to-click` は可動域の 30%、`t3b` のダブルクリック前置は 32% を押していたが、
  どちらも目的は「フェーダーを動かすこと」だけだった。工場値 0 dB ではキャップ中心が可動域 25% にあり、
  このプロジェクトのビューポートではキャップが可動域の約 14% を占めるので、**どちらもキャップの内側**
  だった — メインフェーダーがジャンプではなくキャップを掴む形に変わった日に、両方とも無言で
  「何も書かないジェスチャ」へ化けた。既定の 800px 高ウィンドウなら外側なので、**ビューポートが
  変わるだけで意味が反転する**。狙いは対象の矩形から取ること。そしてコントロールに
  「ここを押しても書かない」側ができたら、その側を `silent: true` のジェスチャとして登録する —
  sweep の第 2 assert が両側を担うようになるのはそれによる
- **測る区間は判定を決める区間そのものでなければならず、この 2 つは取り違えやすい。** アプリ側の窓を
  張るジェスチャは、張った瞬間から測る — その次のステップからではない。`t1b` の飢餓ストリームは
  ティック 1 発目から間隔を採っていた (`at.slice(1)`) ため、`maxGap < DEBOUNCE_MS` のガードは
  「クリック→1 発目」という**判定を決める唯一の区間だけ**を覆っていなかった。余裕は 22〜30 ms で、
  CI のランナーがそれを奪った。同じ形は 1 ケースに留まらない: 繰り返しの**回数**が繰り返し間隔の
  代役になっている、`mark()` がアサーションの手前ではなくドライバ操作の手前に置かれていて操作自身の
  コストが次の段に入り込む、メッセージを配達する直前に採った位相がアプリの適用する位相の**下限**に
  しかなっていない (配達の両側で挟み、窓の内側に置く段は遠い側の端に対して判定する)。1 つの窓を共有
  しなければならないジェスチャと編集がドライバの往復で隔てられているなら、揃うことを期待せず
  **ページ内の 1 タスクで撃つ**
...
サブアーム数件だけで、それぞれ理由を隣に書いてある
(固定接続なので UI から削除できない / 判定を分離できる観測点が無い / `--experimental` の launch mode を
疑似デバイスに偽らせるとアプリではなくハーネスを測ることになる、等)。**その台帳は grep ではなく
`pnpm check:skips`** — Playwright が収集した結果を読むので `test.fixme` も 1 件に数え、パターンが
拾い損ねるファイルのケースも隠れられない。全件が `e2e/race/skip-ledger.json` に、理由を保つテストか
「何も保っていない」の明示のどちらかと結び付けて登録されていなければ落ちる。
...
**素の `--shard=k/3` は CI の shard k ではない**。CI はシャードを所要時間で切っており
(`PWTEST_SHARD_WEIGHTS`。下の「シャードの大きさの決め方」)、この変数が無いと同じフラグは本数等分の
区間を取る — 同じ名前で別のケース集合になる。赤くなったシャードが何を走らせたかを再現するには、台帳の
配列を前置する: `PWTEST_SHARD_WEIGHTS=$(jq -r '.collect.shardWeights | join(":")' e2e/race/skip-ledger.json)`。
...
プロジェクトの境界はそのまま CI のティア分割でもあり、そこで切った根拠が次の実測値。**2026-08-11 に
`d85af81` で測定**。`ci.yml` 1 回と、オンデマンドの `race.yml` 1 回のログから採った — ジョブあたり
2 ワーカー、`ubuntu-latest`、Playwright コンテナ内。マシン時間はレポーターが出す各テストの所要時間の
総和なので、シャードが分け合う実時間ではなく仕事量:

| ティア | マシン時間 | 最遅テスト | 実行元 |
| --- | --- | --- | --- |
| `chromium` — 従来のスイート | 6.5 分 | 5.8 秒 | `ci.yml`。3 分割。Markdown / docs のみの変更を除く全 PR と main への push (生成物の `model-*.md` は例外) |
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。所要時間で 3 分割 (下記)。ドキュメントのみの PR を除く全 PR と、バージョン更新 PR |
| `race-webkit` | 2.1 分 | 48 秒 | `race.yml`。専用ジョブ。条件は同じ |
| `app-webkit` — 従来ティアのレイアウト不変条件 | — | — | `ci.yml`。専用ジョブ。条件は `chromium` と同じ |

`app-webkit` に数値が無いのは、数値がこの境界を切っていないため。切っているのは**エンジン**であって
時間ではない — 中身は従来ティア自身のケースで、判定対象が「2 つのエンジンが同じようには実装していない
レイアウト規則」であるがゆえに macOS ビルドの描画エンジンで走らせ直している。アプリの外枠はスクロール
しない、溢れは名前の付いた内側のコンテナが持つ、到達し続けなければならないものは到達できる、の 3 点。
ストリップが自分の溢れをラックへ渡さず自分で切るようにする規則は、WebKit では CONSOLE の縦スクロールを
奪い、Chromium では何も起こさない — Chromium だけの緑は出荷物についての判定になっていない。ジョブが
使うのはケース数ではなく固定費 (イメージ・install・2 本のビルド) の側で、従来ティアの 4 本目のシャードに
しない理由もそこにある: WebKit は別のブラウザ取得であり、各シャードがそれを取った上で何も走らせない。

**従来ティアの値は予算ではなく幅として読むこと。** 同じ 428 件が同日の別実行では 5.2 分 — 上の値より
2 割低い。スイートが変わったのではなくランナーの揺れ。ハーネス側は安定しており (shard 1 が 2 回の実行で
12.5 分と 12.3 分)、これがどちらの数値も揺れを免れないのにティア間の比較が成立する理由。従来ティアは
**CI が走らせている形のまま**測っている — カバレッジ送信が入って以降それは `pnpm test:e2e:coverage`、
つまり非圧縮バンドルとネイティブ V8 収集を伴い、そのオーバーヘッドも PR ごとの実行コストの一部。

テスト件数は意図的に列から外している。テストを足す PR のたびに動き、実際この表が主張していた件数は
誰も気付かないまま従来ティアで 45 件ずれていた。そしてティア分割は件数からは導かれない — 切ったのは時間。

ハーネス単体で **E2E のマシン時間のおよそ 5 分の 4** を占める (上の従来ティア + race の 79%)。これが
`ci.yml` にシャードを 3 本足すのではなく、条件を前に置いた独立ワークフローにしてある理由で、6.5 分の
従来スイートは「マージ後に回す」より「全 PR で回す」方が安いが、25 分になると「この PR はそれを壊し
得るか」を先に問う価値が出る。ティア分割を切って以降どちらの数値も増えたが比率はほとんど動いておらず、
境界が実際に依っているのはその比率。

走るのは **差分がドキュメントだけの PR を除く全 PR と、アプリのバージョンを変える PR**。節は 2 つで、
`detect` は最初に当たった方で返さず両方を評価して両方を出力する。バージョン更新は `package.json` の
編集であり、それはドキュメントではないので第 1 節が必ず先に真になる — 早期 return にすればバージョン
比較へ制御が渡る経路は残らず、それでも「ハーネスをリリースの直前に立たせているもの」として読まれ続ける。
この節を残してあるのはまさにその表明だからで、かつ第 1 節を後から狭められても生き残る側だから。タグの
後ろではなく前である点が本質で、リリース時に回すと既に存在するタグについて報告することになる。トリガーの
`paths: package.json` では「バージョン更新」を意味しない (Dependabot が毎週そのファイルを触る) ため、
比較はフィルタではなくジョブにある。

**第 1 節を除外の形にしてあるのは、監視リストとして書く試みが 3 回とも漏れたから。** 順に読む価値が
ある。どの項目も指摘されて初めて足されたもので、そのどれもが「ハーネスを壊しうるのに `race-required`
は緑のまま」の PR だった:

| リストの内容 | 漏れていたもの | そのファイルが効く理由 |
| --- | --- | --- |
| `src/**` と `e2e/race/**` | `e2e/graph-helpers.ts` | `t0b-sweeps.spec.ts` と `t2b-shape-change.spec.ts` が import しており、CLAUDE.md のアセット表がハーネス側の集約先として名指ししている |
| — | `playwright.config.ts` | `race` と `race-webkit` のプロジェクト定義がそこにしか無い |
| + 上記 2 つ | `pnpm-lock.yaml` | Playwright のバージョンを固定し、それが両ハーネスジョブの `container:` イメージになる |
| + `package.json` と lockfile | `race.yml` 自身・`.github/actions/install-browsers/`・`.env.trace`・`vite.config.ts`・`scripts/e2e-serve.mjs` | ジョブ定義・ブラウザ導入・trace ビルドフラグ・ティアが serve するバンドル・その serve 側 |

`src/**` を最初からディレクトリ全体にしてあるのも同じ class の理由による — 記録に残る 1 件の事故
(ライブセッションのロック下で解決しなくなった `page.selectOption` が、このゲートを緑にしたまま 17
マージを通過した) の PR が触っていたのは `src/main.ts` だけだった。ハーネスが読むものの境界は、誰も
一度で正しく引けていない。リストの失敗様式は「誰も足さなかった依存」で、除外の失敗様式は「誰も必要と
しなかった 1 回の実行」なので、後者に倒してある。`detect` は `ci.yml` の `preflight` と同じ規則・同じ
順序・同じ例外を適用する: ドキュメントとは `docs/**` と `*.md`、ただし生成物の `references/model-*.md`
は `skill-export.test.ts` が生成器と差分照合するのでコード扱い。

`detect` は API のファイル一覧ではなく `git diff --name-only` をベースに対して読む。バージョン比較の
ために全履歴を既に取得しており、diff には切り詰めの天井が無い — `ci.yml` の `preflight` が守っている
天井は、このジョブには存在しない。その diff に付けた 2 つのフラグはいずれも推測ではなく実測で決めたが、
問いが除外の形になった時点で守る向きが別々になった。**沈黙のスキップを止めているのは `--no-renames` の
方**で、rename 検出は移動したファイルを移動先だけで出力するため、モジュールを `docs/` へ移すと出力は
ドキュメントのパス 1 本になり、差分全体が「ドキュメントのみ」と読める。**`-z` は逆向きに倒れる** —
git は literal に出力できないパス (非 ASCII バイトに加えて `"`・バックスラッシュ・制御文字) を C
クォートで出力し、クォートされたパスは `docs/*` にも `*.md` にも一致しないので、コード扱いになって
ハーネスが走る。規則が「パスの表現」ではなく実際のパスを見るように残してある。実測: `core.quotePath=false`
はクォートの非 ASCII の分しか塞がず、`-z` は `--no-renames` を含意しない。`-z` は一時ファイルを要する
— NUL はシェル変数に入らない。

**コメントのみの差分も除外していない。** 除外案は落とす前に測ってある: `main` の直近 80
first-parent マージのうち (2026-08-14 測定) 42 件が `src/**` を触っており、変更ファイルを esbuild で
コメント・空白を落として正規化すると、前後が一致したのは 42 件中 3 件だった。3 回の実行と引き換えに、
「スキップする方向で正しくなければならない」正規化器を抱えることになる。

**この起動条件が発火する頻度。** 同じ 80 マージのうち該当は **75 件** — 走らない 5 件がドキュメント
のみのマージ。これに置き換えられた 2 つの監視リストはそれぞれ 52 件と 61 件で、61 と 75 の差 14 件の
うち 4 件が上表の 3 度目の漏れに当たる。

必須ワークフローは並列に走るのでマージはその最遅を待ち、その 75 件ではこれが最遅になる。実時間は上の
マシン時間の 3 分の 1 ではなく**最遅シャード**の値なので、ティアをどこで切るかがそのほとんどを決める。

**シャードの大きさの決め方。** Playwright の分割は、収集順の**連続した区間**をケース「数」で切る
(`filterForShard` がグループをパス順に走査し、累積本数で切る)。ここでは本数が等しくても時間は等しく
ならない — `t2-shape-change` から `t2f-shape-change` までの 6 ファイルは順序上隣接していて、しかも
高価な部類なので、1 つの区間がまとめて引き受ける (派生元の走行では shard 1 が `t2` / `t2b` / `t2c` /
`t2d` / `t2e` / `t2f` を持っていた)。その走行での実測は、57 / 57 / 56 ケースに
対してマシン時間 12.3 / 4.5 / 7.4 分 — 仕事量の 3 分の 1 である 8.1 分に対し、最遅シャードが 53% 上。
`PWTEST_SHARD_WEIGHTS` は同じ連続区間を所要時間で切り直すもので、同じ走行が 8.3 / 8.5 / 7.5 分になる。
切れ目は、シャードが実際に時間を使う形のモデル — 2 ワーカーが宣言順に次のケースを取る、加えて
ワーカー起動の約 9 秒 — の下で最遅シャードの予測実時間を最小化して選ぶ。このモデルは同じ走行の
3 シャードを 381 / 144 / 261 秒と予測し、実測は 380 / 143 / 261 秒だった。

配列は `e2e/race/skip-ledger.json` の `collect.shardWeights` にあり、書かれている場所はそこだけ:
`race.yml` がそれを読み、`pnpm check:skips` がその重みの下で収集をやり直す。**そこで捕まるのは
コーパスの「大きさ」が変わった場合** (合計が合わなくなる。配列が古くなる通常の経路がこれ) **と、
Playwright が変数を読まなくなった場合** (`PWTEST_` 接頭辞の、同梱の型定義に無い変数)。1 件追加と
1 件削除の相殺、収集順を変えるファイル改名、単にケースが遅くなった場合は、どの本数も動かないので
ここでは何も報告されず、導出し直すしかない。その導出が
`node scripts/race-shard-weights.mjs <run id>` で、このコーパスを記述していないログ (中断・キャンセル
された走行のログは、走っていないスイートに対して算術的に妥当な配列を出していた) と、切れ目を
ランナーが再現しない案を、どちらも出力せずに落とす。

**現在の配列。** `[36, 59, 70]` は 165 ケースのコーパスに対するレース走行 (2026-08-23) からの
導出である。その走行は 158 件を計時し、宣言済みスキップ 7 件は 0 として扱われた。その走行の所要時間に
2 ワーカーのモデルを当てると 3 シャードは 277 / 269 / 276 秒、連続性の制約が一切無い分割なら 266 秒で、
ランナーは案をケース単位でそのまま再現した。再導出の意味は残るこの差ではなく、切れ目が編集の残りでは
なく所要時間の読みに戻ることのほうにある (所要時間が動いた配列は、何も報告されない)。

2 つの走行を継ぎ合わせたログは、それだけでは届かない。重なる部分があれば「retry 無しで 2 回計時された
ケース」が露見させるが、重ならない半分どうしはコーパスをちょうど 1 回覆い、それでいて切り分けの元に
なる所要時間は 2 台のマシン・2 つの時刻から来る。読みは 2 つあり、届く範囲が違う。リストレポータは
分割走行の冒頭に `Running 38 tests using 2 workers, shard 1 of 3` を出すので、半分はそれぞれ自分の
ヘッダを持ち込む — ヘッダ集合がシャードごとちょうど 1 本でなければ落とす。これはいつ書かれたログにも
効き、既存のログも読める。次に `race.yml` がテストの手前で run id と attempt を、シャードのジョブ
ごとに 1 回 echo する。これが落とすのは、id が 2 つ・同じ id の attempt が 2 つ・そして **id 1 つが
シャード数より少ない行にしか無い**場合。最後のものは echo 以前に書かれたログへ新しい走行を継いだ形で、
他のどの読みも通ることを実測した。Playwright は走行を識別する出力を持たず、`gh run view --log` の
タイムスタンプでも答えられない (シャードのジョブはいくらでも待ち行列に留まりうるので、2 本の間隔で
1 走行と 2 走行を分ける値が置けない)。

**どれも届かないのは**、echo より古いログ**同士**を、ヘッダがシャードごとちょうど 1 本になるよう
組んだ継ぎ合わせ。これは閉じるのではなく、古いログが尽きるにつれて縮む。

**コーパスの大きさは意図的に問わない。** ログの本数がチェックアウトの収集数より 1 多いのはケースを
削除した直後の通常状態で、まさにそのとき配列を導出し直す必要があり、存在するログはすべて削除前になる。
ここで落とすと、その瞬間にツールを使えなくしていた拒否が戻る。判定は、どのケースも説明できないログを
扱う側 — 既に重み付けが決まっている場所 — で note として出す。

値はシャードのステップへ 4 つの名前を経由して届き、GitHub は存在しない名前を空文字に解決する。
Playwright はそれをエラーではなく「重み無し」として扱うので、シャードのステップは空値をその場で
拒否する — 重み付き分割の名前で本数等分を走らせないため。

**ランナー側**の実時間はここに残していない — CI の時計は走行ごとに動くので、書き留めた数値は「一度そう
だった」以上を意味しない。重みをランナーの実時間ではなくテスト 1 件ごとの所要時間から導くのも同じ理由。
既知のマシンで分離実行して測ったローカルの実時間は上の専用表にあり、その表を単一の家と宣言する行が
すぐ下に付いている。

`race.yml` は**トリガーにフィルタを一切持たない**ので、`detect` はすべての PR で走る。これが
`race-required` をマージ条件として成立させている: トリガーのフィルタでスキップされたワークフローは
チェックランを 1 つも作らないため、ハーネスが走らない PR には、他の PR が待っているチェックが存在
しないことになる。そちら側ではハーネスの 2 ジョブが自分の条件でスキップし、数秒で成功を報告する。
この配置と「必須チェックはすべての PR で報告できなければならない」理由は CLAUDE.md の
"What a merge waits for" にある。

ブランチに対して手動で回す場合 ── 判定の前倒しと、再実行の経路:

```sh
gh workflow run race.yml --ref <branch>
```

スキップ側で失うのは読み取り: **`race-required` はハーネスが通ったときもスキップされたときも同じ緑**
になる (`if:` でスキップしたジョブは成功を報告し、必須コンテキストに third colour は無い)。どちらの
パスも触らずバージョンも動かない PR では、それはスキップされた **2** ジョブの上のゲート (`detect` は
走り、`race` と `race-webkit` が自分の条件でスキップする — **ジョブ自身の `if:` が偽の matrix ジョブは
展開されない**ので、`race` は 3 回ではなく 1 回だけ報告する)。どちらが起きたかを言うのは `detect` の
ログで、両側とも理由を出力する。WebKit はブラウザのダウンロードが別なので専用ジョブにしてある。3 回
払うとケース自体より高くつく。

PR に push し直すと実行中の run は打ち切られる (`race.yml` の `concurrency`。グループキーの fallback を
ref ではなく `github.run_id` にしてある理由は同ファイルのヘッダにある)。
...
残る 3 つ ── 13 (著作者帰属)、一般形の 3 (スナップショット汚染)、そして「実機が報告したサンプルレートの
うちどの読み出しもまだ消費していないもの」── だけが probe を要する。
...
| 報告済みのサンプルレートのうち**まだ残っているもの** | `ratesOf(page)` |
| 確定済みのアンドゥ / リドゥ深さ | `depthOf(page)` |

レート履歴を probe が持つ理由は台帳と同じ。無言の insert FX 消去と操作者自身の消去を切り分けるのは
この報告 (`readback.ts` の `HoldContext.ratesSeen`) で、消費されたかどうかが効くのは目の前の消去では
なく**次の**消去なので、IPC ログにはジェスチャ 1 つ分遅れて現れるか、まったく現れない。
...
`graph.ts` の `alignLinkedPairs` はレイアウト側の同じ形 — 既にリンク済みで届いた STEREO ペアが受けるスナップ
— で、リンクを持ち込んだファネルがまだ名指しできる場所で走る: `Graph.setModel` の中 (読み込み・デバイス
再描画)、棚からの復帰の中、そして各デバイス追従 reconcile のその読み出し自身のサンプルより前。
...
- **レートで消された insert FX を保持して送り返す。** 実機が報告するのはレートだけなので、その通知が
  エスカレートさせた読み出しがセレクタとバイパスの消失を見つける。マージの hold が無いと後続の flush は
  **1 件も書かない** — プランが消去を採用してしまい、送るものが残らない。hold があると 3 段が順に出る:
  `135:0:0=1793`、`689:0:6=-990`、`134:0:0=1`。判定は画面ではなく書き込みに対して行う。192 kHz では
  全オプションがレートロックされインスペクタの行が select でなくなるため。エフェクトがプランに残っている
  ことは、レートを戻した後の画面から読む

**T2 — アドレス集合の形**

- **EQ 1-knob の自己盲目**。単発 notify の差分で分離: ON のフラッシュは 18 のバンドアドレスを書込集合
  からも**登録集合**からも同時に外すので、そのうち 1 本への notify はブリッジで拒否され費用は **0**
  (settle も idle 網も `onNotify` の中で張られ、その `onNotify` が走らない)。トグルが残すアドレス
  (1-knob level、48) への 1 発は **1 回**。アプリは再計算の**原因**を追うことで整合を保つ ── 48 は
  `sideEffect: "refetch"`。フラッシュが登録し直す前は同じ差分が 2 対 1 で、2 の側はブローカが配達し
  `live.lookup` が解決しないアドレスの昇格だった
- **4 発のバンドバーストは集中度しきい値を踏まない**。踏まないと言える読みは、それを置き換えた読み
  そのもの: 登録済みアドレスへ配達すると scoped read 1 回 + idle 網の掃引で済む。`follow.ts` が数える
  のは `node:name` 対で、カタログはバンドではなく**パラメータ**を名付けている (`EQ_BAND_GAIN` /
  `EQ_BAND_FREQ`) ので、2 バンドにまたがる 4 アドレスは 2 コントロールである。以前の全体掃引 2 回は
  解決不能アドレスの昇格であって集中度ではなかった ── ケース自身のコメントは「バーストはしきい値と
  交絡している」と書いていたが、交絡していなかった
- **1-knob OFF は 18 のバンドアドレスを書き、同じフラッシュで登録もする**。書込がそのフラッシュ内で
  購読より先に出ることは変わらない。閉じたのは、それがフラッシュを越えて残っていた分である
- **COMP/EQ バンク入替**は、入替自身のフラッシュで新バンクを登録し旧バンクを外す。放棄したバンクへの
  notify は拒否され (全体照合 0 回)、生きているバンクへは 1 回
...
- **MIDI 由来のストリップ再構築が捕捉中のポインタの下で要素を差し替え、画面とデバイスが食い違ったまま
  静置しても戻らなかった → 修正済み** (修正 11)。再構築は今も起きるが、ジェスチャは要素と一緒に終わる
  ようになり、文書の外から書き続けることはなくなった
- **BAL 鏡映**が、MIDI の 1 メッセージごとに元ノードのパラメータ全体を相方へ深いコピーし、直前の
  無関係な UI 編集を破壊する
- 相対ドラッグは pointerdown 時に凍結した基準値から絶対値を再計算するので、途中の MIDI 書込を
  上書きではなく**算術で消す**。その算術は今も残っているが到達しなくなった (修正 11) — 消す側の
  move そのものが起きない

**T5 — 失敗注入**

- **リンク切断後も書込が漏れ出ていた**。送信ループのどの位置で落としても漏れ、位置が後ろほど件数が
  減った → **修正済み** (修正 10)。どの段でも漏れは 0 になった。既にワイヤ上にある 1 コマンドだけは
  完了するので、境界は切断時刻ではなくテアダウン時刻に置く
...
- 購読・解除の呼び出し順が、コンソールのメーター登録が**自分でしていない呼び出しに押しのけられる**
  ことを示す
- **機種切替がフラッシュより長生きし**、残りのコマンドが旧機種のアドレスを狙い続けていた →
  **修正済み** (修正 10)
- **その切替はもうライブセッション中には起きない**。ピッカーがセッションの間ずっと無効化される
  (`syncDeviceActionUi`) ため、操作者に残る手順は「ライブを止めてから切り替える」。フラッシュは
  その両方をまたいで止めたままにでき、読みは 2 つを分離する — 停止側はセッション世代を動かすが
  履歴には触れず (undo 深さ 1)、切替側がそれをリセットする (0)。どちらからも漏れない:
  遅延 `vd_set` 0 件、孤児アドレス 0 件、MIDI キャッシュは張り直される (`0.0` → `+4.0`)。
  守られていない照合側が 3 フローでなく 2 フローの対比になったのも同じロックが理由で、そこでの
  ピッカーの保護はセッション由来 — 代役をしていた照合側の門は残り 2 つに対して今も無い
...
- **フラッシュ自身の書込が送信中の間に入った編集を refetch が取り消していた → 修正済み**。フラッシュは
  開始時に値を取り、refetch は書込が戻ってから開くので、その間の編集はどの書込にも載らず、読みの最中の
  編集でもなかった — 読みは書込が運んだ値で答え、プランがそれを取っていた。同じケースの arm D が書込を
  バリアで止めてレベルを一段上げる: 修正前 `2 → 3 (書込保持) → 4 に上げる → 3`・書込 `[3]`、修正後
  `→ 4`・書込 `[3, 4]`。ドラッグの腕が負荷下でたまに落ちていた (最後の位置 91 が書かれずプランが 86 で
  終わる) のもこれで、`--repeat-each=12 --workers=4` で main は 24 回中 1 回、修正後は 48 回中 0 回
...
- **SETUP > GENERAL への notify は何も買わない ── その notify はブリッジを越えない**。Rust 側の
  ...
  `BULK_CHANGE` に限って**フロントへ渡す。**どの登録にも入っていないアドレスはセッション中ずっと
  配達不能**であり、SETUP > GENERAL の 13 アドレスはその位置にある最大の一族である。**ただし
  「プランが発行しないアドレス」がそのままこれに当たるわけではもう無い**: 登録集合はかつて書込集合
  そのものだったが、以後アドレスが手で足された ── 文字列経路のアドレスと追従だけを行うケースで、
  実機が告知し、アプリは emit が書かないまま購読している。その列挙は 1 箇所にだけある:
  architecture.md「書込は ACK された時点ではまだ読めない」(名前の経路と、その後に続く追従だけを行うケースの表)。
  機器スコープ `"scene"` が落とす 65 アドレスは配達不能のままで、**シーン外の追従だけを行うケースも
  同じくそちら側に入る** (どれが該当するかも同節にある) ── 追従だけを行うケースの一覧にも同じ
  `sceneExternal` フィルタが掛かる。そのスコープの全体読みは読了後にプランのシーン外の値を書き戻すので、
  それらを追従すると食い違うためである。従来の「約 1350 読み」は、出荷版が受け取り得ない刺激に疑似デバイスが
  ...
  置き換えた価格より悪い: idle の掃引を張るのは `onNotify` の中で (もう 1 つの入口は書込 settle 自身の
  sink で、実機が告知しなかった書込を要する)、
  配達される notify を 1 発も受け取らないセッションには idle の安全網も無い。発見する予定が
  何も立たないまま変更が見えなくなる
- **決着 ── 「登録は converge / refetch フラッシュに遅れる。その窓こそが到達可能な『未知アドレスの
  昇格』」**。当時は遅れていた: `live.ts` の `capture()` はスナップショットとアドレス索引を同時に
  組み `begin()` / `resync()` / converge 後 / refetch 後に走るのに対し、`follow.ts` の `subscribe()`
  は `begin()` と照合完了後にしか走らなかったので、`sideEffect` 付き編集のフラッシュは索引だけを
  縮め、ブローカ側の登録は大きいまま ── アドレスは配達されるのに `live.lookup` が解決しない状態が
  生まれた。フラッシュが自分の末尾で登録し直すようになった (`followSetStale` → `DeviceFollow.refresh`)
  ので、この項が挙げていた 3 つの開き手 ── EQ 1-knob ON・COMP/EQ Type・Insert FX ── はもう窓を開けない:
  `capture()` は索引とアドレス列を 1 パスで組み、フラッシュは戻る前にその列を登録する。従来からの例外は
  名前アドレスで、これは登録されるが数値側の索引ではなく `nameIndex` に載る ── `onNotify` は `lookup` を
  呼ぶ前に `applyName` でそこへ届く
- **到達可能な「未知アドレスの昇格」は、同じ分岐のもう半分に移った** (`follow.ts` の
  `addr === undefined || addr.node === undefined`): 登録済み・索引済みで、所有者がノードではなく
  **機器全体**のアドレス。scoped read では直しようがない。アプリが発行するそれは `SAMPLE_RATE` (766) で
  ── `translate.ts` がこのブロックを `own(undefined)` で機器所有にしている ── 置き換えた 2 つの刺激と
  違い、後の変更で閉じる窓ではなく恒久的である。5 発の notify storm での実測: **1350 読み**に対し、
  ノードに置ける burst が **675**、拒否される burst が **0**。すなわち昇格自身の全体読み + idle 網に
  対する、idle 網のみ。アドレスを使わない代替は従来どおり `BULK_CHANGE` センチネルで、これは
  仕様上フィルタを素通りする
- **撤回 ── 「機器スコープ scene はデバイス側ノブ操作 1 回の代償を倍にする」**。scene スコープの
  セッションはそのアドレスを登録していないので notify は拒否される ── 代償は倍ではなくゼロである。
  正しい所見は、**この設定は落とした 65 アドレスへの追従を黙って盲目にする**こと。実機で動かした
  MONITOR_LEVEL や OSC_MODE は、そのセッション中ずっとアプリに伝わらない
- **撤回 ── 「839 / 193 / デバイス側リネームは追従が高くつく (各 2 回の全体照合)」**。3 つとも
  高くつくのではなく**追従不能**だった。名前アドレスは登録に入らず (文字列経路でスナップショットにも
  載らない)、idle の安全網も配達された notify でしか張られないので、実機で行ったリネームを拾うものが
  セッション中に無かった。**リネームの側は 2026-08-06 に解消** ── `live.ts` が `planToCommands` とは
  別に名前アドレスを登録し、`follow.ts` が名前 notify をプランへ直接置く (URX44V / URX44 で 17 本・
  URX22 で 15 本、数値側の約 800 に対して。名前 notify はリネームのときしか出ないので定常的な通信は
  増えない)。**839 / 193 の側も 2026-08-11 に同じ形で解消** ── 実機が両方とも告知することを測って
  から (URX44V・System V1.3.1.0 で front-panel の PRE/POST と Track Count がそれぞれ notify を push)、
  発行しないまま (193 は broker が PRE 書込を拒否するため、839 は broker が実用値をすべて上限で切る
  ため) **登録し、修復できる所有ノードへ索引付けた**。撤回時の主張が言う「各 2 回の全体照合」ではなく、
  1 ノードの読みで済む
...
**WebKit との比較 (`race-webkit`)**: ポインタ捕捉下でのストリップ作り直しは**エンジン非依存**である。
2026-09-24 に `a94d0b28` で `t4-midi.spec.ts` の「a CC burst 50 ms into a CH 1 fader drag rebuilds the
strip under the pointer」を各エンジン 2 回走らせて実測: どの走行でも、掴んでいる要素は `connected=false`
= 切り離され、画面は MIDI の値 `+5.0` を出し、切り離された側は `-22.4` を持ち、デバイスは `+5.0` (500) で
終わり、ドラッグが出す CH 1 フェーダーのフラッシュは 1 回。同じドラッグを各エンジンで計装すると (各 3 回)、
pointermove は 46 件が合体なしで届き、そのディスパッチの所要は合計 0〜0.5 ms。ドラッグの実時間は違う
(Chromium ≈1.57 秒・WebKit ≈1.30 秒) が、このケースの判定はそれに依らない。
...
### 4. インスペクタの再描画をパネルの足跡に限る (`main.ts` / `inspector.ts`)
...
始まる。そこで**パネルが読むノード**に限った (`inspector.ts` の `inspectorNodes()`: ノード選択=そのノード /
ワイヤ選択=**両端**。宛先 Bus の BUS Type・Pan Link が送りコントロールの有無を決めるため)。足跡は
描画側が持つので param カタログの増減で腐らない。併せて再描画がフォーカスとスクロールを継承する。

パネルが読むものは選択に属するものだけではなく、初版が落としていたのはその外側である。Ducker
ノードは 2 箇所から読まれ、どちらも選択自身の行ではない: プラン全体を走査し選択なしを含む全選択で
描かれる ducker バイパス警告カードと、PRE タップが吊り元 CH の Ducker より前に位置することを告げる
送りの注記 ── 吊り元はワイヤの source だが、ducker はそこに吊られた**独立ノード**で、注記が実際に
読むのはそちらである。この分岐に ducker ノードを載せる書き手は外部 MIDI の Ducker on/off だけで、
両方ともそこで陳腐化していた。デバイス側の変更は `follow: "direct"` を持つ ducker param が無いため
scoped / full 分岐を通る。
...
(`"rate" | "link" | "slot" | null`) で返す。インスペクタはそれを直接描画し、コンソールの候補列はその上に
定義する (`insertFxFree`)。後から増えたロック理由はインスペクタには編集なしで届く (理由が何であれロック済み
option を disabled にするため) が、理由を行に名指しするコンソールは分岐とメッセージを取る。
...
### 10. フラッシュをセッションと一緒に終わらせる (`live.ts`)

`flush()` は入口で `active` を読むだけで、その後は一度も読み直さなかった。`end()` は同期なのに送信
ループは同期ではないので、セッションが落ちた時点でループに残っていたコマンドは全部そのまま送られた
── リンク切断後にも、機種切替後にも。機種切替では `loadPlan` がセッションを止めてプランを差し替える
間、ループはコマンドとコマンドの間の await に座っており、その残りは旧機種のアドレスを運んでいた。
修正 9 の値の取り直しが読むのも同じ捕捉 (`model` と `plan` はフラッシュのローカル) なので、差し替えの
後はもう誰も保持していない文書から取り直していた。

`LiveSync` は `begin()` と `end()` が進める `sessionGen` を持つようになり、ループは await のたびに
── 2 つの送信、converge、refetch ── それを比べる。throw ではなく return する: テアダウンは失敗では
ないし、報告する相手ももう居ない。スナップショットに書き込む**前**に返すのが要点で、1 つの await の
中で終わって始まり直したセッションは既にデバイス読みからそのスナップショットを作り直しており、死んだ
フラッシュの値をそこへ書けば新しいセッションのデバイス真値を汚す。フラグではなく世代なのも同じ理由で、
停止と再開をまたぐとフラグはフラッシュが入ったときの値に戻っている。

テアダウンは切断を await せず繋ぐ (`vd_disconnect` は待てる値を返さない) 設計で、遅れて届いても安全なのは
epoch ガードのおかげ。`end()` から切断が Rust に届くまでの窓は開いていて、世代が閉じるのは「その窓へアプリ
自身が送り続けるかどうか」の方。その窓はもう台帳のフラッシュだけではない: `releaseLive` は切断の前に、まだ
往復している follow read を待つ — 単に終わっただけのセッションは read を最後まで走らせ、read が使うリンクは
そのセッションのものだから。その間は他の何も接続できない (リンク保持者を返すのはトグル時点ではなくこの解放の
最後) — read は epoch を持たないので、途中で別の操作が installed したワーカーの上をそのまま走ってしまう。

実測、`drop-link-loss-mid-flush-ladder` の 3 段すべて: **漏れた書込 15 / 10 / 5 → 0 / 0 / 0**。
どの段も 15 コマンドを仕込んであるので、この 0 はガードの結果であって残りが空だったからではない。
`teardown-model-switch-during-flush` は **ループの残り (URX44V にしかないアドレスを含む) → 0**。
セッション終了時に飛行中だった 1 コマンドは完了するので、両ケースとも境界をテアダウン時刻に置く。

### 11. ジェスチャをコントロールと一緒に終わらせる (`console.ts`)

CONSOLE のドラッグはすべて `window` で追跡し (コントロール側の捕捉だけではポインタが外れた瞬間に
報告が止まる)、開始時の `StripRef` を閉じ込める。再構築がその要素を差し替えても、ハンドラは画面に
無いコントロールからプランと実機へ書き続け、操作者が見ているストリップは再構築が描いた値を出していた。
以後 2 つは離れたまま: どちらの側も古いと思っていない値を、何も描き直さない。実測は**画面 `+5.0` に
対して実機 `-1.2`**、ラダーの全位相・両エンジンで。

引き金はデバイスリンクではない。MIDI の反映、デバイス追従のストリップ再構築、シーン再読込、そして
**環境設定の言語・テーマ切替**がどれも同じ `Console.render()` / `refreshStrip` に届く。だから修正は
どれか 1 つを塞ぐ形ではなく、このビューの 3 つのドラッグが唯一の登録口として使う `trackDrag` が、
move のたびにコントロールがまだ文書内かを訊き、消えた場所でジェスチャを終える形にした (規則は
ドラッグごとではなく 1 箇所)。繰り越しはせず、再構築側はジェスチャの存在を
知らなくてよい ── 繰り越し形はチューニング画面で Close 押下を飲んだ形であり、ここで繰り返せば同じ種類の
欠陥を買うことになる。

操作者が失うのはそのドラッグの残り。それが代償で、そちらの方が小さい。もう一方は、画面に無い
コントロールからプランと実機がジェスチャを受け取り続けることだから。

実測: `midi-vs-main-fader-absolute` の 50 / 300 / 900 ms ── 見えている読み値・ワイヤ上の最後の
コマンド・実機の 3 つが全位相で `+5.0` に一致し、切り離された要素は差し替え時の値
(`-24.0` / `-18.0` / `-7.2`) で凍る。ケースが**走らせるのは 50 ms の位相**である: 3 位相が一致し、
再構築はジェスチャの後半で別の事象になるわけでもないので、残る 2 つはティア最大の単価で同じ
機序を測り直していた。`midi-vs-send-fader-relative-baseline` は、凍結基準の算術は
コードに残ったまま到達しなくなり、メッセージを上書きする move が起きないので `-22 → +5.0` が
スケール末尾まで消されずに残る。引き金が純粋にローカルな `baseline-view-locale-churn` は
**再構築後の書込 → 無し**。

これに伴って測り方を 1 つ変えた。プラン編集はフラッシュ 1 窓ぶん遅れて書かれるので、切り離しの数
ミリ秒後に出るセットはジェスチャが既に済ませていた編集のもの。不変条件 10 のケースは窓を `detachAt`
ではなく `detachAt + 300 ms` に置く。それでも、以前は書込だらけだった数秒ぶんのポインタ移動は除外
できている。

### 12. reconcile を converge から遠ざける (`follow.ts` / `live.ts`)

device-follow の reconcile と converge のラウンドが、同じリンクを交錯しながら読む。reconcile を arm する
のは converge 自身の書込が実機に起こさせた notify なので、ラウンドのエコーが止まってから
`RECONCILE_DEBOUNCE_MS` 後 ── つまり次のラウンドの内側 ── で発火する。両者を隔てるものは無く、不変条件 4 が
その結果を報告する: `vd_get 139:0:0 issued inside an in-flight vd_get 31:0:0`。

顕在化したのは `t1-overtake` の converge 対照ケースで、silent アドレスの park が converge の手前に入り、その
読取ぶんラウンドがずれたときである。同じ遅延は他にも作れるもので、A/B がそれを言っている: park を外すと
緑、park は置いたまま読取対象を空にしても緑 ── ラウンドを動かすのは park の読取で、動いた先にあったものは
元からそこにあった。ケースの値の検証はすべて通ったままなので、不変条件 4 が捉えたのは損失ではなく構造である。

修正は `follow.ts` 側: flush が書いている間 (`live.isWriting()`。converge はその一部) reconcile を先送りし、
窓は消費せず保持するので、バーストが名指ししたノードは settle タイマー自身の再アームで読み直される。待つのは
reconcile の開始時だけである: reconcile の読みの最中に編集が予約する flush はその横で出ていき、これが
`t8-stress` が自らの前提として主張する 2 本のチェーンの競合である。このケースは打鍵列を始める前に読みを
リンクへ載せる: 打鍵列そのものが起こす reconcile は、打鍵列の最後の flush が終わるまで待つからである。
...
名前でも同じことが起きていた: 自分の名前変更が送信中のチャンネルに届いた名前の notify は、届いた時点でプランへ
置かれた。2026-09-24 に、名前変更をバリアで止めて同じチャンネルへ別の名前の notify を注入して実測: 盤面の
ラベルが注入した名前を表示した。

修正: `LiveSync.hasUnannouncedWrite` が、あるアドレスへの自分の数値書込が送信済みでまだ告知されていないか
(送信中、または ack 済みで未告知の書込の待ち行列に値が残っている) を答え、`hasUnannouncedName` が文字列の書込に
ついて同じことを答え、`DeviceFollow` が直接反映と名前の反映の手前でそれらを `isSuperseded` として問う。真と
答えた notify はプランへ入れずにそのノードを読み直し、その読み直しは、窓の中のどの notify も未告知の書込を
背後に持たなくなるまで保留する。それより前に読むと、settle が上書きされる側の値 — 書込のマーク以降にその
アドレスについて実機が最後に言った値 — を書込自身の告知とみなしてその値で答え、アドレスを読めば書込前の値が
返る。`t1b-overtake.spec.ts` の `overtake-foreign-notify-inside-our-write` (`quiet`・`busy`、および notify の
280 ms 後に書込を解放して読み直しの期限を書込の ACK と告知の間に置く `acked`) と、`t1d-name-window.spec.ts` の
名前のケース (`held`・`released`) が、盤面を毎フレーム記録して検証し、書込・スナップショット・実機も検証する。
実機の書込前の値の読み (`staleReadsAt`) を入れた状態である。2026-09-24 に実測: 判定を無効にすると `quiet` と
`busy` が落ち、読み直しを保留しないと `acked` が idle の全体掃引まで上書きされる側の値を表示し、名前を届いた
時点で置くと名前の両変種が上書きされる側の名前を表示する。

### 14. 送信前の編集を追従の読みが取り消す (`follow.ts` / `live.ts`)

編集がまだ flush の 120 ms 窓の中にある間、またはその書込が送信中の間に出た追従の reconcile は、編集された
アドレスを編集が置き換える側の値で読んだ — 実機はまだ受け取っていないか、まだ書込前の値を答えていた — そして
マージはその値を取り込んだ。プランは読みを発行した時点の値を保っていたからである。reconcile はその後、読んだ
値でスナップショットを取り直すので、プランとスナップショットが一致し、編集のための書込は出なかった。
2026-09-24 に実測: CH 1 の HPF 周波数を実機側で変えた 250 ms 後に動かした CH 1 の PAN は、画面上で元の値へ戻り、
一度も書き込まれなかった。`a94d0b28` でも、§13 の保留を入れた状態でも同じで、上書きされるフェーダーの値の
後ろで同じ PAN 操作をしても同じだった。

修正: `DeviceFollow` は `live.isWriting()` — flush が予約済み・実行中・順番待ち — の間 reconcile を先送りし、
窓は保持する。読みはどの編集も送信・ACK された後に始まり、その settle が告知を待つ (待ちが止めないものは
§12 にある)。`overtake-edit-in-the-flush-window-vs-follow-read` と
`overtake-foreign-notify-with-an-edit-on-the-strip` (`held`・`released`・`external`・`late`) が PAN のノブを
毎フレーム記録して検証し、PAN の書込が 1 回であること、スナップショット、実機を検証し、`external` と通常の
ケースでは実機自身の HPF の変更がスナップショットに入り、その上から何も書かれないことも検証する。2026-09-24 に
実測: この先送りを外すと、通常のケースと `late` が落ちる。

アプリ側のドラッグ中に実機でシーンを呼び出した場合、その読みはドラッグの 2 つの flush の隙間 — ある flush の
ACK から次の移動までの数ミリ秒 — か、リリース後に始まる。`tzb-tail` の BULK_CHANGE のケースがそれぞれで
成り立つべきことを判定する。前者ではフェーダーが文書から外れてジェスチャがそこで終わり、後者ではドラッグが
最後まで実機上の呼び出した値の上から書き込み、リリース後の読みがその時点で実機が持つ値を取る。どちらでも画面は
実機の値で終わり、ケースはこれを判定する。呼び出しは告知の瞬間に fake の状態へ書き込むので、後の書込は実機と
同じくその上に載る。
...
- **文字列経路はその両方から除外されており、除外の理由だけが先に失効していた**。疑似デバイスは名前に
  staleness の窓を掛けず、告知も出さなかった。後者には理由が書いてあった — 「実リンクでは名前の notify は
  どの登録集合にも入らないので、アプリはどのみち受け取らない」。書かれた時点では真であり、そして窓を塞いだ
  のと同じコミット列の中で偽になった。名前のアドレスを登録したことが、実機側のリネームを追従可能にした変更
  そのものだからである。矛盾は誰にも検出されなかった。その主張が書かれていたのは疑似デバイス自身のコメント
  だけで、それが記述している当のコードと突き合わされていなかった。そして隠れていたのはハーネスではなく
  **アプリの欠陥**だった。実機は受理した名前書込を必ず告知するので、アプリ上で操作者が行ったリネームは
  そのままエコーとして戻ってくる — 追従層はそのエコーに、実機由来のリネームと同じ後処理を与えていた =
  アイドルの全体照合をアームしていた。その照合の反映は `planHistory.reset()` を呼ぶので、**ライブ同期中の
  リネーム 1 回が 900 ms 後に約 800 回の読みを起こし、自分自身のアンドゥ項目を消していた**。名前を登録した
  時点から実機では到達可能で、疑似デバイスが黙っている限りハーネスからは到達不能だった。告知を入れて初めて
  `t2b` のアンドゥ深さの判定がこれを捉え、pin は `follow.test.ts` にある。教訓は「全部モデル化せよ」より
  狭い: **モデル化の除外は機器についての主張であり、アプリの登録集合が変わるたびに読み直す必要がある** —
  除外そのものは、行われた時点では誤りではなかった
- **フィルタされた刺激は「静止」の第 2 の形**。ブリッジが拒否した notify は、アプリを
  ...
  (`pushNotifyDelivered`) を置くこと。未知アドレスの昇格が欲しいケースは、登録済みでノードを持たない
  アドレス (`SAMPLE_RATE`、766) を押すか `BULK_CHANGE` センチネルを使う ── `sideEffect` 付き編集の
  フラッシュが開けていた落ちた窓は閉じている。フィルタ
...
- **`vd_*` という名前のコマンドが自動的にワーカー上のコマンドであるとは限らない**。`onWorker` は
  「接頭辞が vd の面を指す」という理由でプレフィックス全体を疑似デバイスの単一直列キューへ載せていた。
  リンク台帳の読みがこれを破る: `vd_link_stats` は state mutex を取って atomics を読むだけで `Cmd` を
  一切送らない — 約 800 コマンドの掃引中に取った読みが掃引開始時点ではなく現在を返さねばならない、
  というのがこの設計の要点だから。疑似デバイス側でキューに載ると、実機アプリが何も置かない FIFO 位置を
  セッション開始と終了のたびに占有し、さらに専用の `case` が無いため未対応コマンドの throw を通っていた
  — 台帳側は読みの失敗を「リンクが消えた」として飲むので、これは見えない。決めるのは Rust 側コマンドが
  ワーカーへ `Cmd` を送るかどうかであって、接頭辞は破れるまで成り立っていた経験則にすぎない。発見は
  2 回のリリース走行の flaky 件数の比較 (2 → 4) からで、**失敗からではない**: その flaky との因果は
  確立していないし、修正前のツリーで当該 2 ケースをローカルで 5 回走らせても全て通った。4 件は後から
  実ログで特定してあり、比較の数字自体も 1 件ずれていた — 走行は件数をジョブ単位で報告し、1.6.0 は
  2 ジョブが 1 件ずつ報告していた。うち 3 件は自分の機序を持っていて修正済み (`t7-meter` の梯子 2 段は
  `b96868c`、`baseline-view-locale-churn` は `83eb598`)。残る 1 件は欠陥が再現することを要求する pin で、
  上記の修正 11 が `3ba19c4` で削除した結果、`vd_link_stats` との因果は確立しないまま反証もできなくなった。
  修正が入って以降にハーネスが走った 16 走行はいずれも **flaky 0**
- **前提条件はジェスチャではないが、ページ全体に掛けた fail-fast の上限はそこまで届く**。`t0b-sweeps`
  は到達できないコントロールがスイープの error 欄に名前付きで並ぶよう `boot()` で 4 秒の
  `page.setDefaultTimeout` を張るが、Playwright のページ既定はそれ以降の全アクション — このファイルで
  唯一スイープでないケースが呼ぶ `goLive` のクリックを含む — に適用される。結果、そのケースはこの
  ティアの最初の 3 回の WebKit 走行で `page.click("#btn-device")` がちょうど 4000 ms でタイムアウトした
  (1.6.0・1.6.1・その次の走行。1 回目は 9.0〜9.4 秒、リトライは常に緑)。クリックが待つのは actionability
  判定で、これは同じ矩形が 2 フレーム連続することを求める `requestAnimationFrame` ループである。
  2026-09-24 に CI の WebKit ランナーで、そのクリックにだけ 4 秒の上限を戻した計装版のハーネスで
  実測: 起動したばかりのワーカーの最初のテストとして
  クリックは 3767 ms かかり、最初のアニメーションフレームは待ち始めから 2531 ms 後で、ボタンの矩形はどの
  フレームでも同じだった。同じ走行で、既に 1 ケースを走らせたワーカーでは、静的ページのボタンは 130 ms、
  アプリの Device ボタンは起動直後で 570 ms (フレーム間に 494 ms の空白)、起動 2 秒後で 69 ms でクリック
  できた。待っているのは動く矩形ではなくフレームで、それはアプリの起動直後に集中している。`goLive` は
  読み戻し用に `waitForSelector` が既に持っていたのと同じく、全ステップで自前の上限を持つようにした。
  スイープ側の上限はそのまま — それが本来書かれた対象だからである。ケースはセッション確立に要した時間を
  印字する (無負荷の Mac で 437 ms)。余裕が数値としてログに残り、次に調べ直さずに済む
- **梯子の段を駆動コスト 0 の前提で置いていた**。`meter-rescope-inside-subpending-ladder` は
  2 回目の再スコープを 200 ms の登録に対し 10 / 50 / 260 ms の位置へ置き、低い 2 段は登録の
  **内側**に落ちることを意図していた。sleep からマークまでにはクリック 2 回とロケータ解決が挟まり、
  CI ではそれが同一 job の 3 段で 147 / 150 / 153 ms かかっていた — 結果 D=50 は 200 ms の窓の
  ちょうど 200 ms 地点に達し、dead zone ガードに掛かって落ち、D=10 は 157 ms で余裕 14 ms しか
  無かった。3 回のリリース走行で flaky として読まれていたが、そうではない: 200 ms の窓に対して
  内側の段は当該ランナーでは**配置不能**であり、ガードは仕事をしていた。窓を 1000 ms、段を
  10 / 300 / 1100 に置き直し、これまで観測した最悪の駆動コスト (retry で 470 ms。
  `trace: "on-first-retry"` が効くため) でも縁から 230 ms 離れる。教訓は 2 つ。コストは一方向なので
  失敗の仕方は非対称であり、「縁の後ろ」の段は遅いランナーに奪われず、「縁の手前」の段は必ず奪われる。
  そして空虚に通す代わりに落ちる dead zone ガードは flaky として報告される — まさにそう誤読された。
  それを否定するのは印字された達成位相で、最初からログに載っていた
- **何も壊さないアプリ側の変更でケースが無効化されることがあり、それを隠すのがティア分けである。**
  `picker.disabled = liveSessionUp` が機種ピッカーをライブセッションの間ずっと固めた — 意図された
  ガードレールで、それ自体は正しい。セッション中に機種を切り替える 2 ケース
  (`teardown-flow-refusals` の守られていない側・`teardown-model-switch-during-flush`) は、その後
  永遠に解決しない `selectOption` を待ち続けていた。誰も報告しない: 当時ハーネスはバージョン更新 PR
  でしか走らなかったので、以降どの PR でも `race` ジョブは**スキップされることで**成功を報告し、破綻が
  表に出たのは on-demand ディスパッチのときだけだった。起動条件を後に広げたのはこの事故が理由で、
  現在は `src/**` を触る PR でも走る (その変更が置かれていた場所。「どのティアがどこで走るか」)。「遅れる」を「見えない」に変えた性質は 2 つ —
  スキップされたジョブは成功を報告すること、そして**前提そのものが消えた**ケースは判定ではなく
  **タイムアウト**で落ちるため、失敗文がロケータを名指しして理由を名指ししないこと
  (`locator resolved to <select disabled id="model-picker">` が診断の全部で、基盤側のハングにしか
  読めない)。ケースのジェスチャが解決しなくなったら、まずアプリ自身のガードレールを読むこと。
  そして変更以降の緑は、ケースがまだ成立している証拠ではなく、走っていない証拠である
- **ページ側へ渡るコールバックは別世界であり、それを教えてくれる仕組みはツールチェインのどこにも
  無い。** 疑似デバイスの launch flag への応答を、インラインの `switch` 節から
  `FAKE_LAUNCH_FLAGS_OFF` の参照へ移した — しかしそれを読むコードは
  `page.context().addInitScript` に渡す関数の内側にあり、この関数は文字列化されて**ページ側**で
  評価される。駆動側のモジュール束縛はそこに存在しない。それでもコンパイルは通り、
  `pnpm typecheck:e2e` も通り、`--list` は全ケースを収集する。破綻は実行時に、疑似デバイスが処理する
  **最初の**コマンドで現れる。そして表に出る形はエラーメッセージではなく不在である:
  `goLive` が `#btn-live[aria-pressed="true"]` で 30 秒タイムアウトし、ライブセッションが上がらない
  — **どのエンジンでも** (2026-08-13 実測: `race-webkit` 5 件中 4 件、Chromium の `race` でも同一
  ケースが同じロケータで同じ 30 秒タイムアウト)。失敗文はロケータしか名指ししないので、前の項と
  同じくそれだけでは基盤側のハングと見分けが付かない。見分けたのは失敗した実行自身のトレースに残る
  page console 行 (CI run 31635304181 の `trace.zip`)。**grep する文字列を間違えないこと**:
  その行は JavaScriptCore では `Can't find variable: FAKE_LAUNCH_FLAGS_OFF`、V8 では
  ...
  149 は spec 側、1 は `ui.ts`)。ドライバ側の値がこうしたコールバックへ届く 2 経路 — 引数タプルと
  クロージャ参照 — のうち、型検査が及ぶのは引数タプルのほう: コールバック側の注釈と `as` キャストが
  同一のタプル型なので、要素数の不一致はコンパイルエラーになる。クロージャ参照にはその保護が無い。これを書いた PR とともに `main` に
...
- 実際に使える観測点は DOM・ステータス行・`localStorage`・疑似デバイスが見た IPC の 4 つ。
  このうちメニュー状態の送信が、機械可読な唯一の undo 深さ信号である — ただしこれは**確定前の
  開いた項目も深さとして報告する**ので、問いが違う (2026-08-13 実測: これを観測点にしたガードは
  reset を消しても通った)。確定済みの深さは `VITE_TRACE` ビルドが答える — `trace-probe.ts` は
  `depth()` を既に公開している
...
- **プランの「編集」は IPC トレースに現れない**。現れるのはその書込だけで、フラッシュ窓 (最大 120 ms)
  ...
  (`tzb` の BULK_CHANGE で、リコールの読みがドラッグ中に始まったとき。単独実行では `-7.0` = シーン、
  `--workers=4` では `-14.0` = ドラッグの通過値)。正しく決めるにはバリアで編集を置くしかないが、
  そのケースの変数は既に「リコールがジェスチャのどこに落ちるか」なので、置き換えになってしまう。
  そのためポインタ下のキーが**どの値で**終わるかはログに出すだけで判定しないが、画面が実機の持つ値で
  終わることは毎回判定する。そのためには呼び出しを fake の状態へ書き込む (`setMemAt`) 必要があり、読みを
  固定する (`divergeAt`) 形では、実機は後からドラッグが何を書いても `-7.0` を答え、画面と fake の保持値が
  fake の作った理由で毎回食い違っていた。flush が予約済み・実行中の間は reconcile が始まらないので、読みが
  ドラッグ中に始まるのは、flush の ACK から次の移動までの隙間だけである
- 文字列パス (`vd_set_str` = CH 名・Sweet Spot Data) はアドレス集合系の不変条件すべての外にある ──
  どちらもスナップショットにエントリを持たないので、節 B は構造上これについて沈黙する。
  **実機で確定 (2026-07-31)**: チャンネルのリネームがブロードキャストするのは
  `/vd/parameters/18:0:0` 1 本だけで、他には何も出ない (`BULK_CHANGE` もポインタ param も無し)。
  **本体 LCD で操作してもクライアントが書いても同一**。追従層の生涯を通じて、この放送はどこにも
  届いていなかった ── 名前アドレスが登録集合に入っていなかったからである。**2026-08-06 に名前を
  登録するようになった**ので、リネームは直接追従され、文字列パスは半分内・半分外になった: 名前は
  登録にあってスナップショットに無い = `FOLLOW_USB` (848) と同じ無害な向きで、節 B が判定しない
  逆差分にあたる。Sweet Spot Data はどちらにも無く、完全に外のままである。実機側の事実は
  私設 param 台帳に記録した
