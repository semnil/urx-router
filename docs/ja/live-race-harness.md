...
| p | 同じ窓の**名前経路**版。`staleAfterWrite` は文字列アドレスにも効き、`vd_set_str` の後 n 回の読みが置き換えた前の名前を返す。URX44V で実測 81 ms = 文字列経路は例外ではなく、疑似デバイスはその生涯ずっと例外として扱っていた。**告知は意図的に出さない** — 実リンクでは名前の notify はどの登録集合にも入らず (`planToCommands` の発行対象外なので `Subs::absorb` が落とす)、窓を消費できるのはケースが台本を書いた読みだけである。`t1d-name-window` がこれを必要とした最初のケース |
