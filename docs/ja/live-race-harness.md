...
| `race` — ハーネス | 24.6 分 | 60 秒 | `race.yml`。3 分割。ドキュメントのみの PR を除く全 PR と、バージョン更新 PR |
...
必須ワークフローは並列に走るのでマージはその最遅を待ち、その 75 件ではこれが最遅になる。Playwright は
ケース数で分割し重いティアが固まるので、実時間は上のマシン時間の 3 分の 1 ではなく最遅シャードの値。
**ランナー側**の実時間はここに残していない — CI の時計は走行ごとに動くので、書き留めた数値は「一度そう
だった」以上を意味しない。既知のマシンで分離実行して測ったローカルの実時間は上の専用表にあり、その表を
単一の家と宣言する行がすぐ下に付いている。
...
- **ケースから `src/core/control/` と `src/core/plan.ts` は import できない。** これらは
  ...
  その旨を書いてある
- **CI の 3 シャードはテストの「本数」で等分されており、ティアごとのコストは等しくない**。Playwright は
  順序付きリストを本数の等しい連続した塊に切る (ファイル境界も跨ぐ) ため、分割はファイル名に
  従い、ファイル名はティア順に従う。**2026-08-11 に `d85af81` で実測**: Playwright が各シャードへ
  割り当てたのは **57 / 56 / 56 ケース** (ここで言う本数等分がこれ) で、テスト時間は shard 1 が
  **752 秒**、shard 2 が 272 秒、shard 3 が 451 秒。うち実行されたのは 56 / 51 / 54 件で、残りは
  ケース自身のガードによる skip なのでコストを持たない — 上の秒数は実行された分に対する値。
  スケジューリングの偶然ではない — T1 (overtake) と T2 (shape change) は約 800 コマンドの全デバイス
  読み戻しを誘発し、
  その最中にバリアを保持するティアで、1 ケース 33〜48 秒。順序上隣接しているのでまとめて同じ塊に落ちる。
  ワークフローは最遅シャードを待つので、この偏りは時間で均した場合に対して**約 1.5 倍**を払う
  (752 秒 対、1475 秒の 3 分の 1 である 492 秒)。これが置き換えた読みも同じ形 (749 / 253 / 315 秒) だったので、
  偏りは特定の走行の性質ではなく順序の性質。
  そのままにしてある — ハーネスはリリースにつき 1 回しか走らない — 記録する理由は、真っ先に思いつく
  「シャード数を増やす」が最大値を下げるだけで偏り自体には効かず、ファイルを手でシャードへ割り当てる案は
  新しいケース 1 つで黙って崩れる分割にしてしまうため
