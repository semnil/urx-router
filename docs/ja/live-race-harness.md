...
| `shape-string-path-writes` | 混合 | 差分機構を完全に迂回する書込。無害な例と重大な例を並置。デバイス側リネームは配達されるようになり、Sweet Spot Data は今も配達されない |
...
- **SETUP > GENERAL への notify は何も買わない ── その notify はブリッジを越えない**。Rust 側の
  ...
  そのものだったが、以後 3 系統が手で足された ── 文字列経路のアドレス (`NAME` / `SWEET_SPOT`) と、
  読取専用の追従 (839 microSD Track Count、193/197/320/324 CH → FX send タップ) で、後者は実機が
  告知しアプリは一度も書かないまま購読している。機器スコープ `"scene"` が落とす 64 アドレスは
  配達不能のままで、**Track Count も同じくそちら側に入る** ── 読取専用の追従にも同じ `sceneExternal`
  フィルタが掛かる。そのスコープの全体読みは読了後にプランのシーン外の値を書き戻すので、839 を
  追従すると食い違うためである。従来の「約 1350 読み」は、出荷版が受け取り得ない刺激に疑似デバイスが
