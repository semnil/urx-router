...
**現在の配列と、ツールが取らなかった切れ目。** `[38, 74, 51]` は 163 ケースのコーパスに対する
レース走行 (2026-08-17) からの導出である。導出自身の最適は `[38, 73, 52]` で、ツールはそれを
出力せずに落とした: その切れ目が `t5-drop` の `test.describe.serial` ブロックの**内側**に落ち、
Playwright はブロックごと動かすので、ランナーの割当は案の 73 / 52 に対して 74 / 51 になる — 上の段落が
述べている拒否がそのまま働いた形である。配列が持つのは到達可能な側の切れ目。その走行の所要時間に
2 ワーカーのモデルを当てると、`38:66:59` (削除の後を減算で直した値) が 253 / 248 / 262 秒、
`38:74:51` が 253 / 256 / 253 秒、連続性の制約が一切無い分割なら 248 秒。最悪シャードは 6 秒縮む —
再導出の意味はこの数字ではなく、切れ目が編集の残りではなく所要時間の読みに戻ることのほうにある
(所要時間が動いた配列は、何も報告されない)。
