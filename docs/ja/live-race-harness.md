...
- **SETUP > GENERAL への notify は何も買わない ── その notify はブリッジを越えない**。Rust 側の
  ...
  **プランが一度も発行しないアドレスはセッション中ずっと配達不能**であり、SETUP > GENERAL の 13
  ...
  アプリが読むだけのアドレス (839 microSD Track Count、193 CH → FX センドタップ)、そして機器スコープ `"scene"` が落とす
...
- **撤回 ── 「839 / 193 / デバイス側リネームは追従が高くつく (各 2 回の全体照合)」**。3 つとも
  ...
  増えない)。839 と 193 はそのまま ── どちらも発行しない (193 は broker が PRE 書込を拒否するため、
  839 は broker が実用値をすべて上限で切るため) 以上、書込集合にも登録にも入らない
