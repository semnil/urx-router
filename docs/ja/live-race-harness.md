...
| 3 | スナップショット汚染 | 採取時点で、送信済みでない値がスナップショットに入った |
...
ケースごとの手順・疑似デバイス設定・判定は `e2e/race/cases/*.ts` が唯一の真実として持つ。
この表は何を測るケースなのかを述べる。
...
| `baseline-view-locale-churn` | 混合 | 言語・テーマ・ビュー切替がジェスチャ中に全再構築を起こす、デバイス非依存の経路 |
...
| `teardown-flow-refusals` | 混合 | 守られた読みと守られていない読みを、同一の 6 フローで対比 |
...
| `meter-tap-change-resubscribe` | console | 1 ストリップの変更で全ストリップが約 1 秒黙る。機種別表の解決も |
...
| `stress-latency-jitter-storm` | console | 応答順序が入れ替わりうる唯一の入力。固定遅延との対比で「遅い」と「順序が狂う」を分ける |
...
| b | 実状態マップ。読み戻しは書かれた値を返す。加えて「書かれた値と無関係に X を返す」乖離フック |
...
```sh
npx playwright test --project=chromium     # 従来のスイート (race は testIgnore で除外)
npx playwright test --project=race         # ハーネス
npx playwright test --project=race --shard=1/3   # 分割
npx playwright test --project=race-webkit  # WebKit で走る 3 ケース (@webkit タグ)
```

`race` プロジェクトはテストタイムアウトを **180 秒**に上げている。1 ケースが converge ラウンド
(約 800 アドレスの逐次読み) をバリアで跨ぐことがあり、既定の 30 秒では通るケースが欠陥に見える
タイムアウトへ化ける。
...
- **scene 書込スコープ**は追従種別の異なる 2 param で測って、いずれも 2 回対 1 回
- **Signal Type = STEREO** は両インデックスへ書き、**相方ノードを丸ごと再著作する**。undo 1 回で
  全部戻る。ただし converge ラウンドは**修復ではなく再送**で、実機が相方をリセットしても
  アプリの値を押し戻し続ける
- **文字列経路は差分機構を完全に迂回する**。改名はスナップショットにも登録にも入らず、実機側の改名は
  分類できずに全体照合 2 回を起こし、その読み戻しが**operator が今打った名前を消す**。
  SWEET_SPOT_DATA は同じ経路に乗るため sideEffect フラグを持てず、**書込後に読みが 1 件も走らない**
  (対照の数値 converge param は 1578 アドレスを読む)
