...
`race.yml` がそれを読み、`pnpm check:skips` がその重みの下で収集をやり直すので、台帳が主張する本数を
シャードが取らなくなったときは遅い走行ではなく赤いチェックになる。この検査は、Playwright の更新が
変数を読まなくなった場合も報告する — `PWTEST_` 接頭辞の、同梱の型定義に無い変数だから。
`node scripts/race-shard-weights.mjs <run id>` が race.yml の走行から配列を導出し直し、切れ目を
ランナーが再現しない案は出力せずに落ちる。
