...
- FX 1 / FX 2 チャンネル → STEREO / MIX 1 / MIX 2 (`send`。**チャンネル → STEREO** は FX の主経路で
  同様に**固定** = 常時結線・削除不可・**PRE/POST なし** (LEVEL/BAL のみ)。初期レベルは **-∞ (オフ)** でシードし、
  上げるまで主ミックスに加算されない。MIX 1/2 への Send は LEVEL/BAL/**PRE/POST** を持ち、**工場出荷状態では
  ON だがレベル -∞** (主経路と同じ扱い) でシードする。任意にオフ可)
  - 各 FX チャンネルは独自の**チャンネル ON/OFF** (ミュート) を持つ。入力チャンネルや STEREO マスターの ON と
    同じ扱いで、**工場出荷状態は FX 1 / FX 2 とも ON**。`新規` 計画にも ON でシードする (オフにすると盤面で
    ノードが減光し MUTE タグが付く)。
...
- **CH n → STEREO と FX 1/2 チャンネル → STEREO は固定の Send** (主フェーダー / チャンネルの経路)。
