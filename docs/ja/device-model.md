...
- モノ CH とステレオ CH の構成は固定 (機種で本数のみ変化)。MONO IN ペア (CH1/2, CH3/4) は
  ...
  **リンク済みペアはモードに関わらず 1 組の値を持つ**: 片 ch への編集はもう一方へ自動ミラーされる
  (下記の 3 つを除くノードパラメーター + 各 Send の LEVEL/PRE-POST/ON)。実機がそうするためである。PAN モードでの実測:
  ...
  **モードが決めるのは pan である**。BAL では pan はペア共有のバランス 1 値なので両 ch で一致し
  ...
  フラグは primary のみ保持。**A.Gain・Clip Safe・位相反転はどちらのモードでも各メンバー自身の値**で
  ある: リンク中に片メンバーへ書いても、BAL でも PAN でも相方の同じ値は動かなかった (2026-09-11 実測)。
  アプリのミラーもこの 3 つは相方の値を残す。ヘッドアンプの残り 2 つ (+48V・Hi-Z) は他と同様にミラー
  される。メーターも Signal Type に応答する — 実機のチャンネル調整画面は
