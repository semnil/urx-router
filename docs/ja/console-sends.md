...
- **その枠は残り、隣の面はどのレートでも同じ幅を保つ。**支えは 2 つ。面は自分の幅を宣言する
  ...
  読んでいた頃は開示を落とすと隣のボタンが 58px から 39px へ縮んだ。もう 1 つは、空いた半分を
...
- **フォーカスの行き先は、何かが壊れる**前**に、1 箇所で決める。**ポップオーバーを閉じる再構築は
  操作者が立っている行を取り除くので、閉じた**後**の採取には記録するものが残っていない — 全ラック
  再描画 (ライブ同期が読み戻しを要する実機側の編集ごとに走らせる経路) で、操作者が何もしていない
  のに `<body>` へ落ちていたのはこれ。`captureFocus` を先に走らせ、対象をストリップラックだけで
...
- **ストリップを再構築する選択では、消えた要素ではなく同じストリップへ着地する。**まず INS FX の
  ...
  落としている場合は**面** (`<body>` へ落ちていたのはこの場合)、そのどちらも無いストリップでも
