...
- 数値列は置かない。列のホバー / ドラッグ / フォーカス中、ラックヘッダの `SENDS` ラベルが
  ...
  リフローさせず、読み値が使えるのはストリップ幅から開閉矢印と flex gap を引いた残りで、
  最長の読み値がそこに収まる必要がある — 折り返しも省略記号も持たないため、幅を超えた読み値は
  切り詰められ、短くなったレベルがそのまま実在の値に見える。タップは載せない (読み取り先は
  列自身の PRE ボタン)。`aria-valuetext` はレベルを載せ、床より上ではタップも併せて載せる
  (`"PRE, -3.2 dB"`)。OFF の送りは `"off (-∞)"` となりタップを名乗らない。
