...
- **再描画は DOM が保持していた状態を引き継ぐ** — 全体再描画 (`render()`) は全ストリップの要素を差し替える。
  ...
  メーターの弾道を新しいレーンへ移し (`carryMeterState`・`refreshStrip` と共有)、ストリップ領域のスクロール位置を
  復元し、キーボードフォーカスを同じコントロールへ戻す (`markFocus` / `restoreFocus`・ストリップ id + 索引 +
  クラスで照合し、再構築でそのストリップの形が変わっていた場合は復元しない)。再構築後のメーターは同じタスク内で
