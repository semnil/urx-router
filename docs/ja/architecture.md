...
> および `Assign` サブメニュー)。**INS FX の各画面は 2026-08-28 に同じ方法で読んだ**。その 4 行が、
> Effect Reference Guide を裁定者にできない理由である: ガイドは文書であり `dev()` が名指すのは**画面**の
> 印字で、この 4 行では両者が食い違う。本体は `1-Knob` / `1-Knob Level` と印字する (ガイドは `1-knob`)、
> ギターキャビネットは `Gate` (ガイドは `Off/Gate`)、マルチバンドコンプは `M.B.Comp` (ガイドは `M.B.COMP`)。
> 4 つ目は文書をいくら読んでも決まらないもので — **Clean の変調セレクタには本体にラベルが無い**。だから
> アプリが独自に付け (`Modulation`)、`dev()` ではなく `fixed()` にしてある: 写すべき実機の文字列が存在しない。
> 本体が名前を付けている INS FX の行はいずれも先頭のみ大文字で、全大文字ではない — ギターパネルのカード
> ラベルはこれに従っている。したがってこの 7 画面は全体が翻訳対象外になる — `Range` / `Ratio` を含む
