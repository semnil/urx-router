...
`confirmedAddrs` (`control/client.ts`) がその集合である: 送信され受理されたものを、**converge の最後の
diff が実際に読んだ集合と交差**させ、その diff がまだ差があると見たものを引く。交差が要となる半分で、
読み取り失敗を引くだけでは足りない — diff は最初の失敗で止まるので、それ以降のアドレスはどちらの一覧にも
入らないまま送信済みとして残り、確定と報告されていた。ライブの**直接書込**は意図的にこの外に置く:
