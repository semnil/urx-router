...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  microSD Rec は意図的に除外)、およびアナログ出力の MONO が何を示すか (`outputMono` / `isAnalogOutput`。
  ...
  dB 値の正典リストと、位置 / スナップ / ステップのヘルパ。**アプリが作る**レベルはインスペクタ・CONSOLE・外部
...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  すべてキャンバスと同じ表記を出すためである) / `graph-text.ts` 盤面のテキスト計測を切り出したもの。文字列が
  ...
  **差分は編集時ではなくコミット時に取る** — 複数のファネルが `markChanged()` を呼んだ後にさらにプランを
  ...
  (ルーティング規則と宛先バスの params だけで決まり、並びは実機の SEND TO 画面の読み順。DOM は持たず、注記は
  ...
  **後**に設置する。これがコードログに「アプリがそのコードを取ったか」を報告させている / `dom.ts` 共有 DOM
  ...
  開いたノードを調整しない。ダッカーはステレオチャンネルの下にぶら下がる別ノードで、キーはよそから来た
...
> という状態を作らない。実際にその形で割れていた: ラベルと凡例は英語のまま、`tr()` の文 7 箇所が片仮名に
> 転写していて、何も検出できなかった — `dev()` / `fixed()` / `tr()` の印が強制するのは文字列の**同一性**で
> あって、訳文がどの語を使うかには何も効かないため。**本体の画面にあるコントロールを写した行は、その画面の英語ラベルを
...
> ステータスとエラー文はすべて翻訳し、説明文・ヒント・ツールチップも同様 (**上の 5 語を除く**)。実機ラベルの略号を展開するツールチップも本体の表記のまま
...
| --- | --- | --- |
| CH → FX 送りの tap (193 / 197 / 320 / 324) | broker が `max_value` 0 を公開しており PRE を書けない (`sendTapWritable`) | **チャンネル** — その scoped read が、送り先の全バスについて `params.tap` を読み直す |
...
CH → FX センドタップ `193`) と、そもそもプランに含まれない機器全体の設定 (`planExternal` — Follow USB と
...
実機は [MONO] スイッチを MONITOR バスにだけ持つ (`device-model.md` §4)。MAIN OUT / LINE OUT は
...
- **CUE Interrupt もアナログコネクタに及ぶ**。両方のタップより上流にあるため。工場出荷 ON なので、
  MAIN OUT に MONITOR をパッチしている状態でどこかの CUE を押すと、スピーカーの音が置き換わる。
  パッチ線の注記がそれを述べる — 警告ではなく、MONO 行と同じ「提示」として

図が与え**ない**のは MONO ブロックの利得則で、そこは実機で測った (URX44V、2026-08-13): **電力和
`(L+R)/√2`** である。STEREO L だけに −30 dBFS のトーンを入れると、モニターのメーターは MONO off で
−30.0、**on で両レーンとも −33.0**、戻すと正確に −30.0 に復帰した。したがってセンター定位の素材は
モノ化しても音量感が変わらず、片側に振り切った素材は 3 dB 下がる (据え置きでも半分でもない)。
アプリのどの文字列もこの法則に依存しないが、この機能でモノ互換を判断する人は依存する。
...
注記が付く。判定は `core/constraints.ts` の `outputMono` / `isAnalogOutput`、線の文言は
...
いたため、チャンネル発の**ダッカーキー**は選択時に説明が出るのにホバーでは何も出なかった。いまは
...
**警告カードは意図的に作っていない**。ここが、その他の点では対応関係にあるダッカーバイパスとの
分岐点になる。あちらは plan 内の矛盾 (`duckerOn` が ON かつライブ出力へのプリフェーダータップ) で
...
まま残すし、どのアナログ出力にも繋がっていない MONITOR バスは普通の構成である。警告にせず提示に
