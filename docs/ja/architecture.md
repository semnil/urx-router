...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  そうしないと A→B→A と動いた編集は触れられていないキーと区別できず、読みが運んだ過渡値 B が正典化される) /
  ...
  (`channelDuckerOn` = PRE 送りの注記、`duckerBypassWarnings` = USB ダイレクトアウトのプリフェーダータップ警告。
  microSD Rec は意図的に除外) / `plan-validate.ts` プラン読み込みの単一検証ファネル `planProblems`
  ...
  dB 値の正典リストと、位置 / スナップ / ステップのヘルパ。フェーダーと送りレベルはこのグリッドにスナップし、
  ステップは等間隔に配置する) / `storage.ts` 保存 / 読み込み / 画像書き出し (PNG・PDF。PDF は自前の FlateDecode) /
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **チャンネル調整画面が編集する全パラメータ**を対象とし、それらの画面が使うのと同じグリッドへスナップする
    ...
    着地することはない)。id の第 3 要素は**スコープ**である: 送り先バス (`@bus.mix1`) か、プロセッサ / バンド
    (`@gate`、`@comp`、`@eq.low`) — 1 つのノードはフェーダーを 1 つ持つがしきい値は 3 つ持ち、バンドがカーソル
    ではなくスコープなのは、割当が画面を閉じたままでも働かなければならないからである。実機のロックは書込を
    拒否する (FIXED バス送り、Pan Link の送りパン、レート制限されたステレオ CH EQ、1-knob 下で実機が駆動する
    ...
    フローがプランを保持している間 — 操作者が始めた 2 つのラッチであり、モーダルでもライブフラッシュの
    再取得待ちでも learn でもない — に起き、ステータス行へはメッセージごとではなくゲートされた窓ごとに 1 回
    報告する。MIDI ラーンの状態機械、フィードバック (送信キャッシュとの差分 + 受信中 300 ms のエコー抑制 +
    トグル用の受信側ワンショットエコーガード)。複数のマッピングが 1 つのアドレスを共有するとギャング
    (`byKey`) を成し、1 つの物理コントロールが全メンバーを駆動する (受信メッセージは全員へ配る) 一方、最初に
...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  すべてキャンバスと同じ表記を出すためである) / `history.ts` アンドゥ / リドゥ (`Ctrl/Cmd+Z`、
  `Ctrl/Cmd+Shift+Z`、`Ctrl/Cmd+Y`): ジェスチャの境界、キーボードの述語、`core/plan-history.ts` の上の適用
  シーケンス。1 エントリは最初の編集から最初の境界まで続く (`pointerup` / `pointercancel` を 1 マクロタスク
  ...
  **差分は編集時ではなくコミット時に取る** — 複数のファネルが `markChanged()` を呼んだ後にさらにプランを
  ...
  `inspector.ts` 選択要素のエディタ / `console.ts` CONSOLE ビュー (ミキサー型のレベル一覧。GRAPH/CONSOLE タブで
  切り替える、同じプランの別ビュー。フェーダー / MUTE / EQ の編集はグラフと同じライブ同期のために
  `markChanged` を通る。ストリップごとの SENDS ラック。スクリブルの電源 LED が各ノードマスタ (CH_ON / バス
  ...
  **ウィンドウ**を開く。ポート、エンジン、learn 状態、`urx-midi` 下の機種ごとの永続化を所有し、武装フックと
  ...
  `src/midi-window.ts`、デモビルドからは落とす 2 つ目の Vite エントリ — それは**ビュー**である: 押し出された
  ...
  **後**に設置する。これがコードログに「アプリがそのコードを取ったか」を報告させている / `dom.ts` 共有 DOM
  ...
  解放と再取得を指示する。`dyn-gate.ts` / `dyn-comp.ts` / `dyn-eq.ts` が記述子、`dyn-chan.ts` が 2 つの MONO IN
...
- `src-tauri/` — Rust シェル。webview のホスト + tauri-plugin-dialog + ファイル IO コマンド
  ...
  (どの切れ方も 1 つのエピローグに着地する: セッションが登録した全アドレスを解除し、それから WebSocket の
  クローズハンドシェイクを完了する — 置き換えられた接続、明示的な切断、落ちたチャンネル、ポンプ自身のエラー、
  ...
  `midi_window_open`、および 4 つの Channel リレーコマンド。メインウィンドウの破棄がこれを閉じ、これ自身の
...
> `Bus send (ON/OFF switch)` / `Pre-fader send`。実機ラベルの略号を展開するツールチップも本体の表記のまま
...
- **輪郭。** 押下状態は `3px double CanvasText` — システムパレットが隣接色へ潰せない唯一の太さ。
  ...
  キャップが走る溝) は、塗りを同じ形状の輪郭へ置き換える。
- **島。** 色そのものが読み取りである面 (スクリブルの機種色、メーターの緑/黄/赤、盤面のワイヤとレールの
  語彙全体) は `forced-color-adjust: none` で離脱する。これらをシステム色 2 色へ潰してもコントラストは
  上がらず、情報が消えるだけ。盤面には加えて縁を付け、島が地に対して端を持つようにする。

3 つ目の手段もあるが範囲は狭い — **システム色**なら塗りとして使える。フェーダーのキャップが
`background: Canvas` を取るのはこの 1 点のためで、通常テーマで不透明なキャップが溝と 0 dB 線を隠すのと
同じ重なりを保つ (0 dB 線の規則はその重なりを前提に書かれている)。**著者の色**はこの用途に使えない —
モードが置き換えてしまう。

**書いておくべき罠が 2 つある。どちらも実際に欠陥を出している。**

- **ミニフェーダーはフェーダーと同じ部品を自分の選択子で持ち直している。** `.con-vfad` は
  `.con-fader` と溝 / キャップ / 0 dB の文法を共有しているが、共有は規約であって選択子ではないので、
  片方を名指した規則はもう片方に届かない。ブロックの初版は `.con-fader` しか名指しておらず、
  ミニフェーダーは 0 dB 線とキャップの刻線を黙って失っていた。
...
シェルが覚える (「ウィンドウの配置」を参照)。このウィンドウはメインウィンドウの**子**として生成されるので、
メインウィンドウの裏へ回ることがない。メインウィンドウを閉じるとこのウィンドウも閉じ、このウィンドウを閉じると
ラーンが解除される (でなければ画面上のどこにも出ていないコントロールに対してアームされ続ける)。

オーバーレイではなくウィンドウであるがゆえに**他のアプリケーション**の背面へ回りうるので、割り当てを操作者の
視線の先へ戻す仕掛けを 2 つ持つ。**ラーン投入時に自分を前面へ出す** (単なる `set_focus`。常時最前面にはしない —
1 つのジェスチャの間だけの問題を解くためにセッション全体でアプリを覆うことになる)。もう 1 つは、割当済みコントロールが**アドレスを
...
そのループはフラッシュの書込台帳に何も記録しないので、名前のアドレスがオーバーレイの材料である `PendingWrites` に
入ることが無い。settle を掛ければ必ず上限を使い切り、送った値で答えるのは本節が禁じている。notify 自体は届く —
...
(SSMCS の Sweet Spot プリセット、param 91)。これはどこにも登録されず、リネームの notify とも無関係である。
...
かは `winfit.rs` の冒頭が持ちます。計算は矩形に対する純粋関数 (`fit` / `host_area` / `at_least`) で
`cargo test` が押さえています。

**復元したウィンドウは自分の最小サイズまで引き上げられます。** 覚えている大きさは物理画素、設定した最小は
論理画素なので、両者が一致するのは 1 つの表示倍率のときだけです — 100% で保存した矩形を 150% で復元すると、
アプリが宣言した最小より小さいウィンドウを指すことになります。そしてそれを戻すものが下流にありません。
**プログラムから与えた大きさは最小まで引き上げられない**ことを Windows で実測しました。プラットフォームが
最小を強制するのはユーザーのドラッグが通る経路だけで、プログラムからの移動はそこを通らないためです。
そこで `at_least` が、フィットの**前に**現在のスケール係数を使って矩形を引き上げます。最小は各ウィンドウが
自分で渡します — メインは実行中の設定から読み、MIDI ウィンドウはビルダーと共有する定数 1 つです。
最小サイズが作業領域より大きいウィンドウは、やはり最小より小さくなります — フィットが勝ち、左上だけが
合わせられ、それ以上に取れる手がありません。
...
メインウィンドウの復元はプラグインから取り上げ (`skip_initial_state`)、保存された矩形を状態ファイルから読み、
補正して一度だけ適用します。MIDI ウィンドウはプラグインの復元のままです — こちらはイベントループが回っている
最中にコマンドから構築されるので、`build()` が返る時点で復元が着地しており、読み戻しが成立します。**ただし
その時点でだけ**です: 将来どのウィンドウでも自動で補正が効くように、window-state プラグインの後ろへ自前の
フックを登録する形を試して**実測で否定しました** — `window_created` フックの中では、プラグインが直前に復元
していてもウィンドウは生成時の位置を返します。そこから出した `set_position` が、起動時と同じようにキューへ
載るためです。したがって補正は `open_midi_window` の末尾にあり、同じ作り方で 3 枚目を足すなら同じ呼び出しが
要ります。

**MIDI ウィンドウはメインウィンドウの子です。**これが前面を保つ仕組みで、Windows では所有ウィンドウが z 順で
常に所有者より上に、macOS では `addChildWindow` がアプリ内で親より上に並べます。「常に最前面」は意図的に採って
いません — それはセッション中ずっとパネルを他のあらゆるアプリケーションより前に出す約束になります。
`focus_midi_window` は残っており、ラーンを入れたときに呼ばれるのも変わりません — 他のアプリケーションより
前へアプリを出すことと、この 2 つのウィンドウの前後を決めることは別だからです。

子であることの代償は両プラットフォームで実測しました。**そして両者は一致しません**:

| | macOS | Windows |
| --- | --- | --- |
| 所有者より上に留まる | はい | はい (所有者を活性化して押し上げても) |
| 所有者の最小化で一緒に隠れる | はい | はい |
| **所有者と一緒に動く** | **はい** (同じ差分、1 画素まで一致) | **いいえ** (その場に留まる) |

Windows の答えは偶然ではなく構造で決まります: パネルはメインウィンドウのクライアント領域の子ではなく、
所有者を持つ**トップレベル**のウィンドウで、ウィンドウマネージャが親と一緒に動かすのは前者だけです —
移動を誰が発行しようと、追従を実装しうる層が存在しません。ドラッグが配るメッセージ列を再現した場合も、
プログラムからの単純な移動の場合も同じでした。

`--reset-storage` は `localStorage` と併せて記憶した配置も消します。状態ファイルはプラグイン自身の setup で
読まれるので、削除はその前に済ませる必要があります。そのための小さなプラグインを 1 つ手前に登録しています。
