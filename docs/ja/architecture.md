...
>
> **面バーのセグメント名は実機ではなくアプリのもの**であり、だから `fixed()` である:
> マルチバンドコンプの `Main` / `Low` / `Mid` / `High` と SSMCS バンクの `Main` / `Comp` /
> `Side Chain` / `EQ`。このうち 3 つは 2026-08-30 まで `dev()` と宣言されていた。marker 自身には
> これを捕まえられない — 型で翻訳を止めるだけで、宣言が真かどうかは何も言わない。したがってこの 7 画面は全体が翻訳対象外になる — `Range` / `Ratio` を含む
