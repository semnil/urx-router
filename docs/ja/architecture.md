...
2026-08-13 に WebView2 151.0.4129.78 で Windows の 4 テーマすべてを実測した: 施錠したインスペクタの
セレクトはどのテーマでも画素で判別できる — 自分の箱の 8544 画素のうち 714〜719 画素が変化し、平均輝度は
48.8 → 36.6 (hcblack)、236.3 → 245.3 (hcwhite)、64.7 → 53.1 (hc1)、19.2 → 5.6 (hc2) — つまり減光は読め、
このモード専用の言い直しは要らない。エンジン側も独自の置換を掛ける: 施錠したセレクトの文字と枠は
`GrayText` になる。一方で、通常テーマでは自前の **0.7** を当てるのに対し、forced colors では
**opacity を一切当てない**。したがってこのモードでの opacity は著者側の 0.45 が全てで、2 つの機構は
択一ではなく重なって効いている。
...
E2E は Playwright の Chromium と WebKit でこれを固定しているが、**出荷エンジンである WebView2 は別途実測した**
(2026-08-13・debug ビルド・ランタイム 151.0.4129.78・ビューポート 1280x800・URX44V)。どちらの tier もこの
...
Windows 側の列は 2026-08-13 に全行を測り直しました (debug ビルド・WebView2 151.0.4129.78・2560x1440 の
プライマリと 1920x1080 のセカンダリ、いずれも DPI 100%)。1 行目が 1 つではなく 2 つの測定なのは、
