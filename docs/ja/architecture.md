...
パネル外のクリックでも閉じる (ラーン中を除く — その間のパネル外クリックは割り当て操作)。メニュー項目は
`--experimental` 起動時のみ表示される (実機コントローラーでの検証待ちのため)。
...
- **マッピング (core/midi/)** — 言語非依存の純ロジック。`message.ts` が CC / ノート / ピッチベンドを
  decode/encode、`mapping.ts` が自由マッピングモデル (アドレス・取り込みモード・相対エンコード) と永続化
  バリデーション、`controls.ts` が CONSOLE の全コントロールを**固定コントロール id** (`node/param[@送りターゲット]`、
  例 `ch1/level@bus.mix1`) で列挙・解決する。固定 id なので表示中のタブや send-on-fader に依存せず、
  「CH 1 のメインフェーダー」と「CH 1 → MIX 1 送り」は別コントロールとして個別に割り当てる。値は正規化 (0..1)
...
- **エンジン (`engine.ts`)** — 受信イベントをマッピング経由でコントロールへ適用する。取り込みモードは per-mapping
  で absolute / pickup (物理値が盤面値へ到達・交差するまで無視) / relative (2 の補数・offset64・符号ビットの
  3 エンコード、1 クリック = 1 detent)。14bit CC は MSB/LSB ペア (n / n+32) を対で組み立てる。トグルには取り込み
  モードの代わりに per-mapping のボタン動作がある: 既定の「Toggle」(ノート on / CC 立ち上がりエッジ ≥64 で
  反転・離すは無視) と「Momentary」(ノート on / CC ≥64 = ON、それ以外 OFF = 値がそのまま状態。押している間
  だけ ON にするボタンや、押すたびに 127/0 を交互に 1 通ずつ送る送信元 — 例: Elgato Stream Deck MIDI プラグイン
  のトグルボタン。エッジ検出では 1 押しおきに無視されるため、こちらを選ぶ)。表示名は他の MIDI マッピングツール
  の慣用語彙に合わせて日本語 UI でも英語のまま、保存値は edge / state。
