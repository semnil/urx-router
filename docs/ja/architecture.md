...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  リバーブタイムへの編集と実機側のルームサイズが同一キー扱いでアプリが両方を取っていた。SSMCS バンクの各
  ...
  3 つ目の理由が来たときに何が得られたかは実測済み: インスペクタは編集ゼロで受けた — 理由が何であれロック済み
  option を disabled で描くため — 一方、理由を行に名指しするコンソールは分岐と専用メッセージが要った。
  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **メッセージ前の状態から**決めてから書く — あるメンバーの適用が別のメンバーのコントロールを動かしうる
    ...
    (ミラーが partner の nodeParams と全 send を置き換えるため)、PAN が運ぶのは insert FX だけである — 順に書いていた頃は 2 つで、ミラーは最後に
    書かれたものに落ち着くため、開始時点で値が食い違うペア (プランはその状態を持てるし、無警告で読み込める)
    は learn 順序が最後に置いた方の値になっていた。**何もしないと決めた**メンバーもグループの席を保つ — ギャングは edge と state を意図的に混ぜられるので、
  ...
  - `src/core/control/` — 実機制御 (vd プロトコル)。デスクトップでは書込とライブ同期は常時有効で、
    ...
    - `comp-ratio.ts` コンプレッサーの Ratio が止まる段 — チャンネルの COMP と SSMCS ストリップのコンプは
      ...
      **どちらの epilogue も持たないフラッシュ** — 読みを 1 つも出さない通常の編集 — は、**待たない watch** で
      ...
      誰にも検査されない書込になっていた。**1 つのアドレスの未処理の義務は前方へ統合される**:
    ...
    - `preferred-numbers.ts` 標準数 (ISO 3・Renard) の grade 表 R40 / R20 と、その参照関数。実機の周波数表は
      ...
      以前は各々が自分のコピーを持ち、浮動小数で食い違っていた (1.0 基点の仮数表は grade 112 の位置で
      112.00000000000001 を返し、クロスオーバー画面は実機が `112` と出す周波数を `112.0 Hz` と印字していた)
...
> `Mid`・`High`・`Side Chain` は `dev()` のまま。アプリ自身の 3 つは 2026-08-30 まで `dev()` と
> 宣言されていた。marker 自身にはこれを捕まえられない — 型で翻訳を止めるだけで、宣言が真かどうかは
> 何も言わない。
>
...
- **フィードバック (MIDI OUT)** — 盤面変更 (UI 編集・実機 follow・実機 readback・plan 読込) を割当の逆引きで
  ...
  **編集**である: 盤面側の刻みが値の渡った 7 ビットより細かいと、復号値が隣の刻みへスナップしてエコーが値を動かし、
  ...
  release、DUCKER の attack / decay。COMP の **ratio** は、フィールドが実機の段の列になった時点でこの一覧から
  外れた — 段の数がワイヤーの位置の数より少ないので、7 ビットのエコーは出ていった段へ戻る)。`localStorage["urx-midi-log"]` を設定すると全 rx/tx バイト列とエンジンの
...
再ネゴシエートする。モデル選択がロックするのはライブ**セッション**中だけであり、切断時ではなくトグルで解除される唯一の項目に
なった: モデル切替は plan を丸ごと
...
- **実機が実際の状態を変え、しかもそれを通知しない場合。** 選択中の insert エフェクトの上限を越える
  ...
  この再送は消去を起こしたレートのままでも受理され、対応レートへ戻しても保持される (エンジン 64 スロット
  全件を非既定の sentinel 付きで照合)。レートが戻るのを待たずに送るのはこのため。**もう一方の消去** —
  ...
  エフェクトは従来どおり採用される。保持した件数はステータス行に
...
**追従だけを行うケースが 4 種類、同じ理由で登録集合に加わる**。名前の経路はその前例であって偶然の一致ではない:
アプリが**読むだけ**のアドレスはどの登録にも入らないため、実機の告知が誰にも届かず、値が追いつくのは次の
全体読みのときだけだった。最初の 2 種類は URX44V で front-panel 操作に対して notify を出すことを実測している
...
担当で、park は元から読んでいない。もう 1 つ、当セッションが最後に送った値を実機がまだ保持しているアドレスでは
...
1. **self-test は中断せず集約する。** これはユーザー操作ではなく診断機能であり、往復に失敗したパラメーターを
   ...
   **そして 1 パスが書くのは値だけではない。** MONO IN ペアの Signal Type も掃引対象で、走行は 2 ブロック
   ...
   完走した走行は出発点に戻る。変わるのは完走しなかった側で、**死んだブロックのペア状態と、遷移で実機が
...
従来どおり画面下部の status 行に出す。**ダイアログを上げられなかった**場合 (シェルのコマンド自体が reject
...
したメモは、ノードがもう保持していないエフェクトについて答え続ける。(そのメモは実在し、2 本目の差し替え
経路 — 取得のキャンセル — のために書かれていた。読み出し前の複製をオブジェクトごと戻していたため、MIDI
バインディングがどこからも参照されないプランに残ったのである。読み出しが専用コピーで走るようになった
今はキャンセルが画面上のプランに触れないので、メモは**起こりえない 1 件を守りながら、起こりうる全てを
取り逃していた**。) アンドゥはオブジェクトを差し替えない。その場でパッチを当ててから、
...
かつては沈黙が 1 つの表明であり (「このチャンネルは実機が持っているままにする」)、スキルの `SKILL.md` と
`references/plan-schema.md` はどちらもそれを使うよう著者に指示していたが、今はそうではない。実機のエフェ
...
— 実装して測って差し戻した — ため、補完は**ロード時**に行う。そこでなら、それが何を意味するかを操作者に伝え
...
は何も送らないため、当該アドレスを一切運んでいない flush が成功しうる。最初の版は走行の成功を根拠にして
おり、どの書込も触れていない値を取り込んでいた — 無関係なフェーダー操作で再現した。
...
入らないまま送信済みとして残り、確定と報告されていた。台帳はラウンド終了時に組み直すのではなく**常に現在の
...
ラウンドが**計画された時点**で無効化していた版は、後続ラウンドの sideEffect コマンドが拒否されるたびに
確定済みの値を捨てていた。
