...
`readback.applySilentState`)。送られるのは実機自身の値になる。効いている制限が 2 つある。**収束の先頭ノードは
外す**: その従属値を実機にリセットさせたのが先頭の書込で、戻すのが収束そのものなので、ここで読むとリセット値を
採用してしまい復元が出ていかない。もう 1 つ、当セッションが最後に送った値を実機がまだ保持しているアドレスでは
