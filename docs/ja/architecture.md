...
運用なので、必要ならそのファイルの履歴から読み出します。**2026-08-13 の走行の分はそこに無い** — あの走行は
各節の「結果」を書き換えないまま節ごと削除して畳んだので、履歴に残るのは「未測定」のままの 3 節です
(決着した 4 件目のセレクトのドロップダウンは、独立した節になったことが一度もありません)。あの走行の数値は
この文書 (フェーダーの押下 / MIDI ウィンドウ / forced colors の施錠) と `src/style.css` (ドロップダウン) が
持ち、画像はあのディレクトリの `screenshots/` が持ちます。
