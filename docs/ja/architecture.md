...
`planToCommands` が FX チャンネルを送信するのは、プランがそのチャンネルの `fxEffect` を持つときだけで
ある。**この沈黙は 1 つの表明**であり、「このチャンネルは実機が持っているままにする」とプランが言う手段
である。スキルの `SKILL.md` と `references/plan-schema.md` はどちらも、エフェクトの変更を依頼されて
いないときはこのセクションを省略するよう著者に指示している。記述の無いチャンネルに既定値を送れば、何も
語っていない文書から実機の FX を初期化することになり、EFFECT TYPE の書き込みは復旧できない — エンジン
配列をそのタイプの既定値で埋め直し、元のタイプに戻しても以前の値は戻らない。

セクションが存在すれば、チャンネル全体が著作される — **部分的な FX 書き込みは存在しない**。文書がタイプ
を名指すかどうかに関わらずセレクタが送信され (不在なら当該チャンネルの工場出荷時タイプへ解決される)、
すべてのパラメータスロットがそのタイプの既定値で伴う。タイプ書き込みはプランが省いたスロットもどのみち
埋め直すからである。したがって `{ "level": 80 }` はタイプを名指す文書とまったく同じようにエフェクトを
初期化し、`fxEffect.params` だけを省いても何も保たれない。

**パネルはこのどれも語らない。** `inspector.ts` は `fxEffect` の不在を `{}` として読み、解決済みタイプ・
ON・レベル 100・各ディスクリプタ自身の既定値を描くので、記述の無いチャンネルには送信されないエフェクト
一式が表示される。2 つの読みはそれぞれ単独では整合していて、互いに食い違う。この食い違いは気付かれるのを
待つのではなく `inspector.test.ts` に pin してある。**送信側**をパネルに合わせる修正は自明に見えて破壊的
だからである — 実際に書き、測り、差し戻した。パネルに必要なのは「この行はプランの値ではなく既定値を表示
している」と言う手段で、それは FX ではなく疎なパラメータ全般 (バスマスタの level と pan も同じ形で飛ばす)
の問いである。

アプリ自身が著作するプランは両チャンネルに `fxEffect` を持つ — 出荷時の既定プラン (3 機種とも) も、
デバイス読み戻しも書き込む — ので、ここが届く先は外部で著作された文書、すなわち生成ツールの `?plan=`
ペイロードか手編集である。
