...
- **Plan** — ユーザーが作成する可変状態。`modelId`、ノード配置 (`positions`)、結線 (`connections`)、
  ...
  (ノードパラメータ + ルーティング + CH SETTING 色) をシードする。実機からキャプチャ済みなのは URX44V のみ。URX44 は
...
ノード数の多い機種では結線しないノードが場所を取り図を読みづらくする。そのため**編集可能な結線を
持たないノードだけ**をキャンバスから一時的に隠せる。隠したノードはキャンバス下部の**シェルフ**
(`graph.ts` が組み立てる HTML オーバーレイ。SVG には含めないため画像出力には写らない) にレール色の
チップとして並び、クリックで個別復帰、「全て表示」で一括復帰する。

- ツールバー「未接続を隠す」が編集可能な結線を持たない全ノードを隠す。インスペクタは選択中のそうした
  ノードに「このノードを隠す」ボタンを出す。
- **複数選択**: Ctrl (Mac は Cmd) + クリックでノードをドラッグせず選択にトグル追加する。2 つ以上選択すると
  キャンバス上にフローティングのアクションバー (シェルフ同様の HTML オーバーレイ) が現れ、一括「非表示」を
  出す。隠せるのは選択中のうち上記不変条件を満たすノードのみで、接続中のため保持したノードがあれば件数を
  通知する。「選択解除」または `Escape` で選択を解く。選択集合は一時的なビュー状態で永続化しない。
- **不変条件**: 隠せるのは編集可能な結線を持たないノードのみ。固定の CH/FX → STEREO 配線は数えない
  (固定 STEREO 配線しか持たないチャンネルも隠せる)。固定配線は端点が隠れている間は描画をスキップするため
  配線は宙吊りにならない。実ソース/Send を割り当てるとそのノードは復帰する (「`hidden` に含まれ、かつ
  編集可能な結線が無い」間だけ隠れる)。
- **Ducker の例外**: Ducker はキーソース配線を持っていても単体で隠せる (その配線は端点が隠れるため
  描画されず、ソース側ポートの点灯も止む)。親チャンネルを隠すと Ducker も連動して隠れ、Ducker を復帰
  すると親も復帰する — Ducker が親無しで単体表示になることはない。シェルフでは親子が共に隠れた場合は
  親チップ 1 つに集約し (子チップは抑制)、親チップの復帰でユニットごと戻す。
...
`autoLayout` と既定グリッドはどちらも列ごとに独立して縦に積む。
