...
(`LiveSync.hasUnannouncedWrite` を `DeviceFollow` が `isSuperseded` として問う)、そのノードを読み直す。書込より
前の notify は、そのアドレスを「実機から読む」側に残す。帰属を誤ったときにマージを正しく保つのはこの読み直しで、
マークが減らすのは無駄な照合 1 回である。2026-09-24 にレース診断ハーネスで実測 (`t1b-overtake.spec.ts` の
「a device value our in-flight write replaces is neither shown nor written back」の両変種): 盤面は操作者の値を
保ち続け、上書きされる側の値を実機へ書き戻すフラッシュは出ない。
