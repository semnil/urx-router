...
> `Bus send (ON/OFF switch)` / `Pre-fader send`。**本体の画面にあるコントロールを写した行は、その画面の英語ラベルを
...
> パン / バランスの行、バスアサインを含むオシレーターのブロック、モニターのブロック、CH SETTING の名前と色、
> および対応する MIDI コントロール名。**アプリ自身の語彙には及ばない** — 節見出し・凡例・ノードと接続の種別・
> ステータスとエラー文はすべて翻訳し、説明文・ヒント・ツールチップも同様。実機ラベルの略号を展開するツールチップも本体の表記のまま
