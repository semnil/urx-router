...
作動中はホバー/フォーカス中のものが琥珀に点灯する。`ui/fine.ts` がキーをグローバルに追跡し、
...
プランオブジェクトは差し替えない — グラフ・コンソールのストリップ・インスペクタのクロージャ・
MIDI バインディングはいずれも構築時の参照を保持している。アンドゥはその場でパッチを当ててから、
...
- 実機読み出し (取得 / ライブ同期開始) またはファイル操作がプランを掴んでいる間 — どちらも await を
  跨いでプランを書き換えるか差し替えるので、その下でパッチを当てるのは動き続けている前提に基づく操作になる
...
- **デバイス入出力の範囲** — 取得・書き込み・ライブ同期に共通の双方向スコープ。「全対応項目」
  ...
  両者は契約テストで固定される。読み取りは常に全域 — スコープ付きフェッチも全パラメーターを
