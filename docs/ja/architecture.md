...
**ラウンドではなく受理**である: 実機が拒否した書込は何も動かしておらず、送信前にキャンセルで止まった
書込も同じで、他のアドレスについて先行の読み取りが確立した事実はそのまま成り立つ。ラウンドが
**計画された時点**で無効化していた版は、後続ラウンドの sideEffect コマンドが拒否されるたびに確定済みの
値を捨てていた。ここに 2 つの経路が乗る。
