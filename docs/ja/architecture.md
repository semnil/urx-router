...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  **差分は編集時ではなくコミット時に取る** — 複数のファネルが `markChanged()` を呼んだ後にさらにプランを
  ...
  park **2 つとも** — 先頭書込の手前のものと収束の手前のもの。系統が重なるのでフラグではなく実行中集合への
  所属で追跡する。収束の**ラウンド**はこれに含まれない — 書込スコープ全体を読むがプランへは何も書き戻さない
  ためで、したがって収束を伴う flush のうち押下を拒否するのはこの 2 つの park だけである)、ファイルフローが保持している間、***ドラッグ*が進行中の間 (押下して動いたもの。
...
**flush はもう 1 つ、自分の先頭書込の手前でも park を取る。**上の park は書込の後段にあり収束のためのもので、
こちらは書込そのもののためにある — silent な系統をリセットする先頭はその系統を入ってくるタイプの工場値で
埋め直すので、実機が持っているのは出ていくエフェクトであり、それを告知するものは無い。flush は自分の先頭が
リセットする系統だけを読み (同じ `ParamSpec.resets`、今度は `only`)、その後でコマンドをプランから derive し直す
ので、出ていくものは読みがマージした値を運ぶ。先頭を動かしたコントロールではなく境界で取るため、1 回の読みが
すべての書き手を覆う: EFFECT TYPE の 2 つのセレクタ、そのどちらのアンドゥ、insert FX のセレクタ、MIDI の割当、
プランのロード。そこではプラン側の先頭を残す (`keepHeads`) — 入ってくるタイプは操作者が既に選んでおり、値は
**実機が乗っている先頭**が所有するキーの下へ入れる。2 つの park と収束は、デバイス追従から見れば 1 つの読取
...
reconcile は追従を停止する — 次の収束が、操作者自身が実機で行った編集の上に古い値を書き戻すのを防ぐ。**flush が取る
2 つの silent アドレス park は、書込がまだ手前にある状態で同じ規則を適用したものである**: 読みが失敗すればセッションを
落とし、後段の書込を止めるのは flush 自身の世代チェックである — したがって先頭書込は**タイプを一切書かず**、収束は
確認できなかった値の上にプランの写しを送らない。どちらも、読みで守ろうとした値を置き換えるのは後段の書込である
(channel-tuning.md「FX EFFECT」。park のスコープは[ライブ同期中のイベントとタイミング](#ライブ同期中のイベントとタイミング)にある)。取得のキャンセルは
