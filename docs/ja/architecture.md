  ...
  - `src/core/control/` — 実機制御 (vd プロトコル)。デスクトップでは書込とライブ同期は常時有効で、
    ...
    - `vd.ts` 値エンコード / `translate.ts` プラン→コマンド (**1 つの本体アドレスはちょうど 1 つのコマンドを
      ...
      **どちらの epilogue も持たないフラッシュ** — 読みを 1 つも出さない通常の編集 — は、**待たない watch** で
      ...
      誰にも検査されない書込になっていた。**1 本の notify は 1 つの書込にしか答えない**: mark は notify の位置なので、
      間に何も届かない 2 書込は同じ mark を共有し、「mark より後に notify が来たか」だけを見る判定では
      先の書込の告知が後の書込の義務まで満たしてしまう。**名称変更も epilogue に関係なく監視する** —
