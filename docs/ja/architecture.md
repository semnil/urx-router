...
ちプランが省いたスロットを埋め直すからである。したがって `{ "on": true }` はタイプを名指ししたドキュメント
と全く同じようにエフェクトをリセットし、`fxEffect.params` だけを省いても何も保たれない。

**ローダーが補完したドキュメントでは、パネルと配線が一致する。** `inspector.ts` は不在の `fxEffect` を `{}`
として読み、解決したタイプと ON を描く。各記述子の既定値は調整画面が描く。補完はその同じ値をプランへ入れるので、
