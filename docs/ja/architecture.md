...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  **既定値はタイプ別**なので、どれか 1 つの既定値を書けばそれは選択中タイプの推測になる)、チャンネルの
  ...
  `params` — サニタイザはこれらを残し、以降の読み手はすべて「無い」ものとして扱う。削除はどの値も動かさ
  ない。FX チャンネルはプランが記述していなくても送信されるためである (「プランが記述しない FX
  チャンネル」節)。丸めと削除は別々に
...
`planToCommands` はモデルが持つ **すべての** FX チャンネルを絶対状態として送信する。プランがその
チャンネルの `fxEffect` を持つかどうかに関わらず、解決済みタイプ・ON・レベル 100・各ディスクリプタ自身の
既定値が、プランが語らない分を埋める。

理由は、パネルが既にそのすべてを語っているためである。`inspector.ts` は `fxEffect` の不在を `{}` として
読み、まさにその既定値を描く。したがって送信側がチャンネルを飛ばすと、操作者が画面で読める値のうち実機が
一度も受け取らないものが生まれる — 各側は単独では整合しているので、赤くなるものは何も無い。アプリには
「このチャンネルは触らない」を表現する手段が無いので、飛ばすことが尊重しうる意図は存在せず、操作者が
持つ読みはパネルのものだけである。送信側はパネルが描くのと同じ既定値を取る。これが両者を「たまたま一致
する 2 つの答え」ではなく 1 つの答えにする。

これは読み取れないエフェクトオブジェクトを読込時修復がどう扱うかも決める: キーの削除が修復のすべてで、
どの値も動かさない。もう一方の案 (そのまま残す) は見た目より悪く、読み取れないオブジェクトが真値
(`[{}]` など) なら送信側に届き、何も語っていない値から工場出荷時の既定値 13 件を書き込む。
