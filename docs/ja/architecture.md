...
読ませ、ウィンドウの矩形を `CGWindowListCopyWindowInfo` で読み戻す方法)。1.0 の外部ディスプレイ上の
1234x813 (2405,-29) として覚えていた矩形は、スケールのファイルが無い状態 = 試行の経路を通って、**そのまま
同じ矩形**で同じディスプレイに戻りました。どのディスプレイにも乗っていない 1280x800 (4032,892) を、スケール
1.0 の記録付きで覚えさせた場合は、外部ディスプレイの内側 (2792,122) 1280x800 に戻りました — フォールバック
だけなら (2016,446) 640x400 になる位置なので、記録したスケールが効いていることが分かります。その後の正規手順
での終了で、矩形とスケールが同じ瞬間に書き直されました。

**Windows ではこの片方が未解決で、しかも未実測です。** あちらの desk が物理画素であることから上記の計算は
影響を受けませんが、復元が適用する**大きさ**は依然ウィンドウが移動する前に適用されます: `set_size` は生成された
ディスプレイ上で走り、続く移動が DPI の境界を越え、その時点で OS がウィンドウの**論理**サイズを保つように
リサイズします (`tao` の `WM_DPICHANGED` 処理。Windows 11 では Windows が提示する矩形をそのまま適用)。
この経路ではフィットの保証が無効になり、最終的な大きさは 2 つのスケール係数の比だけずれます。適用する大きさを
論理単位で渡せば OS 側の再スケールは無害化されますが、それは `WM_DPICHANGED` がプログラムからの移動で発火する
場合に限られ、発火しないなら同じだけ逆向きに外れます。Windows 側で 1 回測れば決着する項目なので、ここで
推測せず `reference/work/windows-verify/` に置いています。
