...
- **フィードバック (MIDI OUT)** — 盤面変更 (UI 編集・実機 follow・実機 readback・plan 読込) を割当の逆引きで
  ...
  **実機からの広い読み戻しすべて** (`planReadFromDevice` 自体にぶら下がるので、取得・ライブ同期開始・`.urxf`
  ...
  エコーとして破棄し、ガードを解除する (共有の仮想 MIDI バスや、フィードバック受信で状態が変わると再送するコントローラでは、
  ...
  **編集**である: 盤面側の刻みが値の渡った 7 ビットより細かいと、復号値が隣の刻みへスナップしてエコーが値を動かし、
  ...
  その返答は受信拒否が解ける前に着くのか後か。macOS + Stream Deck+ (入出力とも 1 本の IAC バス) での実測
...
- **ゲート** — 実機読み出しがプランを掴んでいる間 (`deviceReadInFlight`: 取得・ライブ同期開始の読み戻し)、
  ...
  **実機**を掴んでいる間 (`deviceRunInFlight`: セルフテスト・`--prepare-modified`) は受信メッセージを
  ...
  **分**の単位である。拒否はライブ同期の状態に依存しない — 診断は自前の接続を持つので、走行中に
  セッションが張られれば割当済みの操作はすべて照合の最中に着く書込になる。
...
**実機を掴めるのは同時に 1 つだけで、それを担保しているのはメニューである。** `syncDeviceActionUi` は、接続を
一定時間保持する 2 つのラッチ — ライブセッション (`liveSessionUp`) と破壊的な往復診断 (`deviceRunInFlight`:
セルフテスト・`--prepare-modified`) — から**毎回**ロックを計算し直す。動いたほうのラッチだけを見て設定しないので、
診断の走行中にセッションが終わっても、走行が掴んでおくべきものが再有効化されることはない。取得・書込み・
Compare・`.urxf` 取り込み・本体設定・レート選択はどちらでもロックされる。例外の 2 つは、いずれもそのラッチ状態
から**抜ける**手段そのものである: セルフテストの項目は走行中「セルフテストを中止」になるのでセッション時のみ
ロックし、ライブ同期のトグルは走行時のみロックする。このトグルをロックすることが、通常のコンソール編集が照合の
最中に実機へ届く唯一の経路を塞ぐ — 診断が読むのはプランではなく実機なので、セッションが張られていなければ編集
(および受信 MIDI) は文書の外へ出ない。レートのロックと同じくこれは不変条件ではなくガードレールである: UI より
下で 2 本目の `vdConnect` を拒否するものは無く、vdp トランスポートでは 2 本目の接続が**成功する** — casket では
片方が黙って沈黙することで相互排他になっていた。
...
— この後 macOS 側で上の表の 5 ケースを流し直し、全件同値であることを確認しました。ここに挙げた数値の
生の読みは `reference/work/windows-verify/` で採ったもので、決着した節はこの文書へ畳んでから削除する
運用なので、必要ならそのファイルの履歴から読み出します。
