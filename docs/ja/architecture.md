...
**捨てるのではなく書き換える値が 1 つだけある**: **ノード名**は実機 CH SETTING のフィールド幅 (63 UTF-8 バイト・コードポイント境界で切る。実機は 64 バイトの NUL 埋め要素に格納する) に切り詰める。捨てれば長いという理由で名前を失い、そのまま通せばフィールドに収まらない文字列がワイヤへ出る。名前はデバイスリンクを越える唯一の plan 文字列でもあり、数値の葉には `boundRaw` があるのに文字列には何も無かった。同じ切り詰めは emit 側でも適用する — 名前はデバイス読み取りと実機側でのリネームからも plan に入り、どちらもこの関門を通らないため。メモと色はアプリ自身のものなので上限を設けない。
