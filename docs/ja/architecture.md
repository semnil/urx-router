...
- `isStereoLinkedPair(model, plan, id)` / `mirrorLinkedPair(model, plan, id)` — MONO IN ペアの Signal Type が STEREO のとき、片 ch の編集を相方へミラーする (3 つを除くノードパラメーター + 各 Send の LEVEL/PRE-POST/ON。ゲイン・Clip Safe・位相反転は各メンバー自身の値。Signal Type/PAN-BAL フラグは primary のみ保持)。どれを共有するかは `pairSharesNodeKey` が 1 箇所で答え、ミラー本体・MIDI のミラー identity・各ファネルの書き込み証跡がそれを読む。コピーが相方に残したキーを証跡が申告することはない。**判定は PAN/BAL モードではなく Signal Type** で、これは実機の挙動に合わせたものである (実測値は [device-model.md](device-model.md))。モードが決めるのはその上の 1 パラメーター、pan だけ: `isBalLinkedPair` が pan を判定し、BAL ではペア共有のバランス 1 値、PAN では各 ch 自身の値となる。BALANCE 表示が問うているのも同じ問いである。各編集 funnel から呼ぶ: グラフ/インスペクタは `main.ts` の `onUpdateParams`/`onUpdateNodeParams`、CONSOLE は `console.ts` の `commit`。GRAPH/CONSOLE で同一関数を共有し挙動を一致させる。
