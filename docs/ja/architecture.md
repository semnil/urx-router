...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  アンドゥスタックから取り除く) と、ライブの flush が送る各コマンド (`onExcludedBy`。`live.ts` の
  スナップショットと実機の通知に対して)。ライブセッション中の読み出しは
  ...
  HI-Z の ON が一緒に下げた +40 dB の A.Gain も保留する (`carrierOf`)
...
| もう片方をオンにしている実機が受け取ることになる +48V / HI-Z の ON と、保留中の HI-Z の ON が一緒に下げた +40 dB の A.Gain | ライブの flush の送信ループ (`onExcludedBy`。スナップショットと、follow の読み出しがまだプランへ持ち込んでいない実機の通知に対して。A.Gain は `carrierOf`) | 捨てずにプランに残す: もう片方がオンだと見つけた読み出しが取り戻し、その後の通知やキャプチャが flush に問い直させる |
