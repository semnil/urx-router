...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  **後**に設置する。これがコードログに「アプリがそのコードを取ったか」を報告させている / `dom.ts` 共有 DOM
  ...
  この画面が丸ごと表示しない family はインスペクタ側のエディタに残り、`bind` も同じ答えを返す: Pitch Fix は
  Key・Scale・12 音マスクをフラットなカタログの外に持ち、マルチバンドコンプレッサはそもそもカタログに無い。
  ...
  バンドの選択はバーではなく応答上のマーカーを押して行う。INS FX は応答を描かない: ギターアンプの周波数応答も
  ピッチトラッカーもパラメータから導出できず、実機もどちらも計測していないため、表示列はレーンのラックだけで、
  `plotGeo` / `drawAxes` / `drawCurve` をまとめて省略する (`DynPlotProcessor` が、1 つでも供給する記述子には
  3 つとも必須にする型である)。
