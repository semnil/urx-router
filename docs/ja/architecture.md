...
従来どおり画面下部の status 行に出す。このクリアを 1 箇所だけ打ち消す: 書込のレコーダー再読み出しは `finally` で
**書込が結果行を書いた後に**走るため、そこでの失敗は陳腐化していない行を消してしまう — 値を取り戻した書込は
それを status 行にしか出さず、ダイアログが名指すのはレコーダー読み取りの失敗という別の話だからである。
読み取りの手前で行を控え、ダイアログの背後へ戻す。Live sync 中のランタイムエラーは複数ソース (live / follow / リンク監視) から
