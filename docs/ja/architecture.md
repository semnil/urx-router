...
**登録集合は、それを変えたフラッシュ自身が登録し直す — そのフラッシュが capture したときに。** 集合を組み直すのは
`capture()` で、フラッシュがそこへ到達するのは `sideEffect` param の converge / refetch の後処理だけである。
COMP/EQ モード変更は到達する。**send の結線は到達しない** — `SEND_LEVEL` / `SEND_PAN` / `SEND_ON` / `SEND_TAP` は
どれも `sideEffect` を持たないので、結線・切断は発行集合を動かしながら (実測: CH → MIX の send 1 本で 5 アドレス)、
登録は次に何かが照合するまで動かないままになる。結線についてはこの窓が開いたままである。
