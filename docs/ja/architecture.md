...
ウィットネス自体はさらに前に開く: フラッシュは、refetch の起点を含む値を最初に取った時点 — 最初の一覧、実機側の変化の後に
数値ループが取り直した時点、または名前ループが自分の一覧を取った時点 — で開き (`watchEdits`)、読みに渡す。書込が送信中の
間に入った編集はどの書込にも載らず、同じようにマージが取り消すからである。refetch の起点を持たないフラッシュは開かない。
