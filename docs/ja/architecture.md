...
担う: ノードパラメーターの読み出しを記録するプランでエミットを 1 回走らせ、**コマンドが送られる直前に読ま
れた最後のキー**がそのコマンドの値の出所である。値は push の直前に読まれるので、100 余りのエミット地点へ
手で刻印を置く形と違って両者がずれない。プランが持たないキーも「送るかどうか」を決める過程で読まれるが、
これは除外する — その読み出しの後に push されたコマンドには、背後にプランのキーが無い。
