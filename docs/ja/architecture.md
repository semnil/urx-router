...
| 切替が差し替えるプランの上でまだ続いているジェスチャ | プランの差し替え時に、それを持つ面自身 — 盤面 (`Graph.setModel` に別のプランを渡す)、inspector (ゲートを越えて再構築し、パネルを組んだプランに対してだけ操作が応答する)、調整画面 (`refresh`) | ジェスチャを始めたプランはもう無いので、押したままのポインタは差し替え後のプランに何も書かない |
...
応答する。調整画面は押下を待たずに再構築し、その押下は終わるまで何も書かない。
