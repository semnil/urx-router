...
フラッシュ自身の書込も settle を実機の告知で終えられる。登録されるのはプランがプリセットを持つ間、すなわち
そのチャンネルが SSMCS モードの間だけである。

**登録集合は、それを変えたフラッシュ自身が登録し直す。** 集合を動かすのは構造的な編集だけ (COMP/EQ モード変更・
結線) で、そうした編集を運ぶフラッシュは自分の capture で集合を組み直している。そこで追従層に再購読を依頼する者が
おらず、編集が足したアドレスは次の reconcile がたまたま走るまで届かなかった — SSMCS ブロックではモード変更自身の
プリセット書込が自分への返事を聞けない、という形で出た。依頼はフラッシュの**末尾**で行い、フラッシュの内側では
行わない: 再登録は購読を切ってから張り直すので、途中で走らせると refetch の settle が待っている notify を落とす。
