...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  修復は窓への丸めか、有限数でない葉の削除 — 窓はチャンネルの全タイプで共通だが**既定値はタイプ別**なので、
  どれか 1 つの既定値を書けばそれは選択中タイプの推測になる。
...
検証の後にもう 1 層下で行う、アプリが書き込めない範囲にある FX 値の丸め (有限数でない葉は既定値を推測せず
**削除**し、選択中タイプ自身の既定値に委ねる) と報告 (`plan-validate.ts`)。ここでの
