...
- STEREO リンク済みペアは canvas 上で♥コネクタで結ばれ、1 ユニットとしてドラッグできる。リンク時 (`stereoLink` が ON になった `onUpdateNodeParams` から呼ぶ `alignStereoPair`) はまず相方を保持ノードの隣へスナップする — 選択中のメンバーは固定し、もう一方を既定レイアウトの相対オフセットへ移動する — ため、リンク前の手動移動で開いた隙間に♥タイが伸びることがない。リンク後もペアは独立した保存位置を保つ (ducker の親由来位置と異なる) ので、自由にドラッグできる。アプリが作ったのではないリンクには `alignLinkedPairs` が同じスナップを与える。これは編集ではなく plan からペアを読む: 読み込んだドキュメントと plan 全体のデバイス読み出しはどちらも `Graph.setModel` を通り、実機パネル側で動かされた Signal Type のデバイス追従 reconcile では、そのリンクを持ち込んだ読み出しの隣で — その読み出し自身の trace サンプルより前に — 呼ぶ。レイアウト上の帰結を、次にサンプルする書き手ではなく、それが乗っているファネルに帰属させるためである。ここでは選択中のメンバーではなく **primary** を保持するので、盤面の選択状態に関わらず同じ 2 スロットに落ち着く。どちらかのメンバーが棚上げ (shelve) されているペアは対象外とする — ♥タイが描かれないため埋めるべき隙間が無く、見えているメンバーを棚上げされた相方に合わせて動かすと、画面に無い相方のために位置を変えることになる。そのペアが再び揃うのは棚 (shelf) 側である。チップから復帰したメンバーは、ビューポート中央ではなく既に盤面にいる相方からの既定オフセットへ着地し (`placeRestored`)、盤面にいた側は動かない — 両方が揃った時点で♥タイが描かれるので、動くのは戻ってきた方である。*Show all* もペア単位で同じ規則に従う: 戻ってくるのが片方だけならそれが動き、両方が棚上げされていたペア (どちらも「既に盤面にいる」側ではない) だけが primary 保持の一括整列に落ちる。盤面に相方がいないノードは従来どおりビューポート中央に置く (見つけやすいため)。オフセット自体は既定レイアウトのものだが、**上側ノードの描画フットプリントが収まるまで広げる**: Arrange は展開されたノートのために丸ごとの行を確保しており、両者は同じ 1 箇所 (`rowsFor`) から導くので、スナップが下側のメンバーを上側のノートの中へ置くことはない。2 つの位置が同じスロットかどうかはサブピクセルの許容幅で判定する — 操作者がドラッグしたペアは保存オフセットに ~1e-13 px の浮動小数誤差を持ち、デバイス追従経路では誰にも見えない書き込みでも編集ファネルの外側の plan 書き込みになるためである。
