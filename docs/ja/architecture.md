...
- **メーターポイント (ストリップ毎のタップ)** — ノードは信号経路上に複数の観測可能なメータータップ点を持ち、
  ...
  (遅延時間 1…1000 ms・インスペクタは細かい 0.01 ms グリッド) を持つ (空だったヘッドに内容を与え目的あるストリップ
...
- **実機側操作の追従 (device follow)** — ライブ同期の逆方向。同じ排出経路で**実機側のパラメーター変更**も
  ...
  不要。scoped/全体読み戻しのみ両ビューを再構成し snapshot を全再基底する。可視なビューは常に 1 つだけなので、非表示ビューの再構築は次に開くまで遅延
...
(`channelEqUnavailable` / `insertFxAvailable` が EQ・INS FX チップをロックし、`rateConstraints` が FX2 を減光)。
...
ファイル IO は小さな自前 command (`read_text_file` / `write_text_file` / `write_binary_file`)。いずれも `async` で
`std::fs` 処理をワーカースレッドに逃がし (`spawn_blocking`・vd command と同方式)、拡張子の許可リストを強制する
(読込: `json`・テキスト書込: `json` / `md`・バイナリ書込: `png` / `pdf`)。`write_binary_file` は PNG/PDF の
...
`devCsp`): スクリプトは `'self'` のみ、インラインスタイルは許可 (サンドボックスのライセンスページはページの CSP を
継承する)、画像は出力用ラスタライザとノイズテクスチャのため `blob:` / `data:` を許可、`connect-src` は Tauri IPC の
