...
に置かれる。`app/unauthored-writes.ts` が、書込が動かすアドレスのうち**値を未操作キーが決めている**ものを
持つストリップを報告し、書込の確認ダイアログがそれらを名指しする。判定はエミットの**形**ではなく**値**に
問う — 未操作キーを現在値から動かしたプランを再エミットし、コマンドの値が変わったアドレスがそれである。
形に問うと双方向に別の問いに答えてしまう: `insertFx` はそのノードの insert-FX ブロック全体を左右するので、
取り除くと操作済みのエンジンパラメーターのアドレスまで消え、逆に `fxEffect` のスロットはプランが持っていて
もいなくてもフォールバック付きで送られるので、取り除いてもアドレスは 1 つも減らないのに書込は値を送る。
`plan_tool.py` は同じことを 1 段手前で、生成したプランの著者に告げる。
