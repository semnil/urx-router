...
8544 画素 (箱の周囲 2 px を含む。箱そのものは 263x28) のうち 714〜719 画素が変化し、平均輝度は
48.8 → 36.6 (hcblack)、236.3 → 245.3 (hcwhite)、64.7 → 53.1 (hc1)、19.2 → 5.6 (hc2)。**どのテーマでも
施錠した面は自分の地へ近づく** — アルファ合成そのものである。hcwhite だけそれが輝度の**上昇**として出るのは、
4 つのうち地がコントロールより明るいのがこのテーマだけだからで、向きが逆なだけで同じ信号である。
つまり差はこのモードでも残り、このブロックで施錠を言い直す必要は無い。
...
「押しても変わらなかった」として読めてしまう)。つまみの上端・中央・下端の 3 点を押しても読み値は 1 度も動かず、フェーダー高の
...
異なり、Windows では Win32 の**所有者 (owner)**、macOS ではラーンが armed の間だけのピンで、あちらでは
この関係を外してある。メインウィンドウを閉じるとこのウィンドウも閉じ、このウィンドウを閉じると
...
覆うことになる)。もう 1 つは、割当済みコントロールが**アドレスを
...
`SetWindowPos(midi, HWND_BOTTOM)` は 2 枚を**実際にグローバルな z 順の下へ動かした** (= 呼び出しは
不活性ではない) うえで、パネルはメインの上に留まりました。`SetWindowPos(midi, main)` も同じです。
対称の試行である `SetWindowPos(main, HWND_TOP)` も順序を動かしませんでしたが、**こちらは根拠になりません** —
...
(拡張スタイルは 0x110 → 0x118)、解除で降り、どちらの状態でも
所有者との前後は変わりませんでした。
