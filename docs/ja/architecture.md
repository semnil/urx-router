...
reconcile は追従を停止する — 次の収束が、操作者自身が実機で行った編集の上に古い値を書き戻すのを防ぐ。**2 つの park は、
書込がまだ手前にある状態で同じ規則を適用したものである**: FX EFFECT TYPE の park は読みが失敗したときに
**タイプを一切書かず**、収束の手前に置く silent アドレスの park は、確認できなかった値の上にその収束が
プランの写しを送ることを許さずセッションを落とす。どちらも、読みで守ろうとした値を置き換えるのは後段の書込である
