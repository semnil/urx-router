...
択一ではなく重なって効いている。調整画面のランチャーのキャレットは**逆向き**で、このモードでは著者の減光を
**外して**キャレットを全強度で描く (`e2e/forced-colors.spec.ts`)。あちらの減光が符号化していたのは
**隣との段差**で、このモードは両者を同じ色へ潰すため、符号化するものが無くなって可読性だけを損なうからで
ある。施錠の減光が符号化するのは 1 つのコントロールと自身の非施錠状態との差 — このモードが潰さない側 —
なので、同じ理由から 2 つは逆向きになる。
...
かつて持っていた AppKit の親子関係は、その代償を実測したうえで外してある (「ウィンドウの配置」を参照)。
