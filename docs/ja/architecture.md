  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **メッセージ前の状態から**決めてから書く — あるメンバーの適用が別のメンバーのコントロールを動かしうる
    ...
    (解除される向きは同じ順序のままこれで足りる)。これで決着するのは狭い範囲で、そう書くだけの値打ちがある: **ミラーまたはロックで結合された
    メンバーについて、1 メッセージ分の書込結果は learn 順序に依らない**。ギャング一般が順序非依存になるわけ
    ではない — ピックアップの交差判定は head が所有するので、値の違うメンバーを束ねたピックアップのギャングは
    head の値で交差し、その値がどれかを learn 順序が決める
