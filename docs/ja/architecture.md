...
reconcile は追従を停止する — 次の収束が、操作者自身が実機で行った編集の上に古い値を書き戻すのを防ぐ。FX EFFECT TYPE の
park は、書込がまだ手前にある状態で同じ規則を適用したものである: 読みが失敗したときは**タイプを一切書かない**。
後段の書込こそが、その読みで守ろうとした値を置き換えるものだからである (channel-tuning.md「FX EFFECT」)。取得のキャンセルは
