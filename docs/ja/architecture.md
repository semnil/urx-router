...
- **コンピューターのスリープ** — アプリを開いている間、システムとディスプレイのアイドルスリープ
  を抑止する (既定 OFF・デスクトップ版のみ)。OS が拒否しうる唯一の設定なので判断は OS 側が持つ:
  `set_keep_awake` (`src-tauri/src/keepawake.rs`) が成功して初めて設定を保存し、拒否されたら行は
  そのままで理由を下に出す。抑止はプロセススコープでアプリとともに消えるため、`boot()` が同じ
  呼び出しで起動ごとに取り直し、そこでの拒否も報告する。
