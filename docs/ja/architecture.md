...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  読み戻しと `.urxf` 取り込みは意図的にこれを通さずにプランを著作する — そして **4 種**は報告のされ方が違う:
  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  この 3 種を分けるのは `isRefusal` と `needsDecision` の 2 述語で、それぞれ座席は 1 つ) / `plan.ts`
...
- `src/app/` — アプリのエントリのうち配線ではない部分。`main.ts` から切り出し、アプリ全体を起動せずに
  ...
  **報告**し (操作者のクリックが未応答に終わった)、2 つ目のファイルフローには沈黙する
  ...
  設定した側 (ドキュメントがその値を書いている) で、`default` と `device` はそうではない
...
- **DeviceModel** — 機種ごとの不変な装置定義。`nodes` (入力/チャンネル/Bus/出力/Ducker)、
  ...
  を持つ。`models/build.ts` が機種パラメータから生成する。
...
**追従だけを行うケースが 3 種類、同じ理由で登録集合に加わる**。名前の経路はその前例であって偶然の一致ではない:
...
問い合わせ、その後は自分で値を供給するので、それは誰かが選んだ値ではない。**記録を全く持たないコマンド**が
