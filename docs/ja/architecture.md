...
クライアントにしか**提供しない。2 本目が繋がると 1 本目は双方エラー無しで黙るので、broker に触れる他のツールが
あればこのアプリの実機リンクは奪われ、しかもアプリからは「静かなリンク」と区別が付かない。Device Center は
...
再ネゴシエートする。Follow USB だけは逆向きの例外で、ライブ**セッション**は自分のリンクをこれに貸す
