...
- **実機が実際の状態を変え、しかもそれを通知しない場合。** 選択中の insert エフェクトの上限を越える
  ...
  全体読み出しには保持するものが残らない。
  操作者が本体で選んだ
  No Effect との区別は、**読み出しが**確定させたレート (`ReadbackResult.deviceSampleRate`) が付ける。
  プラン側のレートは使わない — *シーンのみ* のデバイス範囲では読み出しを跨いで復元されるため、実機が
  とうに離れたレートを名乗りうる。レートを 1 つも確定できなかった読み出し (スコープ読みはすべてこれ) は
  何も保持しないので、操作者が手で消したエフェクトは従来どおり採用される。保持した件数はステータス行に
