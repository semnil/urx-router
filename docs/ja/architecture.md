...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  **差分は編集時ではなくコミット時に取る** — 複数のファネルが `markChanged()` を呼んだ後にさらにプランを
  ...
  開始、同様にデバイス追従の 2 つの照合とライブ同期の 1-knob 再取得、そして収束の先頭にある silent アドレスの
  park。系統が重なるのでフラグではなく実行中集合への所属で追跡する。収束の**ラウンド**はこれに含まれない —
  書込スコープ全体を読むがプランへは何も書き戻さないためで、したがって収束を伴う flush のうち押下を拒否するのは
  park だけである)、ファイルフローが保持している間、***ドラッグ*が進行中の間 (押下して動いたもの。
