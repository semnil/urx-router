...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  すべてキャン Bus と同じ表記を出すためである) / `graph-text.ts` 盤面のテキスト計測を切り出したもの。文字列が
...
✕ ボタン (`onClose` → `graph.clearSelection()` で既存の選択解除経路を再利用) と空キャン Bus のタップ。
キャン Bus のズームはホイール (デスクトップ) と 2 本指ピンチ (タッチ) の両方に対応し、どちらも同じ
...
(接続の有無にかかわらず) キャン Bus から一時的に隠せる。隠したノードはキャン Bus 下部の**シェルフ**
...
- **複数選択**: Ctrl (Mac は Cmd) + クリックでノードをドラッグせず選択にトグル追加する。2 つ以上選択すると
  キャン Bus 上にフローティングのアクションバー (シェルフ同様の HTML オーバーレイ) が現れ、選択全体を
  一括「非表示」にする。「選択解除」または `Escape` で選択を解く。選択集合は一時的なビュー状態で永続化しない。
- **隠したノードの配線**: 端点が隠れている配線は固定・編集可能を問わず描画をスキップするため、隠した
  ノードは自身の配線ごとキャン Bus 外へ退避し、配線が宙吊りにならない。配線自体は `plan.connections` に
...
を参照)。配線の判定は `core/routing.ts` の `directOutTarget` で、キャン Bus はその経路が実際に出て行くジャック
から描くだけである。
...
- **CUE Interrupt もアナログコネクタに及ぶ** — 両方のタップより上流にあるため。ただし**それが ON の
  ...
  そのまま述べる — 警告ではなく MONO 行と同じ「提示」で、**その行と違ってスイッチの状態は読まない**。
  `sendlessNote` は設計上 plan を取らずに線を分類するので、注記は現在の状態ではなく仕組みを述べる

図が与え**ない**のは MONO ブロックの利得則で、そこは実機で測った (URX44V、2026-08-13): **電力和
`(L+R)/√2`** である。決着には 2 つの状態が要り、片方だけでは足りない — STEREO L に −30 dBFS の
トーンを入れて MONO を on にすると**両レーンとも −33.0** で、これは電力和とエネルギー和
`√((L²+R²)/2)` の両方が出す値。**両レーンを同相で**鳴らすと **−27.0** になり、これを出すのは電力和
だけである (エネルギー和なら動かない)。
...
- **編集** — ノードを選択した状態で、開いているノートエリアをクリックすると編集に入る。ヘッダー
  (ノート外) のドラッグは移動、未選択ノードはどこでもドラッグで移動。編集はキャン Bus 専用で、
  インスペクタにノート欄は無い。
...
ビューポートを再フィットしてしまい、アンドゥでキャン Bus の見え方を変えてはならないからである。
