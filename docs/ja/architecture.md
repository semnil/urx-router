  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **チャンネル調整画面が編集する全パラメータ**を対象とし、それらの画面が使うのと同じグリッドへスナップする
    ...
    learn したリスト先頭がフィードバック / エコー / ピックアップを所有する。ギャングは全メンバーの目標値を
    **メッセージ前の状態から**決めてから書く — あるメンバーの適用が別のメンバーのコントロールを動かしうる
    ...
    (解除される向きは同じ順序のままこれで足りる)。結果がマッピングを learn した順序で変わってはならない —
    それは 2 つの割当をたまたま作った順序であって、押下が意味するものではない
...
**読取専用のアドレスが 3 本、同じ理由で登録集合に加わる**。名前の経路はその前例であって偶然の一致ではない:
...
3 本目だけはモデルに加えて**プラン**も要る: どのスロットか、そもそも 1 本あるのかが、エフェクトタイプと Sync
スイッチの関数だからで、その 2 つに応じて現れたり消えたりする。対になる相手も違う — 他の 2 本はアプリが元々
書けないアドレスだが、これは実機が導いていないときには書くアドレスで、2 つの一覧は厳密な補集合になる。
