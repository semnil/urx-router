...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  `entryInContext` は競合したネストしたグループ (comp / gate / eqBands / …) を**どれだけ深くても葉まで**絞るので、1 つの
  グループの*異なる*フィールドに対するアプリ編集とデバイス編集が両方生き残る。深さは些事ではない:
  `fxEffect.params` はレコードの中のレコードで、最初の 1 段で止まる走査ではマップ全体が 1 つの競合キーになり、
  リバーブタイムへの編集と実機側のルームサイズが同一キー扱いになってアプリが両方を取ってしまう。SSMCS バンクの各
  セクションと、配列の中のオブジェクトである eqBands も同じ形である。`PlanWriteWitness` は**読みが
  ...
  ステレオ CH EQ を強制 OFF、`channelEqUnavailable`、および FX2 バスそのものを落とす `nodeRateDisabled` —
  グラフが減光するノード、実機が走らせていないバスへ生きたフェーダーを差し出しかねない CONSOLE のストリップ、
  そこを狙う全センド列の 3 者が読む 1 つの述語)。これらは **UI 上だけのもの**である: 実機は 192 kHz でも
  「使用不可」な param への書込を受理して保持する (実測) ので、書込集合をレートでゲートすることは
  しない — 「サンプルレートと Follow USB」を参照。Insert FX メニューもここが持つ (`insertFxMenu` は全選択肢を
  ロック理由 `"rate" | "link" | "slot" | null` つきで返すので、インスペクタとコンソールの双方が 1 つの表の上に定義され、
  インスペクタは理由が何であれロック済み option を disabled で描くので、新しい理由も編集ゼロで受ける。
  一方、理由を行に名指しするコンソールは理由ごとに分岐と専用メッセージが要る。
  「ロックされている」だけを知ればよい面は無料で追従し、「なぜか」を言う面はそうならない。多数のメニューを一度に描く呼び出し元は
  ...
  読み戻しと `.urxf` 取り込みは意図的にこれを通さずにプランを著作する — そして **5 種**は報告のされ方が違う:
  不正なワイヤは文書ごと拒否し、**STEREO リンク済みペアの 2 メンバーが 1 つの insert エフェクトについて
  食い違う文書も拒否**する (実機はペアに対しセレクタ・バイパス・エンジンを 1 つずつしか持たないので、その
  文書を満たす実機状態が存在しない。`insertFxPairProblems`。比較は文書が保存している値ではなく**書き込みが
  残す状態**で行い、その射影 `insertFxWireState` は `translate.ts` の送出経路に置いてあるので比較と書き込みが
  乖離しない)、Insert FX のスロット衝突は警告して「それでも開く」を提示し、アプリが書き込め
  ない範囲にある保存値は文書が開く前に修復してステータス行で報告する (失敗しておらず、何も問うていないため)。
  修復は窓への丸めか、丸める先が無いものの**削除**: 有限数でない葉 (窓はチャンネルの全タイプで共通だが
  **既定値はタイプ別**なので、どれか 1 つの既定値を書けばそれは選択中タイプの推測になる)、チャンネルの
  メニューに無い `type` (メニューには最も近い要素が無い)、そしてオブジェクトですらない `fxEffect` と
  `params` — サニタイザはこれらを残し、以降の読み手はすべて「無い」ものとして扱う。エフェクト
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  チャンネル」節)。丸めと削除は別々に
  数えて別々の文で伝える。送信できる最も近い値へ動かされた値と、消された値は別の出来事だからである。
  実機がソース無しにしない受け口 — STREAMING — に文書が結線を与えていなければ、同じく既定のソースで補い、
  同じ行で伝える (`requiredSourceProblems`。「STREAMING のソースを記述しないプラン」節)。
  ...
  フィルタは Q を無視する、ゲイン 6 dB 以上のシェルフの公称周波数はプラトーから −3 dB の点である。`eq-response.test.ts` で
  ...
  `scene-scope.test.ts` が 2 つのエンコードを互いにピン留めする) / `input-lock.ts` 全ての面が読む +48V / HI-Z の
  規則 (片方がオンの間にもう片方をオンにする操作は拒否し、HI-Z がオンの間 A.Gain は +40 dB で止まる — known-issues.md
  「本体は +48V と HI-Z を同時にオンにできるが、アプリはしない」)。編集を経ずにプランへ届く 2 つの経路は、
  動かすキーではなく**作り出す状態**をこの規則に問う — `phantomHiZNewlyBothOn` はプランが既に持つ分を
  差し引くので、実機読み出しが両方オンにしたチャンネルはそのまま残る: 履歴エントリの適用
  (`ui/history.ts` の `patchBlocked` フック) と、プラン全体の書き込み (リンクを開く前に止める)。
  実機読み出しの在航中に受け付けた ON は、実機が何を持つかをまだ知らないプランから受け付けたものなので、
  この規則を実機そのものに対してさらに 2 回問う: 読み出しの着地点 (`readRefusedSwitches`。`readIntoPlan` が
  全ての読み出しに適用する — その ON と同じ編集が書いたキーを実機の値へ戻し、`PlanHistory.retract` がその編集を
  アンドゥスタックから取り除く) と、ライブの flush と収束ラウンドが送る各コマンド (`onExcludedBy`。`live.ts` では
  スナップショットと実機の通知 — さらに ON を送る直前に読むもう片方のスイッチ。ブローカーは実機が変化を通知した
  時点からその値を返すので、通知がアプリに届くより先に分かる — 、`client.ts` ではラウンド自身の読み出しに対して)。ライブセッション中の読み出しは
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **チャンネル調整画面が編集する全パラメータ**を対象とし、それらの画面が使うのと同じグリッドへスナップする
    正規化 (0..1) の get/set を持つ (調整画面の値は、スライダーが組まれるのと同じ `DynField` 表から
    グリッドを取る。両者がそのグリッドへどの道で到達し、どこで分かれるかは channel-tuning.md の
    ...
    (受信メッセージは全員へ配る) 一方、ギャングの
    **head** がフィードバック / エコー / ピックアップを所有する (`headOf` = 単なる最初に learn したものではなく、
    **現在のプランで解決できる最初の learned メンバー**。今このノードが持っていない処理系のマッピングが役を
    取って残りを黙らせないため)。ギャングは全メンバーの目標値を
    **メッセージ前の状態から**決めてから書く — あるメンバーの適用が別のメンバーのコントロールを動かしうる
    (リンクペアのミラー) ためである。カタログが「別のコントロールに支配される」と宣言しているものを支配側より
    先に書き (1-knob が値を取り上げる前に書けるようにする)、ロックに拒否されたものは後で再試行する
    (解除される向きは同じ順序のままこれで足りる)。これで決着するのは狭い範囲で、そう書くだけの値打ちがある: **あるメンバーの書込が別のメンバーの
    コントロールを動かすとき、どちらが勝つかを learn 順序が決めることはない**。**ミラー**が等しく保つメンバーは
    複数ではなく **1 つの decision** で、ペアの primary で、かつ**同一アドレス内で**取る — 届いた CC は自身の
    7-bit アドレスと、自身がその片割れである 14-bit ペアの両方に一致する — コントローラ番号 32 未満はその
    ペアの MSB、32〜63 は LSB で、learn がペアを作るのもちょうどこの範囲である。1 つのメッセージが
    2 つのギャングへ届き、
    別アドレスのメンバーがこちらの分を決めてはならない。BAL ではペアの全コントロールが対象で
    (ミラーが partner の nodeParams と全 send を置き換えるため)、PAN が運ぶのは insert FX だけである — 順に書けば 2 つになり、ミラーは最後に
    書かれたものに落ち着くため、開始時点で値が食い違うペア (プランはその状態を持てるし、無警告で読み込める)
    は learn 順序が最後に置いた方の値になってしまう。**何もしないと決めた**メンバーもグループの席を保つ — ギャングは edge と state を意図的に混ぜられるので、
    片方が無視する release をもう片方が代わりに実行してはならない。順序がなお決めるのは**ピックアップ**で、これは head の設計
    であって見落としではない — 交差判定は head が所有するので、値の違うメンバーを束ねたピックアップのギャングは
    head の値で交差する
  - `src/core/control/` — 実機制御 (vd プロトコル)。デスクトップでは書込とライブ同期は常時有効で、
    `selftest.ts` の往復診断だけが `--experimental` 起動を要する
    - `comp-ratio.ts` コンプレッサーの Ratio が止まる段 — チャンネルの COMP と SSMCS ストリップのコンプは
      同じ段を通り、書き方だけが違う。独立したモジュールにしてあるのは、下の 2 層がモジュールスコープで
      この表を読むため (channel-tuning.md「Ratio は実機のつまみが止まる段だけを出す」節) /
      ...
      **単一入力レシーバのソースは、線 1 本ずつではなくレシーバ単位で裁定する** (`sourceChoiceHoldKeys`。
      device-follow の読み・Fetch・ライブ同期開始のいずれも `readIntoPlan` の `hold` へ渡す)。この種の
      レシーバが取る選択は 1 つ — 線 1 本、または USB 出力なら MONO IN ペアの 2 チャンネル
      (`isSingleInput` と `monoPairsInto`) — だが、マージの他の部分は線 1 本ずつ裁定する。置き換えの
      ドロップは、退けた線の削除と自分の線の追加を名指す。線単位では削除だけが操作者のものとして patch から
      外れ、同じレシーバへ向かう読み出し自身のソースを止めるものが残らない: 両方が並んで立ち、emit は
      先頭の 1 本 — 誰も選んでいない方 — を送り、保存した文書はローダーが拒む (`singleInput`)。そこで、
      読み出しが飛んでいる間に編集ファネルがそのレシーバへの線を 1 本でも著したら、読み出しはそのレシーバへ
      **どの線が入るか**について何も書かない — 追加も削除もである。削除だけを採るとモノペアが片側だけ残る
      からである。そのレシーバは操作者が残したままの状態になり、それは `canConnect` を通った状態である。
      読み出しが確かめた状態は `deviceView` に残り、それが次の送信差分の基準なので、セッションは操作者の
      選択を実機へ書く: ライブ同期開始は、その読み出しの間になされた編集 — フラッシュを通すセッションが
      まだ無かったもの — のために自らそのフラッシュを予約し、リコンサイルは `reapplyHeld` を通して同じ
      送り返しに至る。保持したものは、どちらの hold が保持したかで分けて知らせる (`heldByHold`): ソースは
      操作者自身のジェスチャを実機に対して保ったもの、insert FX は実機自身の変更を取り消したもので、件数
      だけではどちらとも判らない。ステータス行はそれぞれを自分の件数で名指し、Fetch のレポートは、残った
      編集の隣に、プランが自分の答えを保った実機の値を並べる。
      ...
      **どちらの epilogue も持たないフラッシュ** — 読みを 1 つも出さない通常の編集 — は、**待たない watch** で
      同じ報告を得る。押さえておくものが無いためで、これが無いと日常のフェーダー・ミュート・パンだけが
      誰にも検査されない書込になってしまう。**1 つのアドレスの未処理の義務は前方へ統合される**:
      実機は**最終的に保持した値**を告知し、その書込が通過した値については何も言わない。したがって 1 つの
      アドレスへの連続した書込 — 告知が返るより高い頻度でフラッシュするドラッグ — には notify が 1 本だけ、
      最後の値を載せて返る。判定されるのは最新の義務で、それが追い越した義務はその 1 本で解消される。
      1 書込につき 1 本を要求すると、ドラッグの最後以外のすべての操作が「どこにも届かなかった書込」として
      報告され、そのたびに全体掃引が張られる。統合が届くのは**未処理**の義務までで、それ以上ではない —
      一度判定された書込の次に同じアドレスへ書けば、その書込は自分の告知を要求する。
      **notify がどれに答えたのかは重複ではなく値で決める**: まったく同じ 1 本が、先の書込が単独で告知され
      後の書込が ACK されて黙って捨てられた場合にも出るからで、そこでは後の書込が本当に失われている。
      よって義務は自分の書込が送った値を持ち、notify は「自分が答えになりうる最新の義務 + それより古い義務」を
      解消し、どれとも一致しない notify は義務をそのまま残す。**名称変更はテキストで判定する。理由は同じ** —
      名前の notify はテキストの隣に数値 0 を載せるので、数値で判定するとどの rename でも同じアドレスの
      別の書込に答えられてしまい、さらにその 0 が数値読みの答えに入る。保留中の書込を渡す呼出側はすべて
      値も一緒に渡す (refetch の settle を含む)。渡さなければその経路は重複だけで判定し、まさに書込が
      捨てられたときに誤る読み方を採る。
      **名称変更も epilogue に関係なく監視する** —
      converge は数値コマンドを読み直し、refetch は名前を飛ばす唯一の呼出側なので、どちらも名前を覆わない。
      落ちた名称変更はプランと名前スナップショットが両方新しい名前へ動いているため、後続のフラッシュに差分が残らない。
    ...
    - `preferred-numbers.ts` 標準数 (ISO 3・Renard) の grade 表 R40 / R20 と、その参照関数。実機の周波数表は
      オクターブ分割ではなくこの系列で刻まれており、下の 2 つのカタログが共にこれを index する — 座席は 1 つ。
      コピーが 2 つあると浮動小数で食い違う (1.0 基点の仮数表は grade 112 の位置で
      112.00000000000001 を返し、クロスオーバー画面は実機が `112` と出す周波数を `112.0 Hz` と印字してしまう)
    ...
    - `link-stats.ts` リンク台帳 (`LiveSync` や `DeviceFollow` と同じく `begin` / `end` の後ろに自分のセッション
      ...
      **要点**である — これが対象とする症状 (Device Center を強制終了する必要がある) はアプリが消えた後に現れるので、
    ...
    - `client.ts` 書込シーケンス + ドライラン + `readClockState` / `setFollowUsb` (書込前の Follow USB (848) +
      ...
      emit 順だけでカバーできるのは送信全体であって部分的な再送ではなく、グループが無ければ EQ 1-knob の ON → TYPE → LEVEL
      連鎖はリンク 1 本につき 1 ラウンドを消費して尽きてしまう — 「リセット連鎖と、収束ラウンドが送るもの」
    ...
    - `live.ts` 編集の即時反映 (スナップショット差分、デバウンス。スナップショットと並べてアドレス→ノードの
      ...
      プランがもう持っていない値 — 実機自身の 1 つ前の値 — を、まだ回されているノブへ送り返してしまう。名前は
...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  **差分は編集時ではなくコミット時に取る** — 複数のファネルが `markChanged()` を呼んだ後にさらにプランを
  ...
  開始、同様にデバイス追従の 2 つの照合とライブ同期の 1-knob 再取得、そして flush が取る silent アドレスの
  park **すべて** — 先頭書込の手前のものと収束の手前のもので、それぞれ独立した条件で走るので 1 回の flush が
  持つのは 0 個・1 個・2 個のいずれか。系統が重なるのでフラグではなく実行中集合への所属で追跡する。収束の
  **ラウンド**はこれに含まれない — 書込スコープ全体を読むがプランへは何も書き戻さないためで、したがって収束を
  伴う flush のうち押下を拒否するのは park だけである)、ファイルフローが保持している間、***ドラッグ*が進行中の間 (押下して動いたもの。
  ...
  拒否は peek したエントリに対して `take()` の前に走り、`deactivateLive` は履歴をリセットしない)、
  そしてパッチが残す**結果**をホストが拒否する場合 (`patchBlocked` — 現在は +48V / HI-Z の排他。
  パッチが運ぶキーではなく、それが残す状態を問う。`peekUndo` が答えるのは適用されるパッチ = エントリの逆である)。テキスト
  ...
  三択モーダル (実機のレートで書く / Follow USB を切ってプランのレートで書く / 取り消し) / `error-box.ts`
  ダイアログを上げられなかったエラーの行き先 (シェルのダイアログコマンド自体が reject しうる。status 行では
  受け止められない — 一時的な 1 枠で、走行の結果を載せており、次にアプリが報告するものに奪われる)。
  メッセージは**キュー**する (1 つ上げられなかったシェルが次を上げられる見込みは薄い)。決定ゲートより
  **上の専用の段**に置く: エラーは他の面が開いている最中に届くので、同じ段では隠れる (style.css の重なり順)
  / `licenses.ts`
  ...
  **後**に設置する。これがコードログに「アプリがそのコードを取ったか」を報告させている / `dom.ts` 共有 DOM
  ...
  解放と再取得を指示する。`dyn-gate.ts` / `dyn-comp.ts` / `dyn-eq.ts` / `dyn-ducker.ts` / `dyn-ssmcs.ts` / `insert-fx-screen.ts` /
  `fx-effect-screen.ts` が
  記述子 — DUCKER だけは
  開いたノードを調整しない。Ducker はステレオチャンネルの下にぶら下がる別ノードで、キーはよそから来た
  ワイヤで決まるため、レーンは開いたノードから読むのではなく 3 か所から集める。`dyn-ssmcs.ts` だけは 1 つの
  プロセッサではなく 1 つのバンクの 3 面で、表示の上のバーから閉じずに行き来する。`insert-fx-screen.ts` だけは
  1 つのエフェクトではなく複数を代表する: 表示するのはノードが**保持している**インサートエフェクトで、これは
  別の面が変更するプランの値であるため、呼び出しごとに family を解決し、デバイス追従では画面を閉じて開き直す
  のではなく同じモーダルを再束縛する。EFFECT TYPE の行は意図的に持たない — セレクタ書込は束縛されたエンジン配列を
  そのタイプの既定値で埋め直し、可逆ではない (`core/control/insert-fx-effect.ts`) — また field はパラメータでは
  なくエンジン**スロット**を名前に持つ。これらの値は family とスロットをキーに `insertFxParams` へ入るからである。
  アプリが編集する family はすべてこの画面が表示する — インスペクタ側では何も編集しないので、1 つの値を 2 つの
  面で書けることがない — そしてフラットなカタログが持たない状態を持つ family は、カタログの行の隣に自前の行を
  供給する: Pitch Fix は Key・Scale・12 音マスクを、マルチバンドコンプレッサは読むだけで決して書かない
  1-knob の 2 行を。`fx-effect-screen.ts` は同じ形の 2 つ目で、FX チャンネルのエフェクトが取りうる 3 つのパラメータ系統
  (FX1 の Rev-X、FX2 の Rev.R3、両チャンネル共通のディレイ 2 種) を代表し、プランが持つ EFFECT TYPE から
  解決する。EFFECT TYPE の行を持たない理由は INS FX と同じ。field はパラメータでもエンジンスロットでもなく
  **プランキー**を名前に持つ: 2 つのディレイタイプは同じ family・同じスロットで、違うのは受け取るレンジだけなので、
  family + スロットの綴りでは 1 つのキーが 2 つのパラメータを名乗ってしまう。一方カタログのキーは、系統が衝突する
  ところでは既に系統名を持ち、本当に 1 つのパラメータであるところでは共有されている。表示列はレベルラックだけ:
  リバーブの残響もディレイの反復列も値からは定義できず (アルゴリズムは実機のもので、Hi Ratio が効き始める周波数は
  そもそもパラメータに無い)、曲線を描けばそれは創作になる。2 本のレーンはエフェクト自身の入力と出力 —
  到着するモノラルの FX センド和と、エフェクト直後・フェーダー前のステレオタップ — なので、共有の Input / Output
  の caption がこの画面では文字どおり正しい。チャンネルのフェーダー後のレベルは CONSOLE のストリップのもので、
  フェーダーはこの画面のものではない。
  `dyn-chan.ts` が MONO IN
  チャンネルストリッププロセッサが共有する束縛、`dyn-plot.ts` がそれらが描く dB×dB の伝達プロット、
  `dyn-freq-plot.ts` が 2 つの EQ が描く周波数×ゲインのプロット、
  `dyn-registry.ts` がどのプロセッサが存在するかを知る唯一の場所である。応答を描く画面はプロットとレーンのラックを
  同時に表示し (しきい値は入力メーター上のフェーダーキャップとしてドラッグし、2 つが 1 つの座標を共有する)、
  バンドの選択はバーではなく応答上のマーカーを押して行う。INS FX が応答を描くのは、応答がパラメータから
  定義される場合だけである — コンパンダーの伝達曲線と、マルチバンドコンプレッサのバンドごとの曲線 (その
  MAIN 面は SSMCS ストリップのサイドチェーン面と同様に、その軸を周波数×バンドのメイクアップへ差し替える)。
  ギターアンプの周波数応答もピッチトラッカーもパラメータから導出できず、実機もどちらも計測していないため、
  それらの面は表示列がレーンのラックだけになる。`DynPlotProcessor` は 1 つでも供給する記述子には
  `plotGeo` / `drawAxes` / `drawCurve` を 3 つとも必須にし、その列にプロットを置くかどうかは記述子自身の
  `display` が family ごとに決める。
  EQ のフィルタモデルは `core/eq-response.ts` (実測: 実機の Q はバイクアッドの Q の 2 倍、パスフィルタは Q を
  無視する、ゲイン 6 dB 以上のシェルフの公称周波数はプラトーから −3 dB の点である)。**どのプロットも真の値でカーブを描き、
  ...
  *注釈*の方は、読めるように意図的にクランプしてよい)。`docs/{en,ja}/channel-tuning.md` を参照) /
  ...
  `prefs.ts` 環境設定モーダル (ツールバーの歯車、全ビルド: 言語 + テーマ (ツールバーには置かない。保存キー
  ...
  **`--experimental` の下でのみ**表示する。バーは `LINK_BAR_KEYS satisfies readonly LinkLedgerKey[]` で
  部分集合に縛られており、パネルのラベルと唯一の `ledgerValue` を印字する — バーが自前の短い語彙を持つと、
  バーの `cmd` とパネルの `Sent` が同じ数であることを誰も言わなくなる — カウンタとログ自体はすべての
...
- `src/app/` — アプリのエントリのうち配線ではない部分。`main.ts` から切り出し、アプリ全体を起動せずに
  ...
  終わるからである。したがってパネルは再描画と再描画の間はプランの**スナップショット**であり、書き込む値が
  別の値に依存するコントロール — 同じ編集で A.Gain を上限まで下げる Hi-Z スイッチ — は、その値を
  押された時点のプランから読む。スナップショットが持っているのは、それ以降のスライド前の
  ゲインだからである / `flow-latch.ts` は 2 つの再入ガードとその違い。`singleFlight` は 1 つのハンドラに対する
  ...
  **報告**し (操作者のクリックが未応答に終わった)、2 つ目のファイルフローには沈黙する
  ...
  `param-source.ts` はプランの各パラメーター値の出所を保持する — 補完は走りながら `load` / `default` を、
  編集は `manual` を、確定した読み出しと着地した書込はそれぞれが届いた範囲に `device` を記録する。
  一時状態でありシリアライズしない: ドキュメントが持つのは状態であって操作の記録ではない /
  `unauthored-writes.ts` は書込のうち操作者が選んでいない変更を判定する。プランは密である
  (ローダーが機種の工場出荷値でドキュメントを補完する) ため、書込には誰も設定していないキーが乗る。
  該当キーを空白化したプランを再エミットし、消えたアドレスの所属ノードをストリップとして名指す —
  そのエミットに残るアドレスは、操作者が設定したキーが要求しているアドレスである。`load` は操作者が
  設定した側 (ドキュメントがその値を書いている) で、`default` と `device` はそうではない。ルーティングの
  セレクタを名指すのは、アプリが補った結線 — 読み込み、または実機が NONE だったときの Fetch / ライブ同期開始 —
  のときだけである (「STREAMING のソースを記述しないプラン」節)
...
- `src-tauri/` — Rust シェル。webview のホスト + tauri-plugin-dialog + ファイル IO コマンド
  ...
  なく現在を報告する。`append_link_log` はアプリのログディレクトリへ JSONL を 1 行追記し、`append_midi_log` は
  その隣の MIDI トレースへ複数行をまとめて追記する (dev バイナリ限定・それ以外では拒否する)。`--experimental`
  ...
  `vd::shutdown_blocking` を呼べる。これはそれらのフレームが実際に線に乗るのを有界に**待つ**。放置された
  セッションが Device Center を強制終了させる原因なのかは**確定していない**。終了時のテアダウンはそれを候補から外すという、
...
- **DeviceModel** — 機種ごとの不変な装置定義。`nodes` (入力/チャンネル/Bus/出力/Ducker)、
  `rules` (接続可能な経路 = `RoutingRule[]`)、`channelPairs` (MONO IN ペア = CH1/2・CH3/4、URX22 は
  CH1/2 のみ。入力ソースを共有するチャンネルの組であり、USB 出力が 2 本の結線として受けるペアでもある)、
  `requiredSources` (常にちょうど 1 つのソースを持つ受け口と、ソースを記述しないプランに与えるソースの組 —
  STREAMING と STEREO。「STREAMING のソースを記述しないプラン」節) を持つ。`models/build.ts` が機種パラメータから生成する。
...
- **Plan** — ユーザーが作成する可変状態。`modelId`、ノード配置 (`positions`)、結線 (`connections`)、
  ...
  ただし INS FX 値だけは例外で、採取待ちの推測ではなく採用値)。
...
- `canConnect(model, plan, fromRef, toRef)` — 規則の有無と受け口多重度 (`source`/`patch`/`key`/`record` は1本、`send`/`sendSwitch` は複数) を判定。single-input ポートの占有は既存結線の種別を問わず数えるため、壊れた/異種 kind を持つ手編集ファイルでも単一入力の不変条件をすり抜けられない。**唯一の例外は USB 出力**で、MONO IN ペアの片チャンネルを受けているときは相方の結線を 2 本目として受け、それ以外の 2 本目と 3 本目は `singleInput` ではなく `monoPairOnly` で拒否する (アナログ出力・チャンネル入力・Ducker キー・SD Rec トラックは `singleInput` のまま)。`monoPairsInto(model, toRef)` は受け口が受けるペアを返し (両チャンネルがその受け口への `patch` 規則を持つ `channelPairs` の組すべて。これを持つのは USB 出力だけ)、`monoPairOf(model, toRef, fromRefs)` は結線の組が成すペアを並び順に関わらず primary を先にして返す (成さなければ null)。`validatePlan` は読み込んだプランを同じ組で検査する。デバイスへの変換と読み戻しも `monoPairOf` でペアを読む: ペアについて何を書き何を読むかは [device-model.md](device-model.md) §6。
- `partnerChannel(model, nodeId)` — MONO IN ペアのもう一方のチャンネルを返す。`source` 結線時に同一ソースを相方へミラーし、削除時も連動させる (UI: `graph.ts`)。USB 出力のペアはこのミラーを**通らない**: 2 本は互いに独立した `patch` 結線で、削除も 1 本ずつであり、両方を出力へ入れる操作はリストの下で述べるリンク済みペアの描画である。Ducker のキーソースは `source` ではなく `key` 種別なのでこのミラーリングを通らない (モノペア非所属という偶然ではなく型で保証)。
- `isStereoLinkedPair(model, plan, id)` / `mirrorLinkedPair(model, plan, id)` — MONO IN ペアの Signal Type が STEREO のとき、片 ch の編集を相方へミラーする (ヘッドアンプを除くノードパラメーター + 各 Send の LEVEL/PRE-POST/ON。ゲイン・Clip Safe・位相反転・+48V・Hi-Z は各メンバー自身の値。Signal Type/PAN-BAL フラグは primary のみ保持)。どれを共有するかは `pairSharesNodeKey` が 1 箇所で答え、ミラー本体・MIDI のミラー identity・各ファネルの書き込み証跡がそれを読む。コピーが相方に残したキーを証跡が申告することはない。**判定は PAN/BAL モードではなく Signal Type** で、これは実機の挙動に合わせたものである (実測値は [device-model.md](device-model.md))。モードが決めるのはその上の 1 パラメーター、pan だけ: `isBalLinkedPair` が pan を判定し、BAL ではペア共有のバランス 1 値、PAN では各 ch 自身の値となる。BALANCE 表示が問うているのも同じ問いである。各編集 funnel から呼ぶ: グラフ/インスペクタは `main.ts` の `onUpdateParams`/`onUpdateNodeParams`、CONSOLE は `console.ts` の `commit`。GRAPH/CONSOLE で同一関数を共有し挙動を一致させる。
- **チャンネル調整画面はこのペアを 1 チャンネルとしてメーターする**。判定は PAN/BAL ではなく Signal Type だけで、全レベルレーンが両メンバーのアドレスを持ち、リダクションレーンは 1 本のまま。規則と根拠、および唯一届かない画面 (DUCKER の KEY レーン) は [channel-tuning.md](channel-tuning.md)「Signal Type = STEREO の MONO IN ペア」。
- **ペアの insert FX を解除するのは Signal Type の遷移だけである**。実機実測: この遷移は**どちらの向きでも**、どちらのメンバーが保持していたかに関わらず**両メンバー**のセレクタと ON を解除する。エンジン配列の値は残る (再シードはセレクタ書込のみ)。一方でリンク**中**はセレクタが双方向にミラーし、両メンバーが 1 つのエンジン instance を指す — つまりリンク済みペアは insert エフェクトを 1 つ共同保持する (各 1 つではない)。このミラーは **PAN/BAL に依存しない**: 両モードで実測しており、PAN⇄BAL の切替だけではエフェクトは消えない。したがって insert FX はノードのミラーとは別に自前のミラー (`mirrorLinkedInsertFx`) を 1 パス持つ: 2 つは `stereoLink` という同じ判定を共有し同じ値を書くが、別パスであることが、編集が何に触れたかに関わらずペアの 3 キーをその編集自身の書き込みとして申告する役を担う。`applyPairTransition` は遷移時に両メンバーの `insertFx` / `insertFxOn` / `insertFxParams` を消し、ミラーはリンク中であれば常にこれらを運び、1 枠のセンサス (`insertFxCensus`) はリンク済みペアを 1 保持者として数える — 規則の写しを 2 つ持たず、実機がすることに追従する ([アプリがモデル化するもの・実機に任せるもの](#アプリがモデル化するもの実機に任せるもの))。この遷移は**書いたキーを全て**編集ファネルの write witness に申告する。どれも値が動くとは限らないためである: エフェクトを持たないメンバーへの 3 つの削除は何も変えず、BAL ペアのリンク解除は既にセンターにある pan をセンターへ書く。実行中のデバイス読み出しはまさにその理由で値ではなく著作で調停しており、何も申告しない操作は両方を巻き戻される — 実機が落としたばかりのエフェクトが選び直され、センターにしたばかりの pan が読み出し時点の値へ戻る。**リンク済みペアが取れるのは Compander だけである**。ギターアンプと Pitch Fix はモノチャンネル専用のエフェクトで (ユーザーガイドのエフェクトリストに基づく `INSERT_FX_OPTIONS` の `monoOnly`)、`insertFxMenu` はリンク中、両メンバーでこの 5 つを `"link"` としてロックする。3 種類のロックのうち 2 番目で、判定順も 2 番目 — レートの後である: 全上限を超えたレートではリンク済みペアも全エントリでレートを報告しなければならず、さもないと `insertFxAllRateLocked` がそのペアに答えられなくなり、レート警告と強制 OFF のバイパスも一緒に失われる。他の 2 つと同じく UI 限定だが、ここではそれが取り決めではなく実測である: 実機はリンク済みペアへの mono-only セレクタを**受理**し、両メンバーへミラーしてエンジンを束縛する — ペアのミラー自体がそれで確定した。実機自身のパネルはその項目を出さないので、この状態に持ち込めるのはソフトウェアだけであり、そうした値を持つプラン (ファイル・`?plan=` リンク・旧版が書いた実機の読み出し) は送出経路で黙って書き換えず従来どおり送る。実機の DSP がそれをどう扱うかは未測定。
- STEREO リンク済みペアは canvas 上で♥コネクタで結ばれ、1 ユニットとしてドラッグできる。リンク時 (`stereoLink` が ON になった `onUpdateNodeParams` から呼ぶ `alignStereoPair`) はまず相方を保持ノードの隣へスナップする — 選択中のメンバーは固定し、もう一方を既定レイアウトの相対オフセットへ移動する — ため、リンク前の手動移動で開いた隙間に♥タイが伸びることがない。リンク後もペアは独立した保存位置を保つ (ducker の親由来位置と異なる) ので、自由にドラッグできる。アプリが作ったのではないリンクには `alignLinkedPairs` が同じスナップを与える。これは編集ではなく plan からペアを読む: 読み込んだドキュメントと plan 全体のデバイス読み出しはどちらも `Graph.setModel` を通り、デバイス側の 3 つの読み出し — 実機パネルで動かされた Signal Type の scoped reconcile、シーンリコールやアイドル掃引が取る全体 reconcile、そしてペア primary の本体を読み直すことで Signal Type を一緒に持ち込む side-effect refetch — はいずれも `snapLinkedPairs` を通し、そのリンクを持ち込んだ読み出しの隣で、その読み出し自身の trace サンプルより前に呼ぶ。レイアウト上の帰結を、次にサンプルする書き手ではなく、それが乗っているファネルに帰属させるためである。refetch はさらに全体 reflect を要求する — 細粒度の側は読み出しが名指ししたノードしか塗らず、♥タイを描かないためである。要求の条件はスナップがノードを動かしたことに加えて**リンク状態が動いたこと** (`changesLinkState`) でもある: 既に隣接しているペアはスナップするものが無く、そのままでは次に何かが再描画するまでタイが出ない。ここでは選択中のメンバーではなく **primary** を保持するので、盤面の選択状態に関わらず同じ 2 スロットに落ち着く。どちらかのメンバーが棚上げ (shelve) されているペアは対象外とする — ♥タイが描かれないため埋めるべき隙間が無く、見えているメンバーを棚上げされた相方に合わせて動かすと、画面に無い相方のために位置を変えることになる。そのペアが再び揃うのは棚 (shelf) 側である。チップから復帰したメンバーは、ビューポート中央ではなく既に盤面にいる相方からの既定オフセットへ着地し (`placeRestored`)、盤面にいた側は動かない — 両方が揃った時点で♥タイが描かれるので、動くのは戻ってきた方である。*Show all* もペア単位で同じ規則に従う: 戻ってくるのが片方だけならそれが動き、両方が棚上げされていたペア (どちらも「既に盤面にいる」側ではない) だけが primary 保持の一括整列に落ちる。盤面に相方がいないノードは従来どおりビューポート中央に置く (見つけやすいため)。この配置は相方の居場所に従うので、操作者がパンした表示領域の外になりうる — その間もチップのステータス行は「表示した」と言い、インスペクターはそのノードを選択する — ため、単体復帰では表示領域へ入る最小限のパンを行う (`panIntoView`。ズームは触らない)。入れる対象は**チップが名指ししたノード**とする — hung ノードが位置を借りている親ではない。単体で戻したトラックスロットこそチップ・ステータス・インスペクターが「戻した」と言っている当のもので、上にあるレコーダーを収めてもレコーダーが見えるだけである。相方 (あるいはその親) を一緒に収めるのは**見えている領域**に両方が収まるときだけである: 棚は canvas の下部を覆っており、復帰時は必ず開いている。残りの高さに収まらないペア (小さいウィンドウ・深いズーム・Arrange が丸ごとの行を確保した展開済みノート) を上端基準で収めると、相方が見えてチップが名指ししたノードが棚の裏に残る。そこでは操作者が求めたノードを優先する。書き込む位置は既定レイアウトのオフセットだが、「既にスロットにいる」かどうかは**帯**で判定する (`inPairSlot`): primary と同じ列、レイアウトが置く側、そのオフセット以上・上側ノードが占める行数以下。Arrange は展開されたノートのために丸ごとの行を確保しており、帯はその計算 (`rowsFor`) を共有するので、読み込みがそれを潰すことはない。一方で広げたオフセットを*書く*のは答えにならない — Arrange は列全体を再パックするのに対しこちらは 2 ノードしか動かさないので、広げた先の行にいるものの上に乗るためである。帯の下限にはサブピクセルの許容幅を持たせる: 操作者がドラッグしたペアの保存オフセットは既定値を別の基点で足し直したもので、無視できない頻度で ~1e-13 px ずれ、デバイス追従経路では誰にも見えない書き込みでも編集ファネルの外側の plan 書き込みになるためである。

UI (`graph.ts`) はこれらを使い、出力・入力どちらのポートからもドラッグで結線できる。反対側のポートは2層でハイライトする (結線可能 = 塗り、規則はあるが占有済み = 輪郭のみ)。出力からのドラッグは possible があれば開始し、入力からは legal があるときのみ開始する。**STEREO リンク済みペアのチャンネルを USB 出力へ描画すると、2 本の結線を 1 回の変更で描く** (どちらのメンバーからでも、どちらの向きのドラッグでも同じ)。そのチャンネル単独を受けている出力へ描画すると、欠けている相方を足す。リンクしていないチャンネルは単独で入り、それを受けている USB 出力は相方にとって結線可能な先のまま残るので、相方は自身のドラッグで加わる。2 本の結線は 1 本ずつ削除する。既にソースを持つ single-input ポートのクリックは、その結線を選択する (配線クリックと同等)。ペアを受けている USB 出力では primary の結線を選択する。**STREAMING へ別のソースを描画すると、持っている結線を置き換える** — 1 回の変更なのでアンドゥも 1 回分。本体の STREAMING のソースリストには None が無いため、盤面は STREAMING を空にしない (`DeviceModel.requiredSources`、判定は `requiresSource`)。このドロップは結線可能として点灯し、ドラッグ途中の再描画も同じ集合で点け直す (`connectCandidates` がドラッグ開始と再描画の両方を受け持つ)。そのため STREAMING 自身の入力からもドラッグを開始できる。そこをクリックしたときは従来どおり結線を選択する。最後の結線は残す: 盤面の Delete キーは `deleteConnection` に届き、そこでステータス行のメッセージとともに拒否する。インスペクタはその結線に削除ボタンを出さず、代わりにその拒否と同じ文言のヒントを出す (`isLastRequiredSource` が両者の問う唯一の述語である)。
...
- `en.ts` — 基準言語であり、メッセージ構造 (`Messages` 型) の一次情報。**文字列はすべて 3 つの印の
  ...
  `fixed()` は実機以外の理由で全言語同一にするもの (すべて CONSOLE のストリップ上 — 区切りラベルと
  読み値キャプション 2 つ)、`tr()` は
...
> という状態を作らない。型で固定されるのはラベルの側だけである — `dev()` / `fixed()` / `tr()` の印が
> 強制するのは文字列の**同一性**であって、訳文がどの語を使うかには何も効かないため、文とその上のラベルの
> 綴りを揃えるのはこの規則だけである。同じ理由でこの規則は日本語ドキュメントにも及ぶ: そこも同じラベルを引用する。
...
> その各行が、Effect Reference Guide を裁定者にできない理由である: ガイドは文書であり `dev()` が名指すのは
> **画面**の印字で、いくつかの行では両者が食い違う。
> ギターキャビネットは `Gate` (ガイドは `Off/Gate`)、マルチバンドコンプは `M.B.Comp`
> (ガイドは `M.B.COMP`)、クロスオーバーは `L-M Xover` / `M-H Xover` (ガイドは `L-M XOVER`)、バンド名は
> `Low` / `Mid` / `High` (ガイドは `LOW` / `MID` / `HIGH`)。Crunch と Lead の音色スイッチにガイドは名前を
> 与えておらず 2 つの値を並べているだけだが、本体は `Type` と印字する。もう 1 つは文書をいくら読んでも
> 決まらないもので — **Clean の変調セレクタには本体にラベルが無い**。だからアプリが独自に付け
> (`Modulation`)、`dev()` ではなく `fixed()` にしてある: 写すべき実機の文字列が存在しない。
> 本体が名前を付けている INS FX の行はいずれも先頭のみ大文字で、全大文字ではない — ギターパネルのカード
> ラベルはこれに従い、上に挙げたガイドの全大文字はこれに従っていない。
>
> **FX EFFECT の各面は 2026-09-23 に同じ方法で読んだ** — Rev.R3・REV-X・2 つのディレイを 1 面ずつ、
> 各ページのキャプションを並んでいる順に読む。ここでもガイドは裁定者にならない: ガイドは
> `ER/Reverb Delay` と書き、面は `E/R Delay` と印字する。綴りは marker が捕まえられないもう 1 つの
> ものである — `dev()` が固定するのは文字列の**同一性**なので、正しいコントロールを写した行が誤った
> 綴りを持っていても、正しい行とまったく同じようにコンパイルされ、同じように描画される。疑問が出た
> 行だけを読むのではなく面ごとにキャプションを並びごと読むのは、このためである。
>
> **アプリが自分で名付けた面バーのセグメントは `dev()` ではなく `fixed()`** である —
> マルチバンドコンプの `Main` と SSMCS バンクの `Main` / `Comp` / `EQ`。この 2 本のバーが印字する
> 残りはアプリのものではない: `Low` / `Mid` / `High` は実機のバンド名で、同じカタログ項目が行の
> ラベル (`Low Gain`) にも使われる。`Side Chain` は実機の SSMCS 画面の行である。したがって `Low`・
> `Mid`・`High`・`Side Chain` は `dev()` のまま。marker 自身にはこの 2 種を見分けられない — 型で
> 翻訳を止めるだけで、宣言が真かどうかは何も言わない。
>
> したがってこの 7 画面は全体が翻訳対象外になる — `Range` / `Ratio` を含む
...
> 各ストリップ下の読み値キャプション (`FADER` / `METER`) はそれに追従する — ストリップ上で 1 語だけ
> 和文になるのを避けるためで、例外はここまで。同じメーターを指す調整画面の見出しは訳す
> (「メーター」。ユーザーガイドの "Channel meter" /「チャンネルメーター」に拠る)。
...
むしろ識別性を損なう — 種別ごとに色を割いたパレットを 2 型色覚でシミュレーションした実測で、`record` と
`source` は OKLab ΔE で 1.6 (×100)、すなわち同一色だった。`WIRE_GROUP` は `Record<ConnectionKind, WireGroup>`
なので、種別を増やすと配置するまでコンパイルが通らない。読み出し側にフォールバックは無い —
`?? "#888"` のようなフォールバックは、書き漏らした種別をハロー無しの灰色で描き、何も告げない。
...
> **CONSOLE は意図的にこの集約点に入れていない。そしてそれは前提の上に成り立っている。**
> スクリブルのインクはスタイルシートの値ではない — `inkOn()` が地から黒か白かを選び、
...
**macOS のタイトルバーはこのアプリではなくシステムが描画し、どちらのタイトルバーになるかはバイナリ内の
値が決める。** macOS 26 は `LC_BUILD_VERSION` に記録された macOS SDK バージョンを読む — 26 以降なら
下のページに溶け込むタイトルバーになりテーマに追従し、それより古ければシステムの不透明なタイトルバーに
なり追従しない。リンカはこのフィールドをビルド時の SDK から埋めるため、放置すると製品ではなくビルド機の
性質になる — Xcode 26 の機械での `pnpm tauri dev` と、それより古いランナーイメージでパッケージした
リリースが食い違い、食い違ってはいけないと言うものがリポジトリに何も無い状態になる。代わりに
`src-tauri/.cargo/config.toml` がこの値を宣言し、cargo が両方に適用する (「ビルドと配布」を参照)。
...
敷く。色だけで意味を伝えている要素はすべて意味を失う — ルールブロック無しの実測では、既定盤面の
点灯チップ 51 個と消灯チップ 73 個が**すべて同じ色**に解決し、四角が並ぶだけで点灯と消灯の区別が付かない。
...
- **輪郭。** 押下状態は `3px double CanvasText` — システムパレットの下でも 3 行の画素が分かれたままであることを
  実測した太さ (double の他の太さは測っていない)。
...
- **ある部品が別の部品の文法を自分の選択子で持ち直している**ため、片方を名指した規則はもう片方に
  ...
  あって選択子ではない。`.con-fader` しか名指さないブロックでは、ミニフェーダーが 0 dB 線と
  キャップの刻線を黙って失う。**フェーダーから離れた面でも同じことが起きる**: 調整画面のメーターレーンは
  CONSOLE のメーターと同じ文法、パラメーターのスライダーのトラックはフェーダーの溝と同じ文法で、
  ブロックが名指し漏らした部品は実機のコントラスト テーマを入れるまで見えない。
- **状態側の規則がブロックを特異度で上回りうる。** メディアクエリは特異度を足さないので、
  `.con-scol.off .con-vfad .cap::after` (クラス 4 つ) が `.con-vfad .cap::after` (2 つ) に勝ち、
  ブロック側に規則が無ければ OFF にした送り列だけ塗りが残る。これらの部品を塗り直す状態は、すべてブロック側で名指す。
...
(2026-08-08、hcblack と hcwhite)、両テーマで成立している。
...
このスペックの 1 件は computed style に加えて描画された画素も読む。range 入力のトラックとつまみは
`::-webkit-` の擬似要素で、このエンジンは著者の宣言を `getComputedStyle` で返さない — トラックを
描いている規則が、画面に出ている状態で `0px none` に解決する。DevTools プロトコルは届く。入力の
ユーザーエージェント shadow ツリーが 2 つを `#track` と `#thumb` として持ち、それらへの
`CSS.getComputedStyleForNode` が forced-colors の値を返す (2026-09-24、Chromium 153: トラックは
`1px solid`、つまみは不透明な Canvas の塗り)。このケースは 2 つの宣言をそこで assert し、どちらの
assert も対応する規則を外すと落ちる。
画素から読むのは、どの宣言も述べていないこと — 描画されたつまみが描画されたトラックを覆っていること。
...
`-50%` の translate が掛かるため)。両者を**溝**の inset 幅 (`height - 12`) を使う 1 つの絶対
マッピングにすると、押下位置とつまみが一致するのは可動域中央だけになる。既定ウィンドウサイズではつまみの
端を押しただけで最大 1.7 detent (最小サイズでは 3.6 detent) ずれ、ライブ同期中はそれが操作者が動かす前に実機まで届く。
**調整画面のしきい値キャップも同じ分岐を持つ** (`dyn-screen.ts` — キャップ自身のリスナが
掴み、スロット側は `e.target === cap` でキャップへ譲ってからジャンプする) ので、2 つのサーフェスは同じ押下の文法を共有する。
...
押している間ステップを細分化する (実機の非公開「押し込み+回転」微調整の対応操作): 調整画面の
...
よるレイアウトシフトも構造的に起こらない (調整画面の行は静的なラベルの右隣に固定 — 可変幅の値 readout 側に
...
- **メーターポイント (ストリップ毎のタップ)** — ノードは信号経路上に複数の観測可能なメータータップ点を持ち、
  ...
  FX チャンネルは INPUT → PRE FADER → POST (INPUT はエフェクトへ入るモノのセンド和・PRE FADER はエフェクト
  自身のステレオ出力)。モニターと OSC は単一メーターでセレクタ無し。STREAMING と OSCILLATOR は
...
- **再描画は DOM が保持していた状態を引き継ぐ** — 全体再描画 (`render()`) は全ストリップの要素を差し替える。
  一部の編集 (電源 LED のストリップ単位の減光・リンク連動の相方) と device follow の読み戻しは今も全体再描画を
  通るため、要素が抱えていた一時状態は破棄せず再構築をまたいで引き継ぐ: 同じタップを表示し続けるストリップは
  メーターの弾道を新しいレーンへ移し (`carryMeterState`・`refreshStrip` と共有)、キーボードフォーカスを同じ
  コントロールへ戻す (`captureFocus`・ストリップ id + 索引 + クラスで照合し、再構築でそのストリップの
  形が変わっていた場合と、プラン自体が差し替わった場合 (ファイルの読み込み・機種切替) は復元しない。後者では
  フォーカスのあったコントロールは無くなったプランのもので、押したままのキーが差し替え後のプランのどこにも
  届いてはならないため)。ストリップ領域のスクロール位置は**意図的に保存/復元しない** — クリアと
...
- **レイアウト/スクロール** — `#console-host` は `min-width:0; overflow:hidden` で `#stage` 内に収め、横スクロールを
  ...
  ウィンドウが極端に低いときだけストリップ領域内で縦スクロールする。マスター (STEREO) は右端に固定せず、
...
- **実機側操作の追従 (device follow)** — ライブ同期の逆方向。同じ排出経路で**実機側のパラメーター変更**も
  ...
  `Subs` に保持する。「購読差し替え時に旧集合を解除する」用途にも使う同じ集合)。フィルタが無いと他クライアントの
...
同じ実測が、ゲートのバックストップの値打ちを示す。合成中の要素を奪った 349 回の再描画のあいだ、
...
チャンネル調整画面が編集する全パラメータを外部 MIDI コントローラーから操作できる (デスクトップ版のみ)。
画面のどの行がどのスコープで提供されるかは channel-tuning.md の「MIDI 割り当て」が持つ。
...
- **マッピング (core/midi/)** — 言語非依存の純ロジック。`message.ts` が CC / ノート / ピッチベンドを
  ...
  カーソルではなくスコープにしてあるのは、**マッピングは調整画面を閉じていても効かねばならない**ため (画面側の
  バンド選択 — レスポンス上のマーカー — は開くたび LOW に戻る)。2 種は意味が違うので割り当てリストでの表記も違う: `CH 1 → MIX 1 · Level`
  ...
  片方がオンの間にもう片方をオンにする書込はコントロール自身が拒否し (`refuses`)、捨てずにステータス行へ
  出す。この規則がロックした CONSOLE のチップは学習対象のまま残る (known-issues.md「本体は +48V と HI-Z を
  同時にオンにできるが、アプリはしない」)。
...
- **フィードバック (MIDI OUT)** — 盤面変更 (UI 編集・実機 follow・実機 readback・plan 読込) を割当の逆引きで
  ...
  (`planReadFromDevice`: 取得・Live sync 開始時の初期 readback・`.urxf` 取り込み。follow 反映はそのうち
  フィードバックを受け持つ半分の `planValuesChanged` だけを呼ぶ) にぶら下がり、120 ms デバウンス +
  送信済みキャッシュ差分で変化した値だけを送る。受信中のアドレスへは 300 ms の静寂まで送出を保留してスイープと
  衝突させない (エコー抑制)。

  **ライブ同期の読み戻しが完了するまで何も送出しない**。そしてその瞬間が唯一の全送出でもある — 差分を介さず
  全割当の現在値を 1 度送る。実機が「変化なし」と答えた値こそ、キャッシュを埋めた後に挿し直された / 別バンクへ
  移ったコントローラが誤って表示し続けるものだからである。それ以前に送れる値は実機の状態ではなく、その時点で
  プランが持っている値 — 新規ドキュメントの初期値・読み込んだファイル・部分適用で終わった読み出し — であり、
  それを線に出すことはコントローラの表示が誤るだけの問題ではない。ループバックや共有バスでは、他の受信者すべてが
  それを操作者のジェスチャーとして受け取る。**2026-08-16 の実測**: ライブ同期中に**このアプリの 2 つ目の
  インスタンス**が起動し、自分の起動時の値を同じ IAC バスへ送出した結果、ライブ側がそれを受信 MIDI として適用し
  実機へ書き込んだ (CH 1 のゲインが +66 dB から最小へ、チャンネルはミュート解除)。よって出力ポートを開いた
  だけでは何も送らず、取得と `.urxf` 取り込みも開かない (どちらも「プランが実機の状態である」とは言えない)、
  例外や中断で終わった読み戻しも開かない — 全送出は 3 者が共通で通る `finally` に到達したことではなく、
  セッションが実際に立ち上がったことにぶら下がる — そしてライブ同期の終了とプランの差し替えで再び閉じる。
  **取得を除外する代償**は、モーター付きコントローラが直前に伝えられた値を表示したまま残ることである。
  セッションが出力側を開く前にそのフェーダーに触れると、取得した直後のプランへ古い位置が適用される。
  マッピング単位の答えは pickup モードで、代わりに取得でも開くという選択はこの規則が拒否している当のもの
  である (読み出した瞬間に実機と一致していたプランは、その後も実機に保たれているプランではない)。
  **保留されたパス自体は走る**: 受信側への責務 (プラン値が動いたら pickup の係合を外す) は、
  コントローラがそれを聞いたかどうかに依存しない。
  ...
  **編集**である: 盤面側の刻みが値の渡った 7 ビットより細かいと、復号値が隣の刻みへスナップしてエコーが値を動かし、
  ...
  同じ盤面値へ往復するため (`core/midi/controls.test.ts` で固定。2026-09-23 実測で、7 ビットでは URX44V の
  311 件中 97 件が往復しない — 調整画面の EQ 周波数と Q、GATE の attack / hold / decay、COMP の attack /
  release、DUCKER の attack / decay。COMP の **ratio** は、フィールドが実機の段の列なのでこの一覧に
  入らない — 段の数がワイヤーの位置の数より少ないので、7 ビットのエコーは出ていった段へ戻る)。`localStorage["urx-midi-log"]` を設定すると全 rx/tx バイト列とエンジンの
  ...
  その返答は受信拒否が解ける前に着くのか後か。**この時計には実機側の下限がある** — WKWebView は
  `performance.now()` をミリ秒に丸める (macOS で実測・2026-08-16: 5 起動分 419 レコードに小数部を持つ行が 0 件)
  一方、jsdom と Chromium は 1 ms 未満の値を保つ。そのため `report()` は印字するレコード自身から小数桁を決め、
  小数が無い走行では**何件を数えた上でのことか**を併記する — そこでの「0 ms」は*時計より短い*のであって同時では
  なく、また整数のタイムスタンプ 1 件はエンジンについての証拠にならないからである。したがって**この文書に出てくる
  1 ms 未満の値は、いずれもこのプローブで測ったものではない**: `ECHO_MS` の根拠である 0.13 ms のエコー遅延は
  別の CoreMIDI モニタの測定で、その家は `core/midi/echo-repro.test.ts`。
  この記録は**ファイル**にも書く。ページロードをまたぐ問いにはリングだけでは
  答えられないからで — リングはページのメモリにあり `performance.now()` もそこで振り直すため、リロード
  (`tauri dev` では HMR の編集ごと) は調査したい瞬間にリングを空にし、2 ページ分のエントリに共通の時計を残さない。
  出力先はアプリのログディレクトリの `midi-trace.jsonl`、1 エントリ 1 JSON オブジェクトでページ相対時刻の隣に
  エポック時刻を持ち、コントローラの掃引が 1 メッセージごとに往復を作らないよう 250 ms 単位でまとめ、2 MiB で
  1 世代を残してローテーションする (リンク台帳と同じ方針・同じ書き手)。各ページの記録は `page:open` で始まる。
  直前のページを閉じる `pagehide` のフラッシュは best-effort で (シェル側の後片付けがネイティブなのと同じ理由)、
  リロードで失う末尾を抑えているのはバッチ間隔の方であってこのフラッシュではない。両側とも **dev ビルド限定**で、
  `import.meta.env.DEV` がバンドルから記録器を落とし、`debug_assertions` が `append_midi_log` コマンドを拒否させる —
  インストール版は書かないし、書かせることもできない。macOS + Stream Deck+ (入出力とも 1 本の IAC Bus) での実測
  (2026-08-09): 8 アドレスの初期送出は 1 ms 以内に出終わり、拒否区間は同じミリ秒に終わり、ループバックは 5 ms
  後に戻る — 拒否区間は何も守っておらず、そこを覆う長さの区間を張っても、その間の本物の入力を捨てるだけになる。
  この実測があるので、初期送出を拒否区間の内側に留めては**いない** — ライブ同期の開始処理がセッションの成立後に
  呼ぶ `MidiControl.liveReadSettled` から出て、ラッチが解けた後に走る。プランを守っているのは下記のエコーガードで、
...
- **ゲート** — 実機読み出しがプランを掴んでいる間 (`deviceReadInFlight`: 取得・ライブ同期開始の読み戻し)、
  ...
  **分**の単位である。専用のフラグではなくリンクの保持者を読むので、メッセージを拒否するものと 2 本目の
  ...
  (1-knob ドラッグではフラッシュごとに繰り返され、かつ読み出しは代入ではなくマージするので
  その中の編集は失われない)、そして MIDI ラーン (コントロールを束縛するだけでプランを編集しない)
- **適用の反映** — 受信編集は console の編集と同じファネルを通る: ペアミラー (`mirrorLinkedPair`) →
...
他の何かが実機に触れれば届く — 誰でも繋げる経路が既定なので、これは稀ではなく通常の状態である。
...
購読中のチャネル宛の notify は破棄せずそのチャネルのバッチへ吸収する。

**2 プラットフォームで実測済み。** エンドポイントか応答の形が違えば Windows 版は
...
`releaseDeviceLink` は現在の保持者を名乗らない解放を無視する (Rust 側で epoch が課しているのと同じ規則)。
**ライブセッションが返すのはトグルを落とした時点ではなく、リンクが落ちた時点である**: `releaseLive` はまだ
往復している follow read を待ち、read 自身は epoch を持たない — `vd::sender` は問い合わせた時点で installed な
ワーカーを渡す — ので、その窓で接続した操作はワーカーを read の下で置き換え、read の後処理はそれを置き換えた
セッションに対して再ベースとフラッシュを行ってしまう。よって保持者はその解放の最後に返し、切断が失敗した
場合にも返す。閉じられないリンクが、アプリを自分のデバイス操作から締め出したままにしてはならないからである。
接続を開くフロント側の経路はすべてここを通る: メニュー操作 7 か所は `withDevice`、ライブは `activateLive` の**接続時
...
再ネゴシエートする。モデル選択がロックするのはライブ**セッション**中だけであり、切断時ではなくトグルで解除される唯一の項目で
ある: モデル切替は plan を丸ごと
...
実機側で起きる変化は 3 種類あり、どれに当たるかで**そもそもコードを書くかどうか**が決まる。
...
- **実機が実際の状態を変えてしまう場合。** insert エフェクトを選択すると実機が自分で ON スイッチを入れる、など。
  ...
  ON パラメータをセレクタと、それが適用されるエンジン値の**後に**発行し、プランが指定した状態へ戻す。ここを実機に任せると、
  ユーザーが OFF にしたエフェクトが可聴で ON に戻る。
- **実機が実際の状態を変え、しかもそれを通知しない場合。** 選択中の insert エフェクトの上限を越える
  サンプルレート遷移では、実機がセレクタ・ポインタ・バイパスを消去するが、報告するアドレスはレートだけで、
  対応レートへ戻しても何も復元されない。消去済みの値を最初に見るのはそのレート通知が全読み直しへ
  エスカレートした読み出しなので、ここは**マージ**で決める。`insertFxHoldKeys` が device-follow の読み出しで
  プラン側の値を保持するキーを名指し、`readIntoPlan` の `hold` がそれを patch から外す
  (この引数のもう 1 つの使い手は「ソースレイアウト」の `readback.ts` にあるソースの裁定)。device view には
  実機が答えた値がそのまま残る — これがプランとライブスナップショットを食い違わせ、通常の送信差分が
  セレクタ → 退避済みエンジン値 → バイパス意図の順でエフェクトを送り返す。2026-08-18 に URX44V で実測:
  この再送は消去を起こしたレートのままでも受理され、対応レートへ戻しても保持される。レートが戻るのを待たずに
  送るのはこのため。読み戻しは走行した 3 エンジン (Pitch Fix・入力 Compander・出力 MBC) の各 64 スロットを
  遷移前の値と比べて一致したが、非既定の sentinel を置いたのはエンジンごとに 1 スロット (Pitch Fix の
  Speed 37・Compander の Threshold -32.10 dB・MBC の LOW Threshold raw 99) で、セレクタ書込は配列を型の
  既定値で埋め直すため、各エンジンの残り 63 スロットの一致は再送の着地と埋め直しを区別しない。**もう一方の消去** —
  ペア両メンバーのエフェクトを落とす Signal Type 遷移 — と分けるのは、読み出しの値ではなく notify の
  流れである。URX44V 実測: 遷移は両メンバーのセレクタと ON を告知し、レート逸脱はレートだけを告知して
  他は何も出さない。この区別が notify 側にしか無いのは、読み出しがスナップショットではないためである —
  アドレスごとの応答は数百 ms 離れており、ペアの Signal Type はセレクタより**先に**読むので、その隙間に
  落ちた遷移は 2 つの値を等しいまま古くする。よって読み出し中に実機が告知したルートは対象外にする
  (`HoldContext.announced`・`DeviceFollow` の `onDeviceParam` が供給)。レートを notify 側から
  読むのも同じ理由による: 逸脱は読み出しがレートのアドレスを尋ねる前に終わりうる (48 → 96 → 48 なら
  読み出しはエフェクトが動くレートを掴む) ため、判定は読み出しのレートに加えて**実機が途中で告知した
  全レート**を見る (`HoldContext.ratesSeen`)。この一覧は上の告知と違い読み出しの窓へ絞らない — 読み出しを
  起こしたレート notify は開始より前に届いており、消去を起こしたレートを運んでいるのはそれだからである。
  さらに、レートを確定しなかった読み出しを跨いで残る (先に終わった読み出しが消すのではない) —
  リコンサイルは同時に 1 つしか走らないので、notify とそれが起こす全体読み出しの間に**別ノードの scoped
  読み出し**が挟まる。この scoped 読み出しが両方の要点である: レートのアドレスは尋ねないが盲目ではなく、
  ノードの insert FX を他の本体値と同様に読むので、実機が消去した瞬間に走っていればそれが**最初に**
  解除済みセレクタを見る。読み出し自身のレートだけで判定すると、ここで消去を採用してしまい、後続の
  全体読み出しには保持するものが残らない。
  操作者が本体で選んだ
  No Effect との区別は、**その読み出しが持つレートの根拠** — 自身が確定させたもの
  (`ReadbackResult.deviceSampleRate`) か、走行中に実機が告知したもの — が付ける。
  プラン側のレートは使わない — *シーンのみ* のデバイス範囲では読み出しを跨いで復元されるため、実機が
  とうに離れたレートを名乗りうる。**どちらの根拠も無い**読み出しは何も保持しないので、操作者が手で消した
  エフェクトは採用される。保持した件数はステータス行に
  出す (インストール版では console に届かないため)。
...
しまい、続く idle の安全網がその戻った値をプランへ読み込んでしまう。取り直すのは値だけで、順序は flush 自身の
...
**flush が書いている間、reconcile は待つ** (`DeviceFollowHooks` の `deferReconcile` → `live.isWriting`): 編集が
flush を予約した時点から、その flush とその後ろに積まれた flush が終わるまで。それより前に取る読みは、編集された
アドレスに編集が置き換える側の値で答える — flush がまだ送っていないか、実機がまだ書込前の値を答えている — そして
マージは、読みを発行した時点の値をプランが保っているキーに読みの値を入れるので、読みより前の編集はそこで
取り消される。マージが守るのは読みの最中の編集である。2026-09-24 にレース診断ハーネスで実測
(`t1b-overtake.spec.ts` の「an edit waiting in the flush window survives the re-read an ordinary device change
takes」): CH 1 の HPF 周波数を実機側で変えた 250 ms 後に CH 1 の PAN を動かし、ノードの読み直しの期限が来た時点で
まだ flush の 120 ms 窓の中にあった形で、PAN は画面上で元の値へ戻され、一度も書き込まれなかった。`a94d0b28`
でも同じである。待ちが終わった時点ではどの編集も送信済み・ACK 済みで、読みの settle がその告知を待つ。converge
は flush の中で走るので待ちはそれも覆い、そこにはさらに 2 つの理由がある: converge は書込スコープ全体をラウンド
ごとに書き直しており、そこで取る読みはアプリ自身が変えている最中のデバイスの読みになる。またラウンドが走る間、
2 つの読み手が 1 本のリンク上で交錯し、レースハーネスはこれを不変条件 4 として報告する。編集の時点で既に
走っている読みは止めない: その編集が予約する flush は読みの横で出ていき、マージが編集を保つ。窓は**消費せず
保持する**: バーストが名指ししたノードは settle タイマー自身の再アームで読み直され、その再アームが待ちを
終わらせる契機でもある (`follow.ts` には flush の終了を聞くものが無い)。`IDLE_FULL_MS` を跨いで先送りされた
パスは、再試行するタイマーに引きずられて縮退せず全体読みのまま走る。
...
これは通常の盲目の読みであり、**乖離を固定してしまわない唯一の答え**である。
...
実機由来 notify は、ループがフェーダーに到達する前に容易に着く)。**書込より後・その書込の告知より前**に届いた
notify は、実機が変化を行った順に告知する以上、書込が上書きする側の値を伝えている。追従層はそれをプランへ入れず
(`LiveSync.hasUnannouncedWrite`、名前の場合は `hasUnannouncedName` を `DeviceFollow` が `isSuperseded` として
問う)、そのノードを読み直す。書込より前の notify は、そのアドレスを「実機から読む」側に残す。読み直しは、窓の中の
notify が背後に持つ書込がどれも告知済みになるまで保留する: それまではマーク以降にそのアドレスについて実機が
最後に言ったのが上書きされる側の値なので、settle がその値で答えてしまい、アドレスを読めば書込前の値が返る。
帰属を誤ったときにマージを正しく保つのはこの保留した読み直しで、マークが減らすのは無駄な照合 1 回である。
2026-09-24 にレース診断ハーネスで実測 (`t1b-overtake.spec.ts` の「a device value our in-flight write replaces is
neither shown nor written back」の 3 変種すべてと、`t1d-name-window.spec.ts` の「a rename our own rename in flight
replaces is neither shown nor written back」): 盤面は操作者の値を保ち続け、上書きされる側の値を実機へ書き戻す
フラッシュは出ない。読み直しを保留しない場合、`acked` 変種 (notify の 280 ms 後に書込を解放し、読み直しの期限が
書込の ACK と告知の間に来る形) では idle の全体読み込みまで上書きされる側の値が表示された。
...
追従が実機側のリネームを運ぶので (次の段落)、名前のアドレスは登録集合に入っている。どの登録集合にも
入っていないアドレスの notify は、`Subs::absorb` が 1 本残らず落とす。
...
無いままリネームが取り消される。数値と違って振動しないので気付く契機も無い。実機側のリネームはそれでも届く:
...
フラッシュが追従層へ再購読を依頼しなければ、モード変更が足したアドレスは次の reconcile が
たまたま走るまで届かない。依頼はフラッシュの**末尾**で行い、フラッシュの内側では行わない: 再登録は購読を
...
**追従だけを行うケースが 4 種類、名前と同じ理由で登録集合に加わる**:
アプリが**読むだけ**のアドレスはここに挙げない限りどの登録にも入らないため、実機の告知が誰にも届かず、
値が追いつくのは次の全体読みのときだけになる。最初の 2 種類は URX44V で front-panel 操作に対して notify を出すことを実測している
(2026-08-11・System V1.3.1.0)。これが「本当に黙っているアドレス」との分かれ目で、D.Gain と FX / insert FX の
エンジン配列はパネルで動かしても何も出さないため、その用途では登録しても無意味である。**device 自身による
再計算は別の契機で、こちらは announce する**: FX のディレイの Sync / Note / BPM を書くと、実機が BPM と音価から
ディレイタイムを計算し直し、書いた配列アドレスへ notify を出す。沈黙しているのは front-panel 操作に対してであって、
アドレスそのものではない。

| アドレス | アプリが書かない条件と理由 | どのノードの scoped read が修復するか |
| --- | --- | --- |
...
| 実機が計算する FX のディレイタイム (681 / 685 の slot 6) | tempo Sync が入っている間は実機が BPM と音価から導いており、書くとディレイタイムが音価から外れる (`fxRowOwners` の `computed`) | **その FX チャンネル** — scoped read が EFFECT TYPE とパラメータ配列を丸ごと読み直す |
| STREAMING のソース選択 (705 / 706) | プランが STREAMING の結線を持たない間: 本体のリストに None が無いため、emit はそこへ NONE ではなく何も送らない (`leavesUnsourced`) | **`bus.stream`** — その scoped read がノードの DELAY とともにソースを読み直す |

3 種類目は**読取専用のアドレスではない**。見出しが「ケース」と言っているのはそのためで、ディレイタイムが
操作者のものである間、slot 6 は他のパラメータと同じように書かれる。この一覧へ移るのは tempo Sync が入っている
間だけで、Sync を切れば戻る。だからモデルに加えて**プラン**も要る — slot 6 がここに載るかどうかが、エフェクト
タイプと Sync スイッチの関数になる。対になる相手も違う: 他の 2 種類はアプリが元々書けないアドレスだが、これは
実機が導いていないときには書くアドレスで、送る集合とこの一覧は厳密な補集合になる。

4 種類目は 3 種類目と同じ形である: STREAMING のソースはプランが STREAMING の結線を持つ間は書かれ、持たない間だけ
この一覧に加わるので、これもモデルに加えてプランが要る。実機は自身のパネルで選んだソースを、書き込みのときと同じく
そこで告知する (「STREAMING のソースを記述しないプラン」節)。

4 本とも `translate.ts` の `planToFollowOnlyAddrs` が、対になる emit 判定のすぐ隣で列挙する。登録と書込抑制が
...
同じ値について食い違う。Track Count と STREAMING のソースは `sceneExternal` なので *シーンのみ* では外れ、送りの tap はシーン内の値
なので残る。**索引の
...
ゲートが `only === undefined` なら、front-panel の Track Count 操作はそのアドレスに触れない scoped read を
...
**本当に黙っているアドレスを覆うのは登録ではなく、書込の手前に置く読みである。** 収束は書込スコープ全体を
読み直して差のあるものを送り直すので、どの編集も名指ししていないアドレスへプランの写しを載せる唯一の経路に
なる — そしてこの 3 系統ではその写しがいくらでも古くなりうる。動かした手を告知したものが無いためである。
そこで収束を伴う flush は、clone する手前のプランへ先にそれらを読み込む (`live.ts` の `parkSilent` →
`readback.applySilentState`)。送られるのは実機自身の値になる。効いている制限が 2 つある。**この flush が書いた先頭が
リセットしたものを外す** (`ParamSpec.resets`): 実機にその値を持たせたのが先頭の書込で、操作者の値を戻すのが収束
そのものなので、ここで読むとリセット値を採用してしまい復元が出ていかない。**ノード単位ではなく系統単位**である —
1 つのチャンネルは COMP/EQ タイプと insert エフェクトを同時に持ち、先頭のものはそのうち片方だけなので、ノードごと
外すと先頭が触れていない系統まで巻き込み、収束がそのエンジン配列へプランの写しを — パネルで何をしていようと —
書いてしまう。**announce される帯をリセットする先頭は何も名指ししない**: COMP/EQ・BUS Type・pan はデバイス追従の
担当で、park はそれらを読まない。もう 1 つ、当セッションが最後に送った値を実機がまだ保持しているアドレスでは
**プラン側の値で答える** (`live.holdsSent`)。次の flush を待ってプランに座っている編集を守るためで、merge が
守るのは読取**中**の編集であり、こちらはその手前で行われている。この答えの出どころはプラン自身の emit であり、
emit の**並び**を決めるのはプランが持つ先頭の写し — FX チャンネルの EFFECT TYPE と、ノードの insert-FX セレクタ
— である。そこで各先頭は後ろの生値より先に実機から読み、emit は実機の先頭をまとったプランから取る: 両者が
食い違えば、同じ 1 アドレスが実機では或る param、プランでは別の param を指し、そこで答えると読み手へ別 param の
値を渡すことになる。読みが失敗したときはセッションを落とす。
確認できなかった値の上に収束が書くことを許さないためである ([失敗時の中断](#失敗時の中断))。通常の flush は
park を一切取らないので、ドラッグが払うものは無い。

**flush はもう 1 つ、動かす先頭のいずれかが silent な系統をリセットするとき、その先頭書込の手前でも park を
取る。**上の park は書込の後段にあり収束のためのもので、こちらは書込そのもののためにある — silent な系統を
リセットする先頭はその系統を入ってくるタイプの工場値で埋め直すので、実機が持っているのは出ていくエフェクトで
あり、それを告知するものは無い。flush は自分の先頭がリセットする系統だけを読み (同じ `ParamSpec.resets`、今度は
`only`)、その後でコマンドをプランから derive し直すので、出ていくものは読みがマージした値を運ぶ。先頭を動かした
コントロールではなく境界で取るため、1 回の読みがすべての書き手を覆う: EFFECT TYPE の 2 つのセレクタ、そのどちらの
アンドゥ、insert FX のセレクタ、MIDI の割当、プランのロード。そこではプラン側の先頭を残す (`keepHeads`) —
入ってくるタイプは操作者が既に選んでおり、値は**実機が乗っている先頭**が所有するキーの下へ入れる。

**1 回の flush が取る park は 0 個・1 個・2 個のいずれか**で、2 つの条件は独立している: 収束側は flush が収束
するかどうか (`sideEffect`)、こちらは動かす先頭が `ParamSpec.resets` を宣言しているかどうかで決まる。通常の
編集はどちらも取らない。silent な系統をリセットしない収束先頭は収束側だけを取り — COMP/EQ タイプ・バスタイプ・
Signal Type・PAN/BAL が、その系統を既にデバイス追従が聞いている収束先頭である — このとき先頭書込はその park が
走る時点で既に実機へ出ているので、そこでの失敗が何を止めるかが変わる ([失敗時の中断](#失敗時の中断))。FX EFFECT
TYPE と insert FX のセレクタは両方を取る。どれを取るにせよ、park と収束はデバイス追従から見れば 1 つの読取
フェーズであり、その間 reconcile は待たされる (`live.isWriting` は flush 全体で真を返す): 1 本のリンクに読み手が 2 つ載る形が、
レースハーネスの不変条件 4 が捕まえるものである。
...
**待ちのコストは推定ではなく実機で測った** (2026-08-02・URX44V V1.3.1.0・診断計装を載せた使い捨て
...
いないカーブを描き続ける) か「窓の中で読む」(この機構が防いでいる当の欠陥) しかない。
...
sequenceDiagram
  ...
  alt converge: プランが著者である値を実機がリセットした
    ...
    H->>H: plan をクローンする (書込ウィットネスはフラッシュが値を取った時点から開いている)
...
ウィットネス自体はさらに前に開く: フラッシュは、最初に取った値 (数値でも名前でも) に refetch の起点があれば、最初の書込の
前に開き (`watchEdits`)、読みに渡す。読みはノード全体を対象にし、フラッシュの書込が送信中の間にそのどこかへ入った編集は
どの書込にも載らないからである。送信を始めた後に初めて現れた起点 — 実機側の変化の後の取り直し、または名前ループが自分で
取った一覧で — は、ウィットネスが開いていなければ次のフラッシュで送り、そのフラッシュは自分の開始から見張る。同じ
フラッシュの converge はそうした起点 — 次のフラッシュへ回したもの、plan がその後に動いた値で送ったもの、フラッシュの
途中で初めて現れたアドレスのもの — に触れず、snapshot はそのアドレスについて持っていた値 (新しいアドレスなら何も
持たない状態) を名前も含めて持ち続けるので、次のフラッシュがそれを送る対象として見つける。
converge から送ると、実機はそれが動かす値を再計算するのに読みが続かず、converge がその値の plan 側の古い写しを書き戻す。
操作者が動かしていない起点は、引き続き converge が戻す。refetch の起点を持たないフラッシュは開かない。
...
**この受け渡しから取り下げられるものは何も無く**、その必要も無い: 答えは実機自身の告知であり
**最後の告知が勝つ**ので、こちらの書込の後に操作者が盤面で動かしたアドレスは**操作者の値**を告知する。
フラッシュ中に着地した `capture()` にも答えは要らない — 実機読み出しからスナップショットを作り直すので
あり、デバイス読みが同じデバイスの後の言葉と矛盾することはない。この機構全体の存在理由は実在する事象で
ある: 書いたアドレスを**こちらの値**で答えると、マージは実機上の操作者の操作を取り消し、
...
存在しないため、同じ理由でスナップショットから除外される。ノードを限定した読み出し — 一部のノードのリコンサイル、
または refetch が起点の動かすノードを読む読み出し — は、読んだノードのアドレスだけを名前も含めて取り直す。それ以外では
スナップショットは持っていた値を (何も持っていなかった状態も含めて) 持ち続ける。読み出しはそこについて何も言っておらず、
plan が持っていて実機へまだ送っていない値は、引き続き送るべきものだからである。refetch の読み出しは各ノードの本体を
読み、ノード自身の名前は読まないので、そこでは読んだノードも含めて全ノードの名前について同じことが言える。リコンサイルは
対象ノードの名前を読み、それを取り込む。
...
| 実機 notify、direct | 該当 1 エントリのみ (`noteDirect`) | その notify が書いたキーだけ `absorb` (適用の前後で差分を取る)。操作者が開いているエントリは残る。記録済みのエントリが受け取るのは `nodeParams` / `connParams` の入れ子リーフのうち、**before と after の両方**が読みの測定元を保持しているものだけで、そのとき両側が読みの値を取る。フィールド丸ごとのエントリは畳まない |
| reconcile 読み出し (scoped / 全体) | 読み出しのクローンから `resync` (scoped の読み出しは読んだノードだけを取り、残りは保持) した後、読み出し発行より後に押された direct ジャーナルのエントリを再適用 | reflect 内で `reset` |
| 副作用の refetch | 読んだノードについて読み出しのクローンから `capture` (名前は読まないので除く) した後、同じジャーナル再適用 | 実機が著者であるキーだけ `absorb` |
| converge のラウンド | 固定クローンから `capture`。ただし flush が送らなかった refetch の起点と、本体のパネルで点いたスイッチは snapshot の値を保持 | 触らない |
...
sequenceDiagram
  ...
  alt 窓の中で読み出しが着地した
    ...
    R->>I: インスペクタも調整画面も、変更が自分の読むノードを名指した時だけ refresh
  end
...
| 機種切替を伴う取得・ライブ同期開始の読み出し中の編集 | 盤面では書き込む前 (`planTakesEdits`)、それ以外は `markChanged` — プランを読み出し開始時の状態へ戻す。どちらもそのことを示す (`busySwitchRead`) | 画面上のプランは切替が破棄するプランだから ([失敗時の中断](#失敗時の中断)) |
| 切替が差し替えるプランの上でまだ続いているジェスチャ | プランの差し替え時に、それを持つ面自身 — 盤面 (`Graph.setModel` に別のプランを渡す)、inspector (ゲートを越えて再構築し、パネルを組んだプランに対してだけ操作が応答する)、調整画面 (`refresh`)、CONSOLE (`render` — キーボードフォーカスを引き継ぐのは同じプランの再構築だけ) | ジェスチャを始めたプランはもう無いので、押したままのポインタやキーは差し替え後のプランに何も書かない |
| ライブ中の `sampleRate` パッチ | エントリ丸ごと拒否 (文面は他も触れているかで選ぶ) | 部分的なアンドゥはどのジェスチャも作らない状態を残す |
| 適用するとチャンネルの +48V と HI-Z が両方オンになるアンドゥ / リドゥ | peek したエントリに対する `patchBlocked` (作り出す状態を見る) | アプリは片方がオンの間にもう片方をオンにしない。アンドゥも元のジェスチャと同じだけスイッチをオンにする |
| 実機読み出しの在航中に行った +48V / HI-Z の ON で、その読み出しがもう片方をオンと見つけたチャンネルのもの | マージ後の `readIntoPlan` (`readRefusedSwitches`): スイッチと、同じ編集が書いたと witness が見たキーを読み出しの値へ戻し、その編集をアンドゥスタックから取り除く (`PlanHistory.retract`)。ライブセッション中は、flush が送信済みの ON は実機に届いているので拒否せず (`SwitchSession.holdsOn`)、flush が保留したままマージが読み出しの値に置き換えた ON も同じく報告して取り除く (`unsentRefusedSwitches`) | 面は実機が何を持つかをまだ知らないプランに規則を問うた。実機側のスイッチは読み出しが見つけたもので、そのまま残す |
| もう片方をオンにしている実機が受け取ることになる +48V / HI-Z の ON と、保留中の HI-Z の ON が一緒に下げた +40 dB の A.Gain | ライブの flush の送信ループ (`onExcludedBy`。スナップショットと、follow の読み出しがまだプランへ持ち込んでいない実機の通知と、ON を送る直前に読むもう片方のスイッチに対して。A.Gain は `carrierOf`) と各収束ラウンド (ラウンド自身の読み出しに対して) | 捨てずにプランに残す: もう片方がオンだと見つけた読み出しが取り戻し、その後の通知やキャプチャが flush に問い直させる |
| 実機がパネルでオンにしたと通知した +48V / HI-Z と、そのチャンネルのもう片方への、ライブの収束の書き込み | 収束の前のライブの flush (`exclude`。収束中に届いた通知も加わり、収束のその後の読み出しとラウンドの送信がそのたびに問い直す)。その後ろのキャプチャは、そこでスナップショットと通知が持っていたものを保つ | 通知が予約した follow の読み出しが取り込むまで、そのスイッチは実機のもので、プランの古い OFF がそれを切ってしまう |
...
#### 伝播を follow レイヤに任せてよい条件

上のタイマーによる全面再描画は**実機由来**の変更に対する修復でしかない。伝播コードを書くかどうかは、
所要時間ではなく**変更の出どころ**で決まる。境界は 1 つではなく 3 つある:

| 変更の出どころ | 修復するもの | 上限 |
| --- | --- | --- |
| `follow: "direct"` **でない** param の実機 notify | scoped reconcile → `followFull` → `syncRateUi` → `applyRateConstraints` → 全ビューの全面再描画 | バースト沈静後 `RECONCILE_DEBOUNCE_MS` |
| `follow: "direct"` **である** param の実機 notify | アイドルネット。direct のみの窓は settle 時に reconcile せず、スナップショットを再基準化するだけ | 実機が静かになってから `IDLE_FULL_MS` |
| 実機が announce する義務を負いながらしなかったアプリ書込 | 同じアイドルネット。書込側から `writeSettle` の announcement watch が張る | `SETTLE_TIMEOUT_MS` の後にアイドルネット |
| アプリ編集・MIDI 適用・undo・ファイル読込、および Live sync が**オフ**のとき全般 | 無し | 永久に修復されない |

最後の行がほとんどの場合を決める。自分の書込に対する実機の答えはエコーで、`onNotify` はどちらのタイマーを
張るよりも**前**にそこで return する。したがってアプリ由来の変更は自分の修復を何も予約しない —
触れたビューはすべて、その変更を起こした場所で再描画しなければならない。`reflectFollow` の direct 分岐が
持つビュー別の再描画が、遅延の最適化ではなく load-bearing なのはこのためである (外部 MIDI はそのすべてに
到達し、何も張らない)。

この規則が**言っていないこと**が 2 つある。連続コントロールの遅延を許す規則ではない —
スイープ中のフェーダーや PAN がアイドルネットでしか直らないのは出どころに関わらず欠陥で、
`follow: "direct"` はそのために在る。もう 1 つ、アプリ自身の coalesced reflect
(`requestReflect` → `reflectFollow`、`REFLECT_MIN_MS`) との冗長は、どちらのタイマーとの冗長よりも
はるかに厳しい別の議論である。MIDI 適用の傍らで 2 度目の再描画が不要なのはそちらの理由で、
`graph.repaintDirtyNodes` が `redrawWires` で終わるからである。
...
すると、違うものだけを再送するループは 1 ラウンドにつき 1 リンクずつしか進まない: ラウンド 2 が ON を再送して TYPE を落とし、ラウンド 3
...
書込スコープ全体を訊くため)。この待ちは `sendConverging` の全経路に一律 +300 ms を課すものではない —
実機で窓を測ると、待ちは実機自身の
...
代わりにフラッシュ自身の送信リストからループを seed するのは**誤り**である — その値は送信時点で
...
読み取り側は、その結果が何に使われるかで線を引く。**取得 (fetch) は画面上のプランへの部分読み取りを許容する**: 件数は status 行へ出し
...
それを無確認でハードウェアへ収束させてしまうためである。したがって不完全な読み取りではセッションを開始せず、読み取った
値もプランへ一切マージしない (`readIntoPlan` の `accept`)。アンドゥ履歴もそのまま残す。読み取れない
reconcile は追従を停止する — 次の収束が、操作者自身が実機で行った編集の上に古い値を書き戻すのを防ぐ。**silent
アドレスの park は、書込がまだ手前にある状態で同じ規則を適用したものである**: 読みが失敗すればセッションを落とし、
後段の書込を止めるのは flush 自身の世代チェックである。読みは、その書込が置き換えてしまうもののために取っている
(channel-tuning.md「FX EFFECT」。flush がどの park を取るかは[ライブ同期中のイベントとタイミング](#ライブ同期中のイベントとタイミング)にある)。
**したがって失敗の代償は flush ではなく park ごとに決まる。**先頭書込の手前の park は先頭自身を後段に持つので、
そこでの失敗は**タイプを一切書かない** — これは silent な系統をリセットする先頭に対する保証であり、この park が
走るのはその種の先頭のときだけである。収束の手前の park が後段に持つのは収束だけなので、そこでの失敗は確認できな
かった値の上にプランの写しを送らない。先頭書込の手前の park を取らなかった flush では、先頭書込はその時点で既に
実機へ届いており、止まる対象ではない。取得のキャンセルは
開始時のプランに手を付けない (読み出しはプランの専用コピーに対して走る。`readIntoPlan`)。キャンセルが「何も起きなかった」を意味し、旧値とデバイス値の無標識な混合を残さないためである。
機種もその一部である: 取得・ライブ同期開始が提示した実機の機種への切替は、新しいプランへの完全な読み出しが着地して
初めてプランを差し替えるので、キャンセル・失敗・不完全な読み出しはプランとその機種を元のまま残す — 取得も同じで、
部分読み取りを残すのは切替を伴わない場合だけである。切替を伴う取得が不完全だったときのステータス行は、件数ではなく、
何も切り替えなかったことを示す (`fetchSwitchIncomplete`)。その読み出しの間、画面上のプランは切替が破棄するプランなので、
どの面からの編集も、アンドゥ / リドゥも、MIDI 入力による変更も読み出しの間は拒否し、そのことをステータス行に出す
(`busySwitchRead`)。編集ファネル (`markChanged`) がプランを読み出し開始時の状態へ戻す。
盤面は先に問う (`planTakesEdits`)。盤面はノードの位置をポインタの移動中に書き、移動を報告するのはドラッグの
終わりだけだからである: ノードのドラッグ・配線・ノート・棚への出し入れ・Arrange は書き込む前に拒否し、読み出し
開始の時点で既に動いているドラッグはそこで終わらせて通常の編集として報告する。切替が適用される時点でまだ続いている
ジェスチャは切替で終わるので、押したままのポインタは切替後のプランに何も書かない: 盤面はそれを破棄し、inspector は
ゲートが保持しているもの (開いたピッカー・変換中の入力) を越えて再構築され、パネルを組んだプランに対してだけ操作が
応答する。調整画面は押下を待たずに再構築し、その押下は終わるまで何も書かない。CONSOLE は切替後のプランの
ストリップへキーボードフォーカスを引き継がないので、押したままのキーはそのどのコントロールにも届かない。
...
**恒久的な例外は 4 件**あり、いずれも「中断する方が弱い挙動になる」場所である。抜けとして提案されないよう、
ここに列挙しておく。
...
1. **self-test は中断せず集約する。** これはユーザー操作ではなく診断機能であり、往復に失敗したパラメーターを
   ...
   **そして 1 パスが書くのは値だけではない。** MONO IN ペアの Signal Type も掃引対象で、走行は 2 ブロック
   (未リンク → STEREO リンク) に分かれる。リンクしたペアは同じ状態の変種ではなく別の実機状態であり、
   2 メンバーで 1 組のチャンネル値を持ち、どちらへの書込も相方へミラーする。捕獲時の状態のまま走ると
   もう一方の状態が一度も書かれず、捕獲がリンク済みだった場合は隣と違う insert エフェクトをチャンネルに
   持たせることもできない — ペアのセレクタは 1 つなので、そうした文書は実機が取れる状態を指しておらず、
   収束はラウンド上限まで往復する。**これはセレクタの規則ではなくペアの規則**なので、リンク済みブロックは
   `mirrorLinkedPair` — ペアが何を共有するかについてのアプリ自身の答えで、ヘッドアンプと BAL 以外での各
   メンバーの pan は独立のまま — を通して primary の値を secondary へ写す。未リンクで捕獲したペアは残り
   全てについて食い違ってよく、それがこのブロックが書けなければならない捕獲状態そのものである。
   実機自身の Signal Type と PAN/BAL は捕獲プランが持ち復元が書き戻すので、
   完走した走行は出発点に戻る。完走しなかった走行は、**死んだブロックのペア状態と、遷移で実機が
   叩きつけた pan を残す** — 値だけでなくペア構造が摂動したまま残る。

   **部分的な capture で逆に手を付けないセレクタが 1 つある: STREAMING のソースである。** それを読めなかった
   capture (`ReadbackResult.sourceUnread`) は、復元の元にするプランに STREAMING の結線を持たないので、どのパスも
   復元もそのセレクタへ書かない — 実機は保持していたソースのまま残る — そしてレポートは Issues の中でその旨を
   伝える。部分的な capture のそれ以外のアドレスは、上のとおり掃引し復元する。
2. **`translate.ts` の値の丸めは拒否せずクランプする。** ハードウェアの手前の最終防衛線であり、範囲外の値が
   実機に届くより、丸めて範囲内に収めた値の方が良い結果になる。**このクランプは隣の読み値には意図的に適用しない** —
   パネルはプランが持つ値を表示し、実機読取の後ではそれが実機の保持値であり、実機はアプリのレンジが除外する値を
   保持しうる。1 つの読み値は「実機は今いくつか」と「次の書込が何を送るか」の両方には答えられない。
3. **`pump` は不正なバイナリフレームを discard する。** vd プロトコルは JSON テキストであり、フレームの整合性は
   ...
   こちらは `read_text` で loud に失敗する (`broker-bad-response: binary frame`)。
...
画面下部の status 行に出す。**ダイアログを上げられなかった**場合 (シェルのコマンド自体が reject
しうる) は、`ui/error-box.ts` のキュー付きの箱へ回す — 決定ゲートより上の専用の段に置く。status 行は一時的な
1 枠なので、そこへ書いた失敗は次にアプリが報告するものに奪われ、両方を同時に読ませられないからである。
このクリアを打ち消す箇所が 2 つあり、共通するのは規則の側である:
**結果行が書かれた後に**上がるダイアログは、陳腐化していない行を消してしまう — プランを変えた操作はそれを
status 行にしか出さず、ダイアログが名指すのは別の話だからである。書込のレコーダー再読み出し (`finally` で
書込自身の行の後に走る) と、失敗レポートの保存提案 (`withDevice` が戻った後に走る) の 2 つ。どちらも
**ダイアログが上がる時点**の行を読む (手前で控えるのではなく) ので、その間に書かれた拒否 —
デバイス読み取りが履歴を握っているためのアンドゥ拒否など — があればそちらが戻る。書込側はさらに
**書込が終わった場合だけ**戻す — return する経路はすべて行を書き、キャンセルも自分の行で答えるので、残るのは
どこも答えなかった throw だけで、そこに残っているのは進捗表示、つまりクリアの対象そのものである。Live sync 中のランタイムエラーは複数ソース (live / follow / リンク監視) から
...
サンプルレートは、デバイスが受理した上で自ら取り消すことがあるプラン値です。**SETUP > Follow USB** が ON の場合、
URX はクロックを USB ホストに委ねているため、766 への書き込みは受理されてハードウェアをリクロックしますが、
約 0.4 秒後にホストのレートが再主張されます (URX44V で実測)。そのまま書き込むと、定着しなかった変更に対して
成功を報告することになります。

そのすべてに先立って、チャンネルの +48V と HI-Z が両方オンのプランは書き込みをその場で止めます。2 つのうち
後に出た方が、もう片方がオンの間のオンになるので、どの送出順でも合法になりません。したがって拒否は送信の中では
なく、リンクを開く前に行います (known-issues.md「本体は +48V と HI-Z を同時にオンにできるが、アプリはしない」)。
その後の各収束ラウンドも、ラウンド自身の読み出しが見つけたものに対して、送信順に同じ規則をコマンドごとに問います
(`client.ts` の `withoutExcludedOns`)。もう片方を先に切るラウンドは、その後ろの ON をそのまま送ります。これが
ライブの flush の収束に必要な理由は、収束がその後に実機のパネルが動かしたかもしれないプランを送り直すからで、
読み出しから両方オンを持ったプランは、パネルで切ったスイッチをもう片方の横で再びオンにしてしまいます。ライブの
flush は、実機がパネルでオンにしたと通知し、まだどの読み出しも取り込んでいないスイッチを、そのチャンネルのもう片方と
ともに収束から外します。外さなければ、プランの古い OFF がその上に送られます。収束中に届いた通知もその集合に
加わり、その後の読み出しとラウンドの送信がそのたびに問い直します (`exclude`)。送るものを既に決めたラウンドは、
決めたとおりに送ります。
...
**プランはレコーダーの上限に片方向で追従します。** レートが運べる本数の唯一の記述が
`core/constraints.ts` の `trackCountCeiling` で、プランのレートが動く唯一の経路が `core/plan.ts` の
`setPlanSampleRate` です。保存された Track Count を上限まで下げ、二度と上げません — 実機が上げないからです。
レートを動かすすべての経路がこれを通ります: ピッカー、実機のレートの採用、デバイス読み、新規プラン、そして
**読込** (レートと本数が両立しない文書をここで正規化します)。**読み取り時ではなくレート変更時に**適用します。
読み取り時のクランプでは、レートを下げた瞬間に 16 が戻り、プランが実機の持たないレコーダーを記述してしまいます。
インスペクタはレートが許す本数だけを提示し、メニューが短い理由も示します。盤面はレコーダーのトラックスロットを
保存値だけでなく上限でもゲートするので、実機に席の無いトラックへ配線できるプランは作れません。

**レート整定は microSD レコーダーへの代償も明示します。** これはレートの副作用のうち、実機が拒否ではなく
**自ら動作する**唯一のものです — 実機は運べないレートに合わせて自分の Track Count を下げ (44.1 / 48 kHz で
16 トラック、88.2 / 96 kHz で 8、176.4 / 192 kHz で 2)、レートを戻しても上がらず、param 839 はアプリが
書き戻せる値をすべて拒否します。つまり >96 kHz の機能群のように「1 回の書込から外れる」のではなく**破壊される**
ので、通知は決定の手前に置きます。判定は `core/constraints.ts` の `trackCountCeiling` / `trackCountDrop`、
本数は `client.ts` の `readTrackCount` が**実機から**読みます (オフラインのプランが持つ本数は最後に編集された値で、
実際の低下を見逃しもすれば存在しない低下を作りもするため)。すでに収まっている本数では何も言いません
(失うもののない相手に不可逆な損失の通知を出すのが、通知が読まれなくなる経路だからです)。読み取りに失敗した
場合は、クロック状態の読み取りの失敗と同じく書き込みを中止します。経路は 3 つ — 単純なリクロック確認、三択の **release** 側
(そのボタン直下の専用要素。デバイスのレートを採る adopt は費用ゼロで、共有 note は両方の選択肢を代弁するため)、
そして Follow USB のトグル (ホストのレートはこのアプリから読めないので数値なしの形)。**URX22 はレコーダーを
持たないため全経路で沈黙します。** 書込後は書込自身の epilogue がレコーダーノードを読み直します。低下させるのは
実機自身であり、それを告知するかはここでは未測定なので、来ないかもしれない notify を待たずにプランを更新します。
epilogue を予約するのはレート送信が**決着した時点**で、明示的な拒否以外はすべて予約します — シェルは送信して
から待つので、応答が来なかった書込は着地して本数を下げている可能性があるためです。呼び出しの返り値から
予約しないのは、2 つの送信の間でキャンセルが起きるとその返り値ごと失われるからです。
...
(この状態でのクリックはトグルではなく読み取り)、"OFF" と描くことも、デスクトップ版で非表示にすることもありません — 非表示にすると、操作者が
デバイス操作に踏み切った後にしか警告が出ないためです。また永続化せずセッションスコープに留めています — 記憶した値は、接続すらされていない
...
消えるのは DSP だけで設定値は残ります。したがって書き込み集合をレートでガードすることはしません
— ガードしても非収束は防げず (防ぐべき非収束が無い)、単にプランがデバイスへ届かなくなり、
レートを戻したときにプランが要求していない設定が残る結果になるためです。UI 側は機能上の制限を反映します
(`channelEqUnavailable` / `insertFxMenu` が EQ・INS FX チップをロックし、`rateConstraints` が FX2 を減光)。
これらの機能は当該レートで実際に動作しないためです。
...
ありません。バーが自前の短い語彙を持つと、バーの `cmd` とパネルの `Sent` が同じ数だと読み手が
知る手立てがなくなります。
...
この台帳が対象とする症状は**アプリが終了した後**に現れるため、ステータスバーにしか存在しない値は、
...
連続 100 時間超のセッション分 — なので、事象の直前の走行はディスク上に残ります。期間による上限は
追記のたびに全行の parse を要し、先頭からの切り詰めは毎回ファイルを書き直します。
...
「台帳の最終カウンタを読み、まだ線に乗っている follow read を待ち、リンクを落とし、最後に保持者を返す」が
コードの性質になり、3 つの出口がそれぞれ再実装する規則ではなくなります。この待ちは `abandonFollowWork` の
切り分けをプランだけでなくリンクでも成立させるもので — 単に終わっただけのセッションは read を最後まで走らせる
ので、read が使うリンクはセッションより長く生きる必要があります — 保持者を最後に返すことが、その read の上に
別の操作が接続するのを止めます。
...
よいものか」はウィンドウの一覧に関する規則ではなくホールド自身が答えます。このアプリは webview を 2 つ持ち、
MIDI コントロールウィンドウはプランもセッションもポートも所有していません。その窓自身の読み込みがすべてを
終わらせるなら、メインウィンドウのセッションを終わらせ、メインウィンドウが復元済みの MIDI 入力を閉じてしまいます。
しかもフロントエンドには何も伝わらず、ポートは選択済みのまま表示され続け、同じ項目を選び直して
も `change` は発火しないため、復帰手段は「なし」を選んでからポートを選び直すことだけになります。結果として MIDI
コントロールを開くと MIDI ラーンが何も受け取れなくなる — コントロールウィンドウを開く目的そのものが失われます。3 つ目の窓が増えても、誰かが一覧に足すのを憶えていなくても正しく振る舞います。
...
Follow USB (848) にも同じフラグが付いています。`core/control/device-setup.ts` がこれらの全表面で、Follow USB (848) と同じ形 —
...
ウィンドウが「置かれている場所」ではなくそのセッションで入るモードだからです。2 つのウィンドウは
1 か所で同じやり方で覚えられます。
...
重なります**。物理画素で計算すると、この重なりが「そのウィンドウはどのディスプレイのものか」の判定と、
その後の矩形への丸め込みに入り込みます。モニタもウィンドウも物理画素で報告されるのは事実ですが、
それは「1 つの空間で報告される」ことと同じではありません。
...
スケールの記録が無い状態ファイルについては、スケールを**試行で導きます**: 各ディスプレイを順に仮定し、その
...
読み直す保存側です。プラグインの復元を残して結果を後から補正する形は
成立しません — その時点で復元は既に矩形を失っており、補正ができるのは「作業領域の内側にある」ことの強制だけで、
意図されていた矩形を取り戻すことはできません。MIDI ウィンドウの呼び出しは `open_midi_window` の末尾に
置きます。`build()` が返る時点で移動が着地しているからです。試した他の置き場ではそれが見えません: 上の
setup フックと `RunEvent::Ready`、そして将来どのウィンドウでも自動で効くように window-state プラグインの後ろへ
登録する自前のフック — これは**実測で否定されています**。`window_created` フックの中では、プラグインが直前に
復元していてもウィンドウは生成時の位置を返し、そこから出した `set_position` が起動時と同じようにキューへ
載るためです。MIDI ウィンドウのページ読み込み時のフックや、そのページが呼ぶコマンドは試していません。
同じ作り方で 3 枚目を足すなら同じ呼び出しが要ります。
...
方法)。配置は 1512x982@2.0 の内蔵パネル + 2560x1440@1.0 の外部ディスプレイ。下の 2 パスの配置で流し直しても
同値でした:
...
**起動のたびに**失われます — 外形を持ち回る形で 4 回連続で起動して実測すると、外形の幅は
...
desk が pt でスケール係数が変わらないため、ループは 1 回で終わり、上の表の macOS の 5 ケースはこの経路で
成立しています。**2026-08-09 の**数値の
...
macOS の列はその関係が払う代償であって、独立したパネルのあちらでの挙動は
...
スナップショットを読み、古い値を書き戻す) で調整画面にある。表示状態は CSS のみで完結する: `main.ts` が選択の有無に
...
**セレクトは快適なタッチターゲットを高さとして取る。** 同じブレークポイントでラックとインスペクタの
コントロールは最小高さ 40px を取るが、`<select>` はこの形では取らない — WebKit はこれらのセレクトを
`min-height` の値にかかわらずプラットフォーム自身の高さに保つ。`height` なら取り、しかもプラットフォームの
コントロールのまま取る (computed の `appearance` は `auto` のまま) ので、ブレークポイントはラックの
2 つのピッカーとインスペクタの全セレクトに `height: 40px` を与える。2026-09-24 にスマートフォン幅で実測
(Playwright の WebKit 26.6 / Chromium 153 / iOS Safari 26.5 = iPhone 17 シミュレータ): 機種ピッカーは
`min-height` だけで 19 / 40 / 27 px、高さを与えると 40 / 40 / 40。インスペクタのセレクトは
どちらも与えないと 19 / 28 / 27 px、高さを与えると 40 / 40 / 40。デスクトップ版が描画に使う macOS 27 の
WKWebView (幅 390px の画面外のビュー) では、変更前はピッカーとインスペクタのセレクトが 19px、変更後は 40px。
`e2e/chrome.spec.ts` が Playwright の両エンジンでこのターゲットを保持する (ケースは `app-webkit` 用の
タグ付き)。ブレークポイントの上では何も与えないので、デスクトップのパラメータ行のセレクトは
プラットフォームの高さのまま入力欄の 26px と並ぶ — WebKit で 19px、Chromium で 28px。
...
- 加えて、隠した集合は機種ごとに `localStorage("urx-hidden")` (機種 id → ノード id 配列のマップ) にも記録され、
  ...
  アプリ再起動をまたいで残るように)。ファイル読込時はファイルの `hidden` が優先 (現在状態を上書き) し、
...
Ducker を目に見えて迂回するため、インスペクタが文章で述べているのと同じ事実が幾何からも読める。単一の
三次ベジェを上向きに曲げる方式では、宛先が下にあると自ノードのフェースプレート背面へ潜り込み、示したい
抜け方が完全に隠れてしまう。
...
ホバーはこの分類器を通るので、チャンネル発の**Ducker キー**もタップと同じく、選択時と同じ文を
ホバーでも出す。2 つの経路が一致しているかは見た目に兆候が出ないので、`e2e/directout.spec.ts` が
パネル側のヒントを読んで線の `<title>` と突き合わせる形 (リテラル比較ではなく) で pin している。
...
ただ 1 つである。各ビューは使用のたびに `getPlan()` フックで解決するので自動的に追従する。**MIDI 面も
同じで、何もメモ化しない** — 束縛済みコントロールは insert FX の family / プロセッサ / send を閉じ込め、
プランは**その場で**編集されるので、プラン**オブジェクト**をキー (その場の編集では決して変わらないもの) に
したメモは、ノードがもう保持していないエフェクトについて答え続ける。アンドゥはオブジェクトを差し替えない。その場でパッチを当ててから、
...
- 実機読み出しまたはファイル操作がプランを掴んでいる間 — どちらも await を跨いでプランを書き換えるか
  ...
  結果を捨てる)。なお 1-knob 再取得は、読み出し自身の所要時間より**最大 300 ms 長く**拒否を
  保つ。フラッシュの書込を読み出しの内側で待ち合わせ
  ...
  上限が付いている。機種切替を伴う取得・ライブ同期開始の読み出しでは、同じ窓で編集ファネルがそうする
  ように切替を理由として示す (`busySwitchRead`。[失敗時の中断](#失敗時の中断))
...
- パッチが残す**状態**をホストが拒否する場合 (`patchBlocked`) — 現在は +48V / HI-Z の排他。
  パッチは丸ごと適用され、それを記録した編集とここの間で 2 つのスイッチが最終的にどうなるかを
  問う者がいないので、エントリはこの規則を迂回できてしまう。動かすキーではなく状態を問うのは、
  アンドゥも元のジェスチャと同じだけスイッチをオンにするからで、エントリがどちらを動かしたかは
  答えを決めない。保留のしかたは同じで、チャンネルがどちらか一方だけを持てば同じ押下が通る。
  フックが受け取るのは**適用されるパッチ**である: `peekUndo` はエントリの**逆**を返し、
  `takeUndo` が適用するのもそれなので、読んだものと適用するものが同じキーの逆の値になることはない

### 履歴を破棄する箇所

以前のどのエントリもプランが戻れる状態を表さなくなったとき、両スタックを破棄してベースラインを
取り直す。**別ドキュメント** (`loadPlan` — 新規 / 開く / ドロップ / 履歴行 / 機種ピッカー /
取得・ライブ同期開始が提示する機種切替 (その完全な読み出しが着地した時点で適用) / `?plan=` ディープリンク) と、
**広さを問わず実機リードバック** (`rerenderPlan` — 取得・ライブ同期開始・`.urxf` インポート、および
device-follow の全体リコンサイル)。
1 ノードの follow readback はベースラインを取り直すだけで、既に記録したエントリは残す。+48V / HI-Z の ON — 在航中に
行ったもの、またはライブの flush が保留して送信しなかったもの — を拒否した読み出しは、代わりにその 1 つの編集を
取り除く (`PlanHistory.retract`): 戻した各キーを持つ最新の
エントリが拒否した値をまだ持っていればそのキーを外し、空になったエントリはスタックから外し、ベースラインは拒否を
取り込むので、それ自体が編集として記録されることはない。いずれにせよスタックを破棄する読み出しでは取り除く対象が
残らない。エントリが拒否より長く残りうるのは、スタックを残す読み出し — `sideEffect: "refetch"` の書込が行う
refetch — の方である。読み出しが着地しなかった
取得・ライブ同期開始 (キャンセル・失敗・読み出し前の中止、および完全な読み出しだけを採る場合の不完全な読み出し —
ライブ同期開始と、機種切替を伴う取得) は、開いたままのエントリも含めてスタックにもベースラインにも触れない。
読み出しが着地した場合は、値が何も変わらなくても、その時点で開いているエントリも含めて両方を破棄して取り直す。
これは読み出しが着地した時点で行うので、その後 — 取得がリンクを解放している間、ライブ同期開始がセッションを登録している
間 — の編集は独自のエントリを開き、アンドゥできる。
...
consent-box 系 (`.prefs-box`・`min(1180px, 100%)`・2 カラム) で、ボックス自体はスクロールしない —
題を上、「閉じる」を下に固定した固定列で、あいだのグリッドだけがスクロール領域なので、
ウィンドウの高さが小さくても「閉じる」がスクロールの向こうに隠れることがない。各設定は変更した瞬間に適用されるため、ボックス外の
...
遅延ロード) に永続化される。言語とテーマだけは例外で、独立したキー (`urx-lang` /
...
- **言語とテーマ** — UI 言語 (ネイティブ名のドロップダウン。言語の追加に備える) と 3 値の
  テーマモード (自動 / ダーク / ライト。表示テーマの節を参照)。狭い幅ではツールバーが横スクロール
  バーを出してしまうため、両行はツールバーではなくここに置く。
...
- **警告** — 未検証ファームウェアの確認と、サンプルレート制限 / Ducker バイパスの警告カード
  ...
  常に効く。
- **操作** — ホイール 1 ノッチあたりのステップ数 (各 detent はそれでも各コントロールの
...
保存・読込はブラウザではブラウザ標準 (Blob ダウンロード / file input) で行う。
デスクトップ版はネイティブの保存/読込ダイアログ (`tauri-plugin-dialog`) と最近使った計画を加える。
...
中止した場合は何も試行せず項目は残る。
`sampleRate` / `nodeNames` / `nodeColors` / `hidden` / `notes` / `noteCollapsed` フィールドは省略可能 (持たないファイルは読込時に既定値 — 空配列・空オブジェクト等 — を補う)。
読込 (`deserialize`) は壊れた入力に 2 段階で寛容。コレクション自体が想定の器でなければ空既定へ落とし (`positions` も含め対称)、器が正しければ要素ごとに検証して不適合な要素だけを捨てる (文書自体は拒否しない) — null・型違い・未知の `kind` を持つ結線、有限数でもブールでもないノードパラメータの葉、文字列でないメモ・名前・色、文字列でない hidden / noteCollapsed の id、座標が両方とも有限でない position。これにより手編集・生成ツール・旧バージョン由来の壊れた値が plan へ流入してルーティング不変条件を崩したり、投げるフォーマッタに届いたりすることを防ぐ。オブジェクトとして書かれたメモは、この検証が無ければ問題なく読み込まれ、最初の描画でキャンバス を落とす。

### プランが記述しない FX チャンネル

`planToCommands` が FX チャンネルを送信するのは、プランがそのチャンネルの `fxEffect` を持つときだけである
が、**ローダーは不在のまま残さない**。`fillFactoryParams` がドキュメントを機種の工場出荷値で補完するため、
ドキュメントが何も言っていないチャンネルも、そのチャンネルの工場出荷エフェクトを持って書込経路に届く。
したがって沈黙は「このチャンネルは実機が持っているままにする」という表明にならない。実機のエフェ
クトを保つプランは、その値を自分で持つ — 実機からプランを取得してから編集する。

そこでの書込の代償は取り返せない。EFFECT TYPE の書込はそのタイプの既定値でエンジン配列を埋め直し、元の
タイプを選び直しても元の値は戻らない。そこで判断はドキュメントの沈黙が代わりに下すのではなく、操作者の前
に置かれる。`app/unauthored-writes.ts` が、書込が動かすアドレスのうち**その値を操作者が選んでいない**もの
を持つストリップを報告し、書込の確認ダイアログがそれらを名指しする。対応付けは `planToCommandOrigins` が
担う: ノードパラメーターの読み出しを記録するプランでエミットを 1 回走らせ、コマンドとその値の両方を手に
する唯一の場所である `rawCommand` が、組み立てと同時にキーをコマンド自身へ書き込む。傍らの表ではなく
コマンドに載せるのは、共有アドレスの統合が生存コマンドの**複製**を返すためである。

コマンドを名指すのは、エミットがまだ使っていない読み出しである。値は担うコマンドが組み立てられる直前に
読まれ、カーソルは毎回の組み立てで消えるので、その間に他は入らない。複数のコマンドを作ってからまとめて
push するヘルパー (EQ 1-knob の ON・TYPE・LEVEL は 1 本の連なりで出る) でも、それぞれが自分のキーを持つ
— push 側で読む形ではこうならない。**コマンドが運ぶ値がキーの持つ値と一致する必要は無い**: エミットは反転
し (SSMCS の COMP / EQ は ON を 0 として送る)、丸め、範囲に収める。どれもそのキーの値が出て行く姿である。
1 つの値を全リンクインスタンスへ送る連なりは、直前のコマンドから名前を引き継ぐ — ただしパラメーター名も
一致することを条件とする。値だけで一致を見ると、ノードパラメーターではなく接続から来るチャンネルフェー
ダーが、直前に読まれた 0 を持つパラメーターに結び付いてしまう。

答えは 2 つではなく 3 つである。プランが持たないキーは何も名指さない — エミットは送るかどうかを決める前に
問い合わせ、その後は自分で値を供給するので、それは誰かが選んだ値ではない — ただしルーティングのセレクタは
例外で、値は受け口へ入る結線なので、その結線がアプリの補ったものであるときは受け口を名指す (「STREAMING の
ソースを記述しないプラン」節)。**記録を全く持たないコマンド**が
3 つ目で、これは「誰も供給していない値」ではなく「注記が保証できない値」として読む。両者を同じに畳むと、
見失ったことが黙って出て行く。

コマンドを**左右するだけ**のキー (チャンネルの insert-FX セレクタ、comp/EQ の順序) は別扱いを要さない。
それは自分自身のコマンドの値を持ち、しかも同じノードにあるので、書込が実際にセレクタを変更するならその
アドレスが先にストリップを名指す。変更しないなら、操作者が作り込んだエンジンパラメーターは操作者のもの
のままである。`plan_tool.py` は同じことを 1 段手前で、生成したプランの著者に告げる。

記録が正しいことを言うのは `command-origin.contract.test.ts` で、言い換えではなく
測定する: 工場出荷値で補完したプランの全リーフを 1 つずつ、両方向に動かし、また取り除いて、値が追随した
アドレスをそのリーフのものとする。キーを刻んだコマンドは本当にそのキーが動かすものでなければならず、
エミット自身の値と呼んだコマンドはどのリーフも動かさないものでなければならない。comp/EQ の両方の順序で
走らせる — 工場出荷の順序では SSMCS が 1 つも出ず、反転する符号化器を通る行が無くなるためである。

セクションが存在する場合、そのチャンネルは丸ごと著者のものになる — **部分的な FX 書込は存在しない**。
セレクタはドキュメントがタイプを名指ししていても していなくても送られ (不在のタイプはそのチャンネルの工場
出荷タイプに解決される)、パラメータースロットも全てそのタイプの既定値で一緒に送られる。タイプ書込はどのみ
ちプランが省いたスロットを埋め直すからである。したがってパラメーターを 1 つだけ名指ししたセクションは、タイプを
名指ししたドキュメントと全く同じようにエフェクトをリセットし、`fxEffect.params` だけを省いても何も保たれない。

**ローダーが補完したドキュメントでは、パネルと配線が一致する。** `inspector.ts` は不在の `fxEffect` を `{}`
として読み、解決したタイプを描く。各記述子の既定値は調整画面が描く。補完はその同じ値をプランへ入れるので、
行とコマンドは 1 つの数値を運ぶ。**エミット**をパネルに合わせるのがもう一方の閉じ方で、そちらは破壊的である
ため、補完は**ロード時**に行う。そこでなら、それが何を意味するかを操作者に伝え
られる。補完が走らない場所では 2 つはなお食い違う: デバイス読み出しが答えられなかったノードは意図的に疎の
ままであり、インスペクタが不在値に使う代替値は必ずしも機種の工場出荷値ではない。

### STREAMING のソースを記述しないプラン

本体の STREAMING のソースリストは STEREO・MIX 1・MIX 2 だけで None を持たないため、アプリはどのプランにも
STREAMING の結線をちょうど 1 本持たせる (device-model.md §4)。**ソースを記述しない文書は読み込み時に**工場出荷の
STEREO → STREAMING **で補い** (`requiredSourceProblems` / `applyRequiredSources`)、ステータス行で読み込み自体の
メッセージより前にその旨を伝える — 次の書き込みが、文書が名指ししていない選択を送るためである。STREAMING への
結線は保存された種別を問わず数える (取り付け時に規則表から種別を書き直すため)。サニタイザが落とした結線は
数えない。シーン単位の文書はソースを意図して含まない: 同じ機種のプランの上で開けばシーン外の値と同じくその
プランのソースを保ち、別の機種の上では他の文書と同じく補う。`emptyPlan` も同じ結線を持つので、無から作る
プラン — 新規プラン・機種切替・self-test の取り込み — は最初からこれを持つ。

読み込みが補った結線は補完のものとして記録する: 結線自身の競合名 (`connectionContestKey`。編集やデバイス読み出しが
結線を記録するときの名前) の下に `default` とする。ルーティングのコマンドはパラメータのキーを持たない — 値は受け口へ
入る結線である — ので、書き込みが実機を読み込みの補ったソースへ動かすとき、書き込み確認が STREAMING を名指しするのは
この記録による (`app/unauthored-writes.ts`)。書き込み確認が名指しするルーティングのセレクタはこれだけである: それ以外の
結線は、操作者が描いた・読み込んだ・読み出したとおりに盤面にある。シーン単位の文書がソースを引き継ぐときは、この記録も
一緒に引き継ぐ。

**実機の読み出しは実機が持つ値を取り、取れなかったものはそう伝える。** STREAMING のソースの NONE は本体の一覧には無く、ソフトウェアの
書き込みでは至る状態で、読み出しはそれを自身の view では結線無しとして反映し、名指しする
(`ReadbackResult.unsourced`)。Fetch とライブ同期の開始はそのとき、代わりに STEREO をプランに与え
(`supplyRequiredSources`)、読み込みの補完と同じく `default` として記録し、本体の STREAMING が一覧に無い状態だった旨の
メッセージをステータス行の先頭に置く。読み出し自身の view は実機の状態を保つので、ライブセッションが開始時に取る
スナップショットはそこに値を持たない。開始は何も送らず、実機は NONE のままで、操作者の次の編集が始める flush が STEREO も運ぶ。Device → Write は実機をそこへ動かし、
その書き込み確認は STREAMING を名指しする。読み出しの後で失敗したライブ同期の開始は、読み出しの他の値とともにその補完を
残し、そのメッセージは失敗を報告するダイアログの横でステータス行に残る。そのリストに無いソース — チャンネルのスロット — がデコードされたときは
一切取らない: プラン自身の結線を残し、STREAMING は読み出し未完了となる (`ReadbackResult.sourceUnread`、ノードは
`unreadNodes`)。したがって Fetch は部分的な読み出しを報告し、ライブ同期は開始しない。

NONE を見つけた follow の読み出しと `.urxf` 取り込みは、プランを STREAMING の結線が無いままにする。その結線を取り除く
アンドゥ / リドゥ — そのようなプランへ描画したソースのアンドゥ、または実機の読み出しが STREAMING のソースを動かした後に
再生するエントリ — も同じである。そのとき emit はこのセレクタへ何も送らず (NONE は送らない)、アドレスを follow 専用
として登録する (`planToFollowOnlyAddrs`)。実機は自身のパネルで選んだソースを、書き込みのときと同じくそのアドレスで
半分ごとに 1 回ずつ告知する (実機で測定) ので、そのソースはその登録を通じてプランへ届く。

### 書込経路が正規化した値をプランへ取り戻す

正規化された書込は、プランが実機の状態と違う設定を名乗ったまま残す: 書込経路はコントロールが受理する値を
送り、実機はそれを取り、プランは渡された raw を持ち続ける。`comparePlan` はこの差を報告できない — 比較側も
正規化した値を使うので、実機と一致していると読むからである。読込経路は文書を修復するので、ここへ到達する
のは**デバイス読み取り**だけ: 実機自身のエンコーダーは窓で止まるが線は止まらず、旧ビルドのこのアプリは
そこへ raw を書けた。

書込の後、プランはその値を取り戻す。判断材料は**実機が確定したアドレスの集合**であって、書込が成功したか
どうかではない。両者が乖離するのはまさにここで、スナップショットが既に実機と一致しているライブセッション
は何も送らないため、当該アドレスを一切運んでいない flush が成功しうる。走行の成功を根拠にすると、
どの書込も触れていない値を取り込んでしまう — 無関係なフェーダー操作でそうなる。

`confirmedAddrs` (`control/client.ts`) がその集合で、converge 自身の**台帳**から読む: 送信が受理され、
かつその後の読み取りが実機をプランの値で保持していると見たアドレスが入る。**読まれたこと**が要となる半分で、
読み取り失敗を引くだけでは足りない — diff は最初の失敗で止まるので、それ以降のアドレスは差分にも失敗にも
入らないまま送信済みとして残り、確定と報告されてしまう。台帳はラウンド終了時に組み直すのではなく**常に現在の
状態**に保つ: すべての読み取りが進行に合わせて書き込み、**着地した書込**が自分の上書きしたアドレスを
台帳から落とす — 着地したものが `sideEffect` の頭なら全アドレスを落とす。そうした書込が何を動かすかは
カタログが列挙するものではなく、このループが再読出しで収束させる対象だからである。無効化するのは
**ラウンドではなく送信の決着**である: ブローカーが明示的に却下した書込は何も動かしておらず、送信前に
キャンセルで止まった書込も同じで、他のアドレスについて先行の読み取りが確立した事実はそのまま成り立つ。
ラウンドが**計画された時点**で無効化すると、後続ラウンドの sideEffect コマンドが拒否されるたびに
確定済みの値を捨ててしまう。

**「書込が着地していない」と言えるのは明示的な拒否だけである。** `vd.rs` の `do_set` は**先に送信して
から**応答を待つので、応答側で何かが起きた時点で要求は既に線上にある。`SendOutcome.result` がこの
3 値を持つ: `accepted` (response_code 200)、`refused` (200 以外のコードで返答 = 却下されたと言える唯一の
失敗)、`unknown` (応答が来ない、待機中にリンクが落ちた、または応答が使える
コードを持たない — タイムアウト、書込中の device-lost、リンク切断、壊れた応答)。`unknown` は受理と同じだけ無効化し、sideEffect の頭なら全アドレスを落とす — 書込が着地して
リセットまで起きている可能性があるからである。判定は拒否コード 1 つに対する opt-out で、「拒否ではない
失敗」の一覧ではない。今後増えるコードは黙って信用されるのではなく fate 不明側へ落ちる。ここに 2 つの経路が乗る。
**キャンセル**はループから throw で抜けるため読むべき結果が無く、書込フローが台帳自身を持ち、直前までに
実機が確定した分を取り込んで、キャンセル行にその件数を出す。**送信が失敗した**ラウンドは再読出しを行わないが、
それはそのラウンドで受理が何も動かしていないアドレスについては何も言わない — 先行ラウンドが確定した分は
残る。件数はその書込が最終的に書く行に乗り、停止した行も含む。どちらも
機会が二度と巡ってこないため要る: 差が消えるのは着地したその値によってなので、以後の diff は実機が既に
一致していると読み、二度と送らない。ライブの**直接書込**は意図的にこの外に置く:
ack は実機が書込を受理したことを言うのであって保持したことは言わず、converge が覆わなかったアドレスは次の
converge を待てばよい。`paramRangeAddrs` (`control/translate.ts`) は、ノードとキーで報告するローダー側と、
emit が送るアドレスとの接合である。カタログから組み直すのではなく emit 自身の出力を引くので、FX スロットの
アドレッシングが変わってもここが古い答えを返すことはない。

値は編集として押し込むのではなく、デバイス側の再計算が既に使う座席 (`authorFromDevice`) を通して
**デバイスから**著作する: この値は操作者のものではなく書込経路のものであり、書けない raw を戻すアンドゥは
...
理由による (`plan-validate.ts` の SCOPE 注記: 窓が実際に動いたのは FX カタログである)。
走査が扱う 2 つのノードキー — HI-Z チャンネルの +48V と A.Gain — は取り戻さない: `paramRangeAddrs` が
答えるのは FX の `params` 項目だけで、emit はこの 2 キーを plan が持つとおりに送る。

**デシリアライザで捨てるのではなく書き換える値は 1 つだけ**で、書き換え自体はもう 1 種ある — ローダーが
検証の後にもう 1 層下で行う、アプリが書き込めない範囲にある FX 値の丸めと、丸める先が無いものの**削除**:
有限数でない葉 (既定値を推測せず削除し、選択中タイプ自身の既定値に委ねる)、どのメニューにも無い `type`、
そしてオブジェクトでない `fxEffect` と `params`。この最後の 2 つがあるために対象は葉の外へ及ぶ — 上段の
サニタイザはどのキーの下でもブールと空でないオブジェクトを残すので、読めないエフェクトオブジェクトが
そのまま読み込まれ、以降の読み手はすべてそれを「無い」ものとして扱う。真値ならさらに悪く、書込経路が
実機の設定の上にそのチャンネルの工場出荷時の既定値を送る。どちらの動作も報告する (`plan-validate.ts`) — 1 つの
件数ではなく 2 つの文で。同じ段で、HI-Z がオンのチャンネルの 2 つのキーも丸める — +48V をオフに、A.Gain を
+40 dB に (`input-lock.ts`) — そして丸めた FX 値と一緒に数える。デシリアライザでの
1 つ: **ノード名**は **8 文字**に切り詰める。実機の CH SETTING 名入力画面が受け付けるのがそこまで (`ch 1xxxx`) だからで、捨てれば長いという理由で名前を失い、実機が作れない長さをそのまま通せば盤面のラベルが隣のノードに被る。スタックの他のどこもこれを強制しない — URX44V 実測で、ブローカーは 20 文字の名前を受け取ってそのまま読み戻し、設定ファイルの 64 バイト NUL 埋め要素は器であって上限ではない (**この幅を上限と読むのは誤りである**)。名前は、実機へ文字列のまま送る plan の文字列としても 1 つだけである (`planToNameWrites`: SSMCS の Sweet Spot Data も同じ経路を通るが plan は数値の番号で持ち、送りの `tap` のような列挙の文字列は数値で出る)。数値の葉には `boundRaw` があるが、文字列にはそれに当たるものが無い。同じ切り詰めは emit 側でも適用する — 名前はデバイス読み取りと実機側でのリネームからも plan に入り、どちらもこの関門を通らないため。メモと色はアプリ自身のものなので上限を設けない。
...
それ以外の真に存在しない param は読み取り失敗として表面化する。
...
失って白紙アイコンになる。NSIS のショートカットは `$INSTDIR\urx-router.exe` を直接指し、アイコンもその
実行ファイルから取る。更新ではショートカットに手を付けない — `/UPDATE` のときアンインストーラーはピン解除と
削除のブロックを飛ばし、インストーラーは作り直しを飛ばし、インストール先は前回のインストール先に戻る
(`pnpm-lock.yaml` が解決する `@tauri-apps/cli-v2.11.4` タグの tauri-bundler `installer.nsi` —
installer.nsi:823-849・:914-954・:897-901)。ピン留めしたショートカットの参照先は更新がその場で書き換える
パスそのものなので、これが起きない。
...
**バイナリが記録する macOS のプラットフォームバージョンは、ビルド機から取るのではなく
`src-tauri/.cargo/config.toml` が宣言する。** このファイルの `-platform_version` リンク引数が
`LC_BUILD_VERSION` の両方 — 動作要件の下限と SDK バージョン — を決め、cargo がリリースビルドにも
`pnpm tauri dev` にも適用するため、各々がどの Xcode で走っても両者は一致する。SDK 側の値が何を決めるかは
「表示テーマ」を参照。環境変数 `RUSTFLAGS` はこのテーブルに追加するのではなく**置き換える**ため、
...
SDK で crate をコンパイルするため。その選択の背後にあるランナーイメージの提供終了の日程はワークフローのヘッダにある。
...
ランナーに関係なく全ターゲットのクレートを残す。2026-09-24 に `a94d0b28`・cargo-about 0.9.1 で実測:
macOS arm64 で生成した通知と post-merge の走行が ubuntu-latest で生成した通知はバイト一致し、どちらも
Windows 専用の `windows-sys` と `webview2-com`、macOS 専用の `objc2-app-kit`、Linux 専用の `webkit2gtk` を
含む)、ランナーごとに cargo-about をソースビルドせずに済む。
