  ...
  **オブジェクト**の削除だけが送信内容を変え、それは安全な向きである (「プランが記述しない FX
  ...
  - `src/core/midi/` — 外部 MIDI コントロール (デスクトップのみ)。`message.ts` CC / ノート / ピッチベンドの
    ...
    **メッセージ前の状態から**決めてから書く — あるメンバーの適用が別のメンバーのコントロールを動かしうる
    ...
    コントロールを動かすとき、どちらが勝つかを learn 順序が決めることはない**。ギャング一般が順序非依存になる
    わけではなく、例外が 2 つある。どちらも head の設計であって見落としではない — ピックアップの交差判定は
    head が所有するので、値の違うメンバーを束ねたピックアップのギャングは head の値で交差し、その値がどれかを
    learn 順序が決める。もう 1 つは互いを**ミラー**するメンバーで、最後に書かれたものに落ち着くため、開始時点で
    値が食い違っていると (食い違ったまま保存されたプランは無警告で読み込める) learn 順序が最後に置いた方の値に
    なる。上の順序付けが決めるのは**誰が誰より先に書くか**であって、食い違うメンバーを一致させることではない
