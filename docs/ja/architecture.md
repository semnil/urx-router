...
**converge の実行中、reconcile は待つ** (`DeviceFollowHooks` の `deferReconcile` → `live.isConverging`)。理由は
2 つで、1 つ目はリンクが 1 本でなく帯域が無限でも残る: converge は書込スコープ全体をラウンドごとに書き直しており、
そこで取る読みはアプリ自身が変えている最中のデバイスの読みになる。2 つ目はリンクそのもの — ラウンドが走る間、
2 つの読み手が 1 本のリンク上で交錯し、レースハーネスはこれを不変条件 4 として報告する。**flush 全体ではなく
converge だけ**である: 通常の flush は数回の書込のみで読みを持たず、その隣で走る reconcile はアプリが日常的に
持つ 2 本のチェーンの競合であって、動いているデバイスの読み手ではない。窓は**消費せず保持する**: バーストが
名指ししたノードは settle タイマー自身の再アームで読み直され、その再アームが待ちを終わらせる契機でもある
(`follow.ts` には converge の終了を聞くものが無い)。`IDLE_FULL_MS` を跨いで先送りされたパスは、再試行する
タイマーに引きずられて縮退せず全体読みのまま走る。
...
フェーズであり、その間 reconcile は待たされる (`live.isConverging`): 1 本のリンクに読み手が 2 つ載る形が、
