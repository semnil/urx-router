...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  (`channelDuckerOn` = PRE 送りの注記、`duckerBypassWarnings` = USB ダイレクトアウトのpre-fader タップ警告。
...
- `src/ui/` — `graph.ts` SVG ノードグラフ (スタジオラック調の意匠、ダーク / ライトテーマ。`refresh()` は
  ...
  **後**に設置する。これがコードログに「アプリがそのコードを取ったか」を報告させている / `dom.ts` 共有 DOM
  ...
  開いたノードを調整しない。Duckerはステレオチャンネルの下にぶら下がる別ノードで、キーはよそから来た
...
- **実機側操作の追従 (device follow)** — ライブ同期の逆方向。同じ排出経路で**実機側のパラメーター変更**も
  ...
  その親へ retarget する。これを欠くとチップは全体再描画まで更新されない。GRAPH はDuckerが独自ノードなので retarget
...
- **ラーン** — ウィンドウの Learn をオンにするとアーム可能コントロールに破線リングが出る。CONSOLE のストリップ
  ...
  (割当リストは後続行に「連動」タグを付ける)。ぶら下げノード (ステレオ CH 直下のDucker・ラベルは「Ducker」
...
**警告カードは意図的に作っていない**。ここが、その他の点では対応関係にあるDucker バイパスとの
分岐点になる。あちらは plan 内の矛盾 (`duckerOn` が ON かつライブ出力へのpre-fader タップ) で
