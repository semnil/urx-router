...
- **実機側操作の追従 (device follow)** — ライブ同期の逆方向。同じ排出経路で**実機側のパラメーター変更**も
  ...
  **実機読み出しはすべてプランの専用コピーに対して走り** (`readback.readIntoPlan`)、アンドゥの差分器を通して
  ...
  数十秒に及ぶ窓の中で操作者が行ったジェスチャを上書きできなくなる。snapshot の再基底は同じ規則のもう半分で、
...
CONSOLE ビューのコントロール (フェーダー / 送りレベル / MUTE / PAN・BAL / GAIN / PHONES / 各トグル) は外部 MIDI
コントローラーから操作できる (デスクトップ版のみ)。設定は Device メニュー →「MIDI コントロール」の非モーダル
パネルで行う (パネルを開いたまま console のコントロールをクリックして割り当てる)。パネルは ✕ のほか
パネル外のクリックでも閉じる (ラーン中を除く — その間のパネル外クリックは割り当て操作)。割り当てリストの
per-mapping セレクト (取り込みモード / ボタン動作) は、ホバー / フォーカス中の行にアンカーする凡例カードで
選択肢の挙動を説明する — 行の直下 (ビューポート下端が近ければ直上へフリップ) に出すため、行とセレクト自体は
隠れない。ヘッダは設定名と対象の割り当て名を示す (下 / 上フリップの配置は `ui/dom.ts` の `popTop` で console
の popover 群と共有)。
...
- **受け口 (Rust)** — `src-tauri/src/midi.rs` が midir (macOS = CoreMIDI / Windows = WinMM) でポートを列挙し、
  ...
  無いため、ポート一覧はパネルを開くたびに再列挙する。開けなかったポートはエラーを status 行に出し、セレクトを
...
- **マッピング (core/midi/)** — 言語非依存の純ロジック。`message.ts` が CC / ノート / ピッチベンドを
  ...
  バリデーション (削除した「relative」取り込みモードの永続マッピングは読込時に absolute へ移行し、STEREO / MONITOR の電源 LED の旧「mute」id は一律「chOn」へ移行)、`controls.ts` が CONSOLE の全コントロールを**固定コントロール id** (`node/param[@送りターゲット]`、
  例 `ch1/level@bus.mix1`) で列挙・解決する。固定 id なので表示中のビューや SENDS ラックの開閉に依存せず、
  「CH 1 のメインフェーダー」と「CH 1 → MIX 1 送り」は別コントロールとして個別に割り当てる (ラックの
  有効チップ / PRE ボタン / 列フェーダーは同じ送りスコープ id を arm し、MIX 送りの PRE/POST 用に `tap`
  コントロールを追加)。値は正規化 (0..1)
  で越境し、set 時に console と同じグリッド (`levels.ts` の level_gain / GAIN の dB 範囲 / PAN ±63 / PHONES 0.1
  刻み) へスナップする。デバイスロック (FIXED バスの送りレベル・Pan Link の送り pan・176.4/192 kHz のステレオ
  CH EQ) は書込みを拒否する。
...
- **ラーン** — パネルの Learn をオンにすると console のアーム可能コントロールに破線リングが出て、クリックで割当
