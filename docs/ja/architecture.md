...
- **エンジン (`engine.ts`)** — 受信イベントをマッピング経由でコントロールへ適用する。取り込みモードは per-mapping
  ...
  モードの代わりに per-mapping のボタン動作がある: 既定の「押下で反転」(ノート on / CC 立ち上がりエッジ ≥64 で
  反転・離すは無視 = モーメンタリボタン規約) と「値に追従」(ノート on / CC ≥64 = ON、それ以外 OFF。押すたびに
  127/0 を交互に 1 通ずつ送る送信元 — 例: Elgato Stream Deck MIDI プラグインのトグルボタン — はエッジ検出では
  1 押しおきに無視されるため、こちらを選ぶ)。
