...
- `src/core/` — `plan-history.ts` アンドゥ / リドゥの背後にある可逆なプランパッチ (**スナップショットではなく
  ...
  - `src/core/control/` — 実機制御 (vd プロトコル)。デスクトップでは書込とライブ同期は常時有効で、
    ...
    - `follow.ts` 実機側操作の盤面追従 (param notify を購読 → `live.lookup` で分類する: direct = ノードローカルな
      ...
      安全側の全体読み戻しが走る。リネームは 4 つ目の経路であり、上の各フィルタより**前**で処理する:
      `isEcho` は数値のスナップショットを読むが、そこには名前のエントリが無いので、あらゆるリネームを —
      アプリ自身のものも含めて — 実機側の変更と呼んでしまう。`applyName` がエコー判定と適用の両方を所有し、
      所有ノードを返すので、リネームは direct 追従のまま (再描画 1 回、読み戻し無し) でいられる。名前では
      ないアドレスへの文字列 notify は、未知アドレスの経路へ落ちる)
