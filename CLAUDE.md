...
- `docs/en/` + `docs/ja/` — `device-model.md` (grounding for the routing rules) / `architecture.md` (architecture) / `known-issues.md` (list of limitations that cannot be reflected on the device) / `console-sends.md` (confirmed design spec for the CONSOLE per-strip SENDS rack that replaces the "Send to" tabs; implementation pending). Keep English and Japanese in sync
