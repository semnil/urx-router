...
export function paramRangeProblems(plan: Plan): ParamRangeProblem[] {
  ...
  const take = (
    ...
    // Rounded first, then the window. A raw is a broker integer and every one of these controls
    // steps by one, so a fractional value names no setting the unit has — and it is not dropped,
    // because unlike a boolean it HAS a numeric neighbour: dropping sends the type's own default
    // instead, which for the delay LPF is 8.50 kHz where the document said 118 Hz.
    //
    ...
    const bound = Math.min(Math.max(r, lo ?? r), hi ?? r);
