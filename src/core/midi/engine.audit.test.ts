...
describe("gang pickup with a stale list head", () => {
  ...
  // pickupEngaged for isHead; members read the cached engaged flag). apply()
  // returns early when the head's control resolves to null, so pickupEngaged
  // never runs and the pickup state for that address is never created.
  it("AUDIT: swallows every pickup member forever when the gang head is stale", () => {
    const member = fake("ch2/level@bus.mix1", "continuous", 0.5);
    controls.set(member.id, member);
    const maps: MidiMapping[] = [
      // Head = a control that does not resolve for this plan (e.g. a send whose
      // connection was removed, or a mapping persisted for another model).
    ...
    engine.onMessage(encodeCc(0, 7, 70)); // crosses 0.5 — with a live head this engages
    // AUDIT: the valid member never engages because the stale head owns the
    // pickup state; a live head would leave the member at ~0.55 here.
    expect(member.value).toBe(0.5);
