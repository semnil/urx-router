 ...
 *
 *  `ssmcs` selects the OTHER comp/EQ order, which the factory does not: that bank is where the
 *  emit INVERTS a switch (its COMP / EQ send ON as 0), and a plan left at the factory order
 *  emits none of it — so the reading below would never see a transform at all. */
function filled(modelId: ModelId, ssmcs = false): Plan {
  const plan = defaultPlan(modelId);
  plan.nodeParams = {};
  fillFactoryParams(modelId, plan);
  if (!ssmcs) return plan;
  for (const node of getModel(modelId).nodes) {
    const params = plan.nodeParams[node.id];
    if (params?.ssmcs) plan.nodeParams[node.id] = { ...params, compEqType: COMP_EQ_SSMCS };
  }
  return plan;
}
...
describe.each(MODEL_IDS.flatMap((id) => [[id, false] as const, [id, true] as const]))(
  "planToCommandOrigins on %s, SSMCS %s",
  (modelId, ssmcs) => {
    const model = getModel(modelId);
    const plan = filled(modelId, ssmcs);
