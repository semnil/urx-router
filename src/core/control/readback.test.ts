...
describe("applyDeviceState round-trip", () => {
  ...
  // channel is one slot twice. The plan holds that pair as two wires, one from each
  // channel, and reading the L half alone made it CH 3: the next absolute write then sent
  // 2/2 and moved the unit off what the operator had selected on it. What the list does
  // not offer — the pair the wrong way round, two channels of different pairs, one half
  // left clear — the plan cannot hold either, and stays unread. And a scope that does not
  // ask reads none of them.
  describe("USB output source, read from both halves", () => {
    ...
    const usbA = (seed: Array<[number, number]>) =>
      new Map(seed.map(([y, v]) => [`${PARAMS.USB_OUT_SRC_A.id}:0:${y}`, v] as [string, number]));
    const sourcesOfA = (plan: Plan) =>
      plan.connections.filter((c) => c.to === ref("out.usbmain_a", "in") && c.kind === "patch").map((c) => c.from);

    it("takes a mono pair as two wires, one from each channel", async () => {
      ...
      ); // CH 3's slot on L, CH 4's on R — the unit's "CH 3/4"

      const result = await applyDeviceState(model, target);

      expect(sourcesOfA(target), "the pair, and nothing the plan held before").toEqual(["ch3:out", "ch4:out"]);
    ...
    it("takes a single mono channel, which is the same slot on both halves — and drops a partner the plan held", async () => {
      const target = pairPlan();
      target.connections = target.connections.filter((c) => c.to !== ref("out.usbmain_a", "in"));
      target.connections.push(
        { from: "ch3:out", to: "out.usbmain_a:in", kind: "patch" },
        { from: "ch4:out", to: "out.usbmain_a:in", kind: "patch" },
      );
      mockVdGetFrom(
        usbA([
          [0, 2],
      ...
      expect(sourcesOfA(target)).toEqual(["ch3:out"]);
      expect(result.unreadNodes.has("out.usbmain_a"), "read in full, so no badge").toBe(false);
    });

    // Every shape the unit's list does not offer, each its own row: the report names the
    // halves in order, and a read of L alone would have taken the half-clear ones for a
    // single channel or a clear.
    const unexpressible: Array<[string, Array<[number, number]>, string]> = [
      [
      ...
      [
        "two channels of different pairs",
        [
          [0, 1],
          [1, 2],
        ],
      ...
      ["only L selected", [[0, 2]], "2 / NONE"],
      ["only R selected", [[1, 2]], "NONE / 2"],
    ];
    it.each(unexpressible)("keeps the plan's wire and reports %s", async (_, seed, ports) => {
      const target = pairPlan();
      mockVdGetFrom(usbA(seed)); // an unseeded half reads NONE

      const result = await applyDeviceState(model, target);

      expect(sourcesOfA(target), "the plan's own wire, not a guess made from the halves").toEqual(["bus.stereo:out"]);
      expect(
        result.errors.some((e) =>
          e.includes(`out.usbmain_a: source ports ${ports} name neither one source nor a mono pair`),
        ),
      ).toBe(true);
    ...
    // URX22 has one mono pair, CH 1/2; its slots 2 and 3 are the stereo CH 3/4, which is
    // one source whose two slots are L and R.
    it("reads URX22's CH 1/2 as a pair and its slots 2 / 3 as the stereo CH 3/4", async () => {
      const urx22 = getModel("URX22");
      const target = emptyPlan("URX22");
      ensureFixedConnections(urx22, target);
      mockVdGetFrom(
        new Map([
          [`${PARAMS.USB_OUT_SRC_A.id}:0:0`, 0],
          [`${PARAMS.USB_OUT_SRC_A.id}:0:1`, 1],
          [`${PARAMS.USB_OUT_SRC_B.id}:0:0`, 2],
          [`${PARAMS.USB_OUT_SRC_B.id}:0:1`, 3],
        ]),
      );

      const result = await applyDeviceState(urx22, target);

      const sourcesOf = (to: string) =>
        target.connections.filter((c) => c.to === ref(to, "in") && c.kind === "patch").map((c) => c.from);
      expect(sourcesOf("out.usbmain_a")).toEqual(["ch1:out", "ch2:out"]);
      expect(sourcesOf("out.usbmain_b")).toEqual(["ch_3_4:out"]);
      expect(result.unreadNodes.has("out.usbmain_a")).toBe(false);
      expect(result.unreadNodes.has("out.usbmain_b")).toBe(false);
    });
    ...
    describe("under the scene scope", () => {
      ...
      it("does not report a shape it was never going to place", async () => {
        const target = pairPlan();
        mockVdGetFrom(
          usbA([
            [0, 3],
        ...
        expect(sourcesOfA(target)).toEqual(["bus.stereo:out"]);
        ...
        expect(all.unreadNodes.has("out.usbmain_a")).toBe(true);
      });
...
describe("applySilentState", () => {
  ...
  // The other half of that guard, and the one that says WHICH parameter each raw is. A
  // family's raws are laid out by its head — an FX channel's EFFECT TYPE, a node's
  // insert-FX selector — and the emit answered with above is laid out by the PLAN's copy of
  // that head. So a selection the operator has made and no flush has carried yet had the
  // outgoing array read as the incoming effect's: every value filed under a key the
  // outgoing one never had, and the write behind the park sending the incoming type's
  // factory values in their place.
  it("reads an FX array against the type the unit holds, not the plan's unsent one", async () => {
    const plan = defaultPlan("URX44V");
    const hpf = fxParams(0).find((d) => d.key === "revxHpf")!;
    const onTheUnit = plan.nodeParams["bus.fx1"]!.fxEffect!.type!;
    const delay = fxEffectTypes(0).find((o) => o.family === "delay")!;
    expect(
      fxEffectTypes(0).find((o) => o.value === onTheUnit)!.family,
      "the premise: the selection crosses families, so the slots mean different parameters",
    ).not.toBe(delay.family);
    const table = deviceTableFor(plan);
    // The plan's own value for this parameter, and then the selection whose flush is still
    // to come — which is the state every EFFECT TYPE park in front of a second selection is
    // in.
    plan.nodeParams["bus.fx1"]!.fxEffect!.params![hpf.key] = 44;
    plan.nodeParams["bus.fx1"]!.fxEffect!.type = delay.value;

    mockVdGetFrom(table);
    // The unit is holding what this session last sent it everywhere, so what lands below is
    // the emit's — which is the arrangement that has to be laid out the unit's way.
    await applySilentState(model, plan, undefined, undefined, () => true);

    const fx = plan.nodeParams["bus.fx1"]?.fxEffect;
    expect(fx?.type, "the unit's own type").toBe(onTheUnit);
    expect(fx?.params?.[hpf.key], "and each slot read as the parameter the unit has there").toBe(44);
  });

  // The same head on the other family, where it decides which ENGINE the raws come from —
  // and where both halves of that show: read against the plan's unsent selector, a node's
  // values are taken off one engine and filed under another's namespace, and the guard
  // above has no value of the unit's family to answer with, so an edit the flush has not
  // carried is replaced by the value it was made against.
  it("reads an insert-FX engine against the selector the unit holds", async () => {
    const plan = defaultPlan("URX44V");
    const was = plan.nodeParams["bus.stereo"];
    // MBC on the unit, holding what this session last sent it.
    plan.nodeParams["bus.stereo"] = { ...was, insertFx: 1792, insertFxOn: true, insertFxParams: { "mbc:14": 97 } };
    const table = deviceTableFor(plan);
    // The operator moves that slot and then picks the compander; neither has gone out.
    plan.nodeParams["bus.stereo"] = {
      ...plan.nodeParams["bus.stereo"],
      insertFx: 1793,
      insertFxParams: { "mbc:14": 90 },
    };

    mockVdGetFrom(table);
    await applySilentState(model, plan, undefined, undefined, () => true);

    expect(plan.nodeParams["bus.stereo"]?.insertFx, "the unit's own selector").toBe(1792);
    // Filed under the bare slot, which is the namespace a read writes: by construction the
    // family the selector named. Read against the plan's selector, this slot comes off the
    // other engine, which is holding nothing.
    expect(
      plan.nodeParams["bus.stereo"]?.insertFxParams?.["14"],
      "the operator's own edit, under the family the unit is on",
    ).toBe(90);
  });
  ...
  it.each([
    ...
    // Once. A node whose layout head is what could not be read is left alone rather than
    // asked again by the family pass behind it, which has no head to read it against.
    expect(r.errors, "named once").toHaveLength(1);
