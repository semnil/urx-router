// The MIDI control window's view: MidiUiState in, DOM out, MidiUiIntent back.
//
// Split out of the window's entry point (src/midi-window.ts) so the mapping from
// state to screen — and the intent each control reports — can be driven without a
// window, a relay or a MIDI port. The entry point owns the state and the transport;
// everything here reads `state` and writes into `host`, and the only way out is the
...
/** Report one operator gesture back to the main window. */
export type MidiUiSend = (intent: MidiUiIntent) => void;

type MidiMessages = ReturnType<typeof t>["midi"];
...
export function legend(rows: MidiUiRow[], m: MidiMessages): HTMLElement | null {
...
export function mappingRow(row: MidiUiRow, m: MidiMessages, send: MidiUiSend): HTMLElement {
...
export function renderMidiWindow(host: HTMLElement, state: MidiUiState, send: MidiUiSend): void {
  ...
  if (state.rows.length === 0) {
    ...
    for (const r of state.rows) tbody.append(mappingRow(r, m, send));
