// The GATE processor for the dynamics tuning screen.
//
...
// stayed shut), which is what earns the ladder's one gesture — the threshold is a
...
export const GATE_DYN: DynProcessor = {
  ...
  drawCurve: (c, g, v, tok) => {
    ...
    const clampY = (db: number): number => g.py(Math.max(db, OUT_LO_DB));

    c.strokeStyle = tok["--led"];
    c.lineWidth = 2;
    c.beginPath();
    c.moveTo(g.px(LO_DB), clampY(LO_DB + drop));
    c.lineTo(g.px(thr), clampY(thr + drop));
    c.stroke();
    c.beginPath();
    ...
    c.fillText(shown, g.px(thr) + (right ? -6 : 6), Math.min(clampY(thr + drop) - 6, g.h - 32));

    c.strokeStyle = tok["--led"];
    c.globalAlpha = 0.35;
    c.lineWidth = 1;
    c.beginPath();
    c.moveTo(g.px(thr) + 0.5, 14);
    c.lineTo(g.px(thr) + 0.5, g.h - 28);
    c.stroke();
    c.globalAlpha = 1;
    c.fillStyle = tok["--led"];
    c.textAlign = "left";
    c.fillText(formatDyn(thr, "db"), Math.min(g.px(thr) + 5, g.w - 74), 25);
  },
};
